...
function broadcastFrom(from: MwNode, targets: { [nodeId: string]: MwNode }) {
...
describe("MwNode", function() {
    ...
    describe("Serialization", function() {
        it("should add the file to Bob's room", function() {
            ...
            const tNode = new MwNode(sId, sff);
            tNode.rehydrate(value);

            // Use tNode from now on.

            // Server synchronizes the file back to Alice.
            aNode.setEdits(tNode.getEdits(aNode.id));

            const bNode = new MwNode(bId, eff);

            syncFromTo(tNode, bNode);
    ...
    describe("Broadcast", function() {
        it("should add the file to Bob's room", function() {
            ...
            const sNode = new MwNode(sId, sff);
            const clients: { [nodeId: string]: MwNode } = {};
            clients[aId] = new MwNode(aId, eff);

            const aEditor = new EditorFile();
            aEditor.setText(initialString);
            clients[aId].addEditor(fileX, aEditor);

            // Alice pushes.
            syncFromTo(clients[aId], sNode);
            // Server later acks to all clients.
            broadcastFrom(sNode, clients);

            clients[bId] = new MwNode(bId, eff);
            syncFromTo(sNode, clients[bId]);
            // Bob acks back to the server.
            syncFromTo(clients[bId], sNode);
            const bEditor = clients[bId].getEditor(fileX);
