...
function assertTrue(actual: any) {
...
function assertEquivalent(expected: any, actual: any, msg?: string) {
...
describe("diff", function () {
    ...
    describe("Conversion of diffs to delta and back", function () {
        it("diffsToDelta and diffsFromDelta", function () {
            ...
            const diffs = [[DIFF_EQUAL, "jump"], [DIFF_DELETE, "s"], [DIFF_INSERT, "ed"], [DIFF_EQUAL, " over "], [DIFF_DELETE, "the"], [DIFF_INSERT, "a"], [DIFF_EQUAL, " lazy"], [DIFF_INSERT, "old dog"]];
            const text1 = dmp.sourceText(diffs);
            ...
            assertEquivalent(diffs, dmp.deltaStringToDiffs(text1, delta));

            // Generates error (19 != 20).
            try {
                dmp.deltaStringToDiffs(text1 + "x", delta);
            ...
            try {
                dmp.deltaStringToDiffs(text1.substring(1), delta);
                assertEquals(Error, null);
            } catch (e) {
                // Exception expected.
            }
            ...
            text1 = dmp.sourceText(diffs);
            assertEquals("\u0680 \x00 \t %\u0681 \x01 \n ^", text1);

            delta = dmp.diffsToDeltaString(diffs);
            ...
            const text2 = dmp.resultText(diffs);
            ...
            // Convert delta string into a diff.
            assertEquivalent(diffs, dmp.deltaStringToDiffs("", delta));
        });
    });
...
describe("patch", function () {
    ...
    describe("computeNullPadding", function () {
        it("should generate a string of possibly non-printing characters.", function () {
            ...
            expect(dmp.computeNullPadding(2)).toBe([String.fromCharCode(1), String.fromCharCode(2)].join(''));
        });
    });
 ...
 *
 * Copyright 2016 David Geo Holmes
