...
(function(process) {
  ...
  function Yargs(processArgs, cwd, parentRequire) {
    ...
    if (!cwd)
      cwd = process.cwd();
    self.$0 = process.argv.slice(0, 2).map((x, i) => {
    ...
    self.resetOptions = self.reset = function resetOptions(aliases) {
      ...
      groups = {};
      const arrayOptions = ['array', 'boolean', 'string', 'requiresArg', 'skipValidation', 'count', 'normalize', 'number'];
      ...
      try {
        ...
        if (argv._.length) {
          if (handlerKeys.length) {
            ...
            for (let i = (commandIndex || 0),
                cmd; argv._[i] !== undefined; i++) {
              cmd = String(argv._[i]);
