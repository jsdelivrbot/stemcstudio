 ...
 * License: MIT
 */
!function(global,factory){"object"==typeof exports&&"undefined"!=typeof module?factory(exports,require("@angular/core"),require("@angular/platform-browser-dynamic")):"function"==typeof define&&define.amd?define("@angular/upgrade",["exports","@angular/core","@angular/platform-browser-dynamic"],factory):factory((global.ng=global.ng||{},global.ng.upgrade={}),global.ng.core,global.ng.platformBrowserDynamic)}(this,function(exports,_angular_core,_angular_platformBrowserDynamic){"use strict";/**
 * @license
 ...
 */
function noNg(){throw new Error("AngularJS v1.x is not loaded!")}function onError(e){throw console.error?console.error(e,e.stack):console.log(e,e.stack),e}function controllerKey(name){return"$"+name+"Controller"}function directiveNormalize(name){return name.replace(DIRECTIVE_PREFIX_REGEXP,"").replace(DIRECTIVE_SPECIAL_CHARS_REGEXP,function(_,letter){return letter.toUpperCase()})}function getComponentName(component){return component.overriddenName||component.name||component.toString().split("\n")[0]}function isFunction(value){return"function"==typeof value}function supportsNgModel(component){return"function"==typeof component.writeValue&&"function"==typeof component.registerOnChange}function hookupNgModel(ngModel,component){ngModel&&supportsNgModel(component)&&(ngModel.$render=function(){component.writeValue(ngModel.$viewValue)},component.registerOnChange(ngModel.$setViewValue.bind(ngModel)),"function"==typeof component.registerOnTouched&&component.registerOnTouched(ngModel.$setTouched.bind(ngModel)))}function strictEquals(val1,val2){return val1===val2||val1!==val1&&val2!==val2}function groupNodesBySelector(ngContentSelectors,nodes){for(var projectableNodes=[],i=0,ii=ngContentSelectors.length;i<ii;++i)projectableNodes[i]=[];for(var j=0,jj=nodes.length;j<jj;++j){var node=nodes[j],ngContentIndex=findMatchingNgContentIndex(node,ngContentSelectors);null!=ngContentIndex&&projectableNodes[ngContentIndex].push(node)}return projectableNodes}function findMatchingNgContentIndex(element,ngContentSelectors){for(var ngContentIndices=[],wildcardNgContentIndex=-1,i=0;i<ngContentSelectors.length;i++){var selector=ngContentSelectors[i];"*"===selector?wildcardNgContentIndex=i:matchesSelector(element,selector)&&ngContentIndices.push(i)}return ngContentIndices.sort(),-1!==wildcardNgContentIndex&&ngContentIndices.push(wildcardNgContentIndex),ngContentIndices.length?ngContentIndices[0]:null}function matchesSelector(el,selector){if(!_matches){var elProto=Element.prototype;_matches=elProto.matches||elProto.matchesSelector||elProto.mozMatchesSelector||elProto.msMatchesSelector||elProto.oMatchesSelector||elProto.webkitMatchesSelector}return el.nodeType===Node.ELEMENT_NODE&&_matches.call(el,selector)}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function downgradeComponent(info){var directiveFactory=function($compile,$injector,$parse){var ngZone,needsNgZone=!1,wrapCallback=function(cb){return cb};return{restrict:"E",terminal:!0,require:[REQUIRE_INJECTOR,REQUIRE_NG_MODEL],link:function(scope,element,attrs,required){var ngModel=required[1],parentInjector=required[0],ranAsync=!1;if(!parentInjector){var lazyModuleRef=$injector.get(LAZY_MODULE_REF);needsNgZone=lazyModuleRef.needsNgZone,parentInjector=lazyModuleRef.injector||lazyModuleRef.promise}var doDowngrade=function(injector){var componentFactoryResolver=injector.get(_angular_core.ComponentFactoryResolver),componentFactory=componentFactoryResolver.resolveComponentFactory(info.component);if(!componentFactory)throw new Error("Expecting ComponentFactory for: "+getComponentName(info.component));var injectorPromise=new ParentInjectorPromise$1(element),facade=new DowngradeComponentAdapter(element,attrs,scope,ngModel,injector,$injector,$compile,$parse,componentFactory,wrapCallback),projectableNodes=facade.compileContents();facade.createComponent(projectableNodes),facade.setupInputs(needsNgZone,info.propagateDigest),facade.setupOutputs(),facade.registerCleanup(),injectorPromise.resolve(facade.getInjector()),ranAsync&&scope.$evalAsync(function(){})},downgradeFn=needsNgZone?function(injector){ngZone||(ngZone=injector.get(_angular_core.NgZone),wrapCallback=function(cb){return function(){return _angular_core.NgZone.isInAngularZone()?cb():ngZone.run(cb)}}),wrapCallback(function(){return doDowngrade(injector)})()}:doDowngrade;isThenable(parentInjector)?parentInjector.then(downgradeFn):downgradeFn(parentInjector),ranAsync=!0}}};return directiveFactory.$inject=[$COMPILE,$INJECTOR,$PARSE],directiveFactory}function isThenable(obj){return isFunction(obj.then)}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
