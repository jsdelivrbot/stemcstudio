...
(function(process) {
  (function(global, factory) {
    ...
    var VERSION = new _angular_core.Version('4.2.0');
          ...
          this.component.viewQueries.forEach(function(query, queryIndex) {
            ...
            _this.nodes.push(function() {
              return ({
                sourceSpan: null,
                nodeFlags: flags,
                nodeDef: importExpr(Identifiers.queryDef).callFn([literal(flags), literal(queryId), new LiteralMapExpr([new LiteralMapEntry(query.propertyName, literal(bindingType))])])
              });
            });
          });
        }
      ...
      ViewBuilder.prototype.createLiteralMapConverter = function(sourceSpan, keys) {
        ...
        this.nodes.push(function() {
          return ({
            sourceSpan: sourceSpan,
            nodeFlags: 64,
            nodeDef: importExpr(Identifiers.pureObjectDef).callFn([literalArr(keys.map(function(key) {
              return literal(key);
            }))])
          });
        });
        ...
        var Transformer = (function() {
          ...
          Transformer.prototype.visitStringMap = function(map, context) {
            ...
            return new LiteralMapExpr(Object.keys(map).map(function(key) {
              return new LiteralMapEntry(key, visitValue(map[key], _this, context));
            }));
          };
    ...
    var AotCompiler = (function() {
      ...
      AotCompiler.prototype.emitAllStubs = function(analyzeResult) {
        ...
        var sourceModules = files.map(function(file) {
          return _this._compileStubFile(file.srcUrl, file.directives, file.ngModules);
        });
      ...
      AotCompiler.prototype._compileStubFile = function(srcFileUrl, directives, ngModules) {
        ...
        directives.forEach(function(dirType) {
          ...
          ((compMeta.template)).externalStylesheets.forEach(function(stylesheetMeta) {
            generatedFiles.push(_this._codegenSourceModule(((stylesheetMeta.moduleUrl)), _this._createOutputContext(_stylesModuleUrl(((stylesheetMeta.moduleUrl)), _this._styleCompiler.needsStyleShim(compMeta), fileSuffix))));
          });
        });
      ...
      AotCompiler.prototype._createSummary = function(srcFileUrl, directives, pipes, ngModules, injectables, ngFactoryCtx) {
        ...
        return [new GeneratedFile(srcFileUrl, summaryFileName(srcFileUrl), json), this._codegenSourceModule(srcFileUrl, forJitOutputCtx)];
      };
      ...
      AotCompiler.prototype._createOutputContext = function(genFilePath) {
        ...
        var importExpr$$1 = function(symbol, typeParams) {
          ...
          var _a = _this._symbolResolver.getImportAs(symbol) || symbol,
              ...
              name = _a.name,
              members = _a.members;
          var moduleName = _this._symbolResolver.fileNameToModuleName(filePath, genFilePath);
    ...
    function createAotCompiler(compilerHost, options) {
      ...
      var compiler = new AotCompiler(config, compilerHost, staticReflector, resolver, tmplParser, new StyleCompiler(urlResolver), viewCompiler, new NgModuleCompiler(staticReflector), new TypeScriptEmitter(), summaryResolver, options.locale || null, options.i18nFormat || null, symbolResolver);
      return {
        compiler: compiler,
    ...
    var JitEmitterVisitor = (function(_super) {
      ...
      JitEmitterVisitor.prototype.createReturnStmt = function(ctx) {
        var stmt = new ReturnStatement(new LiteralMapExpr(this._evalExportedVars.map(function(resultVar) {
          return new LiteralMapEntry(resultVar, variable(resultVar));
        })));
