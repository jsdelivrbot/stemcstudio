 ...
 */
function forwardRef(forwardRefFn){return forwardRefFn.__forward_ref__=forwardRef,forwardRefFn.toString=function(){return stringify(this())},forwardRefFn}function resolveForwardRef(type){return"function"==typeof type&&type.hasOwnProperty("__forward_ref__")&&type.__forward_ref__===forwardRef?type():type}function resolveProvider(provider){var deps=computeDeps(provider),fn=IDENT,value=EMPTY,useNew=!1,provide=resolveForwardRef(provider.provide);if(USE_VALUE in provider)value=provider.useValue;else if(provider.useFactory)fn=provider.useFactory;else if(provider.useExisting);else if(provider.useClass)useNew=!0,fn=resolveForwardRef(provider.useClass);else{if("function"!=typeof provide)throw staticError("StaticProvider does not have [useValue|useFactory|useExisting|useClass] or [provide] is not newable",provider);useNew=!0,fn=provide}return{deps:deps,fn:fn,useNew:useNew,value:value}}function multiProviderMixError(token){return staticError("Cannot mix multi providers and regular providers",token)}function recursivelyProcessProviders(records,provider){if(provider)if((provider=resolveForwardRef(provider))instanceof Array)for(var i=0;i<provider.length;i++)recursivelyProcessProviders(records,provider[i]);else{if("function"==typeof provider)throw staticError("Function/Class not supported",provider);if(!provider||"object"!=typeof provider||!provider.provide)throw staticError("Unexpected provider",provider);var token=resolveForwardRef(provider.provide),resolvedProvider=resolveProvider(provider);if(!0===provider.multi){var multiProvider=records.get(token);if(multiProvider){if(multiProvider.fn!==MULTI_PROVIDER_FN)throw multiProviderMixError(token)}else records.set(token,multiProvider={token:provider.provide,deps:[],useNew:!1,fn:MULTI_PROVIDER_FN,value:EMPTY});token=provider,multiProvider.deps.push({token:token,options:6})}var record=records.get(token);if(record&&record.fn==MULTI_PROVIDER_FN)throw multiProviderMixError(token);records.set(token,resolvedProvider)}}function tryResolveToken(token,record,records,parent,notFoundValue){try{return resolveToken(token,record,records,parent,notFoundValue)}catch(e){e instanceof Error||(e=new Error(e));var path=e[NG_TEMP_TOKEN_PATH]=e[NG_TEMP_TOKEN_PATH]||[];throw path.unshift(token),record&&record.value==CIRCULAR&&(record.value=EMPTY),e}}function resolveToken(token,record,records,parent,notFoundValue){var value;if(record){if((value=record.value)==CIRCULAR)throw Error(NO_NEW_LINE+"Circular dependency");if(value===EMPTY){record.value=CIRCULAR;var useNew=record.useNew,fn=record.fn,depRecords=record.deps,deps=EMPTY;if(depRecords.length){deps=[];for(var i=0;i<depRecords.length;i++){var depRecord=depRecords[i],options=depRecord.options,childRecord=2&options?records.get(depRecord.token):void 0;deps.push(tryResolveToken(depRecord.token,childRecord,records,childRecord||4&options?parent:NULL_INJECTOR,1&options?null:Injector.THROW_IF_NOT_FOUND))}}record.value=value=useNew?new((_a=fn).bind.apply(_a,[void 0].concat(deps))):fn.apply(void 0,deps)}}else value=parent.get(token,notFoundValue);return value;var _a}function computeDeps(provider){var deps=EMPTY,providerDeps=provider.deps;if(providerDeps&&providerDeps.length){deps=[];for(var i=0;i<providerDeps.length;i++){var options=6,token=resolveForwardRef(providerDeps[i]);if(token instanceof Array)for(var j=0,annotations=token;j<annotations.length;j++){var annotation=annotations[j];annotation instanceof Optional||annotation==Optional?options|=1:annotation instanceof SkipSelf||annotation==SkipSelf?options&=-3:annotation instanceof Self||annotation==Self?options&=-5:token=annotation instanceof Inject?annotation.token:resolveForwardRef(annotation)}deps.push({token:token,options:options})}}else if(provider.useExisting){var token=resolveForwardRef(provider.useExisting);deps=[{token:token,options:6}]}else if(!(providerDeps||USE_VALUE in provider))throw staticError("'deps' required",provider);return deps}function formatError(text,obj,source){void 0===source&&(source=null),text=text&&"\n"===text.charAt(0)&&text.charAt(1)==NO_NEW_LINE?text.substr(2):text;var context=stringify(obj);if(obj instanceof Array)context=obj.map(stringify).join(" -> ");else if("object"==typeof obj){var parts=[];for(var key in obj)if(obj.hasOwnProperty(key)){var value=obj[key];parts.push(key+":"+("string"==typeof value?JSON.stringify(value):stringify(value)))}context="{"+parts.join(", ")+"}"}return"StaticInjectorError"+(source?"("+source+")":"")+"["+context+"]: "+text.replace(NEW_LINE,"\n  ")}function staticError(text,obj){return new Error(formatError(text,obj))}function getDebugContext(error){return error[ERROR_DEBUG_CONTEXT]}function getOriginalError(error){return error[ERROR_ORIGINAL_ERROR]}function getErrorLogger(error){return error[ERROR_LOGGER]||defaultErrorLogger}function defaultErrorLogger(console){for(var values=[],_i=1;_i<arguments.length;_i++)values[_i-1]=arguments[_i];console.error.apply(console,values)}function wrappedError(message,originalError){var msg=message+" caused by: "+(originalError instanceof Error?originalError.message:originalError),error=Error(msg);return error[ERROR_ORIGINAL_ERROR]=originalError,error}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function stringifyValueForError(value){return"string"==typeof value?'"'+value+'"':""+value}function assertEqual(actual,expected,name,serializer){actual!=expected&&assertThrow(actual,expected,name,"==",serializer)}function assertLessThan(actual,expected,name){actual<expected&&assertThrow(actual,expected,name,">")}function assertNotNull(actual,name){assertNotEqual(actual,null,name)}function assertNotEqual(actual,expected,name){actual==expected&&assertThrow(actual,expected,name,"!=")}function assertThrow(actual,expected,name,operator,serializer){throw void 0===serializer&&(serializer=stringifyValueForError),new Error("ASSERT: expected "+name+" "+operator+" "+serializer(expected)+" but was "+serializer(actual)+"!")}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function assertNodeType(node,type){assertNotEqual(node,null,"node"),assertEqual(3&node.flags,type,"Node.type",typeSerializer)}function typeSerializer(type){return 1==type?"Projection":0==type?"Container":2==type?"View":3==type?"Element":"??? "+type+" ???"}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function findNativeParent(containerNode){for(var container=containerNode;container;){ngDevMode&&assertNodeType(container,0);var renderParent=container.data.renderParent;if(null!==renderParent)return renderParent.native;var viewOrElement=container.parent;if(ngDevMode&&assertNotNull(viewOrElement,"container.parent"),3==(3&viewOrElement.flags))return null;ngDevMode&&assertNodeType(viewOrElement,2),container=viewOrElement.parent}return null}function findBeforeNode(index,state,native){var views=state.views;return index+1<views.length?views[index+1].child.native:native}function addRemoveViewFromContainer(container,rootNode,insertMode,beforeNode){ngDevMode&&assertNodeType(container,0),ngDevMode&&assertNodeType(rootNode,2);var parent=findNativeParent(container),node=rootNode.child;if(parent)for(;node;){var type=3&node.flags,nextNode=null,renderer=container.view.renderer,isFnRenderer=renderer.listen;if(3===type)insertMode?isFnRenderer?renderer.insertBefore(parent,node.native,beforeNode):parent.insertBefore(node.native,beforeNode,!0):isFnRenderer?renderer.removeChild(parent,node.native):parent.removeChild(node.native),nextNode=node.next;else if(0===type){var childContainerData=node.data;insertMode?isFnRenderer?renderer.appendChild(parent,node.native):parent.appendChild(node.native):isFnRenderer?renderer.removeChild(parent,node.native):parent.removeChild(node.native),nextNode=childContainerData.views.length?childContainerData.views[0].child:null}else nextNode=1===type?node.data[0]:node.child;if(null===nextNode){for(;node&&!node.next;)(node=node.parent)===rootNode&&(node=null);node=node&&node.next}else node=nextNode}}function destroyViewTree(rootView){for(var viewOrContainerState=rootView;viewOrContainerState;){var next=null;if(viewOrContainerState.views&&viewOrContainerState.views.length?next=viewOrContainerState.views[0].data:viewOrContainerState.child?next=viewOrContainerState.child:viewOrContainerState.next&&(cleanUpView(viewOrContainerState),next=viewOrContainerState.next),null==next){for(;viewOrContainerState&&!viewOrContainerState.next;)cleanUpView(viewOrContainerState),viewOrContainerState=getParentState(viewOrContainerState,rootView);cleanUpView(viewOrContainerState||rootView),next=viewOrContainerState&&viewOrContainerState.next}viewOrContainerState=next}}function insertView(container,newView,index){var state=container.data,views=state.views;return index>0&&setViewNext(views[index-1],newView),index<views.length&&views[index].data.id!==newView.data.id?(setViewNext(newView,views[index]),views.splice(index,0,newView)):index>=views.length&&views.push(newView),state.nextIndex<=index&&state.nextIndex++,null!==container.data.renderParent&&addRemoveViewFromContainer(container,newView,!0,findBeforeNode(index,state,container.native)),container.query&&container.query.insertView(container,newView,index),newView}function removeView(container,removeIndex){var views=container.data.views,viewNode=views[removeIndex];return removeIndex>0&&setViewNext(views[removeIndex-1],viewNode.next),views.splice(removeIndex,1),destroyViewTree(viewNode.data),addRemoveViewFromContainer(container,viewNode,!1),container.query&&container.query.removeView(container,viewNode,removeIndex),viewNode}function setViewNext(view,next){view.next=next,view.data.next=next?next.data:null}function getParentState(state,rootView){var node;return(node=state.node)&&2==(3&node.flags)?node.parent.data:state.parent===rootView?null:state.parent}function cleanUpView(viewState){if(viewState.cleanup){for(var cleanup=viewState.cleanup,i=0;i<cleanup.length-1;i+=2)"string"==typeof cleanup[i]?(cleanup[i+1].removeEventListener(cleanup[i],cleanup[i+2],cleanup[i+3]),i+=2):cleanup[i].call(cleanup[i+1]);viewState.cleanup=null}}function appendChild(parent,child,currentView){if(null!==child&&3==(3&parent.flags)&&(parent.view!==currentView||null===parent.data)){var renderer=currentView.renderer;return renderer.listen?renderer.appendChild(parent.native,child):parent.native.appendChild(child),!0}return!1}function insertChild(node,currentView){var parent=node.parent;if(3==(3&parent.flags)&&(parent.view!==currentView||null===parent.data)){for(var sibling=node.next,nativeSibling=null;sibling&&null===(nativeSibling=sibling.native);)sibling=sibling.next;var renderer=currentView.renderer;renderer.listen?renderer.insertBefore(parent.native,node.native,nativeSibling):parent.native.insertBefore(node.native,nativeSibling,!1)}}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function isDifferent(a,b){return!(a!==a&&b!==b)&&a!==b}function stringify$1(value){return"function"==typeof value?value.name||value:"string"==typeof value?value:null==value?"":""+value}function enterView(newViewState,host){var oldViewState=currentView;return data=newViewState.data,bindingIndex=newViewState.bindingStartIndex||0,ngStaticData=newViewState.ngStaticData,creationMode=newViewState.creationMode,viewHookStartIndex=newViewState.viewHookStartIndex,cleanup=newViewState.cleanup,renderer=newViewState.renderer,null!=host&&(previousOrParentNode=host,isParent=!0),currentView=newViewState,oldViewState}function leaveView(newViewState){executeViewHooks(),enterView(newViewState,null)}function createViewState(viewId,renderer,ngStaticData){return{parent:currentView,id:viewId,node:null,data:[],ngStaticData:ngStaticData,cleanup:null,renderer:renderer,child:null,tail:null,next:null,bindingStartIndex:null,creationMode:!0,viewHookStartIndex:null}}function createLNode(index,type,native,state){var parent=isParent?previousOrParentNode:previousOrParentNode&&previousOrParentNode.parent,query=(isParent?currentQuery:previousOrParentNode&&previousOrParentNode.query)||parent&&parent.query&&parent.query.child(),isState=null!=state,node={flags:type,native:native,view:currentView,parent:parent,child:null,next:null,nodeInjector:parent?parent.nodeInjector:null,data:isState?state:null,query:query,staticData:null};return 2==(2&type)&&isState&&(ngDevMode&&assertEqual(state.node,null,"viewState.node"),state.node=node),null!=index&&(ngDevMode&&assertEqual(data.length,index,"data.length not in sequence"),data[index]=node,index>=ngStaticData.length?ngStaticData[index]=null:node.staticData=ngStaticData[index],isParent?(currentQuery=null,previousOrParentNode.view!==currentView&&2!=(3&previousOrParentNode.flags)||(ngDevMode&&assertEqual(previousOrParentNode.child,null,"previousNode.child"),previousOrParentNode.child=node)):previousOrParentNode&&(ngDevMode&&assertEqual(previousOrParentNode.next,null,"previousNode.next"),previousOrParentNode.next=node)),previousOrParentNode=node,isParent=!0,node}function resetApplicationState(){isParent=!1,previousOrParentNode=null}function renderComponentOrTemplate(node,viewState,componentOrContext,template){var oldView=enterView(viewState,node);try{rendererFactory.begin&&rendererFactory.begin(),template?(ngStaticData=template.ngStaticData||(template.ngStaticData=[]),template(componentOrContext,creationMode)):componentOrContext.constructor.ngComponentDef.r(1,0)}finally{rendererFactory.end&&rendererFactory.end(),viewState.creationMode=!1,leaveView(oldView)}}function elementStart(index,nameOrComponentDef,attrs,localName){var node,native;if(null==nameOrComponentDef){var node_1=data[index];native=node_1&&node_1.native}else{ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex");var isHostElement="string"!=typeof nameOrComponentDef,name_1=isHostElement?nameOrComponentDef.tag:nameOrComponentDef;if(null===name_1)throw"for now name is required";native=renderer.createElement(name_1);var componentView=null;if(isHostElement){var ngStaticData_1=getTemplateStatic(nameOrComponentDef.template);componentView=addToViewTree(createViewState(-1,rendererFactory.createRenderer(native,nameOrComponentDef.rendererType),ngStaticData_1))}node=createLNode(index,3,native,componentView),null==node.staticData&&(ngDevMode&&assertDataInRange(index-1),node.staticData=ngStaticData[index]=createNodeStatic(name_1,attrs||null,null,localName||null)),attrs&&setUpAttributes(native,attrs),appendChild(node.parent,native,currentView)}return native}function getTemplateStatic(template){return template.ngStaticData||(template.ngStaticData=[])}function setUpAttributes(native,attrs){ngDevMode&&assertEqual(attrs.length%2,0,"attrs.length % 2");for(var isProceduralRenderer=renderer.setAttribute,i=0;i<attrs.length;i+=2)isProceduralRenderer?renderer.setAttribute(native,attrs[i],attrs[1|i]):native.setAttribute(attrs[i],attrs[1|i])}function createError(text,token){return new Error("Renderer: "+text+" ["+stringify$1(token)+"]")}function locateHostElement(factory,elementOrSelector){ngDevMode&&assertDataInRange(-1),rendererFactory=factory;var defaultRenderer=factory.createRenderer(null,null),rNode="string"==typeof elementOrSelector?defaultRenderer.selectRootElement?defaultRenderer.selectRootElement(elementOrSelector):defaultRenderer.querySelector(elementOrSelector):elementOrSelector;if(ngDevMode&&!rNode)throw"string"==typeof elementOrSelector?createError("Host node with selector not found:",elementOrSelector):createError("Host node is required:",elementOrSelector);return rNode}function hostElement(rNode,def){resetApplicationState(),createLNode(0,3,rNode,createViewState(-1,renderer,getTemplateStatic(def.template)))}function elementEnd(){isParent?isParent=!1:(ngDevMode&&assertHasParent(),previousOrParentNode=previousOrParentNode.parent),ngDevMode&&assertNodeType(previousOrParentNode,3);var query=previousOrParentNode.query;query&&query.addNode(previousOrParentNode)}function elementProperty(index,propName,value){if(value!==NO_CHANGE){var node=data[index],staticData=node.staticData;void 0===staticData.inputs&&(staticData.inputs=null,staticData=generatePropertyAliases(node.flags,staticData,!0));var dataValue,inputData=staticData.inputs;if(inputData&&(dataValue=inputData[propName]))setInputsForProperty(dataValue,value);else{var native=node.native;renderer.setProperty?renderer.setProperty(native,propName,value):native.setProperty?native.setProperty(propName,value):native[propName]=value}}}function createNodeStatic(tagName,attrs,containerStatic,localName){return{tagName:tagName,attrs:attrs,localNames:localName?[localName,-1]:null,initialInputs:void 0,inputs:void 0,outputs:void 0,containerStatic:containerStatic}}function setInputsForProperty(inputs,value){for(var i=0;i<inputs.length;i+=2)ngDevMode&&assertDataInRange(inputs[i]),data[inputs[i]][inputs[1|i]]=value}function generatePropertyAliases(flags,data,isInputData){void 0===isInputData&&(isInputData=!1);for(var start=flags>>12,size=(4092&flags)>>2,i=start,ii=start+size;i<ii;i++){var directiveDef=ngStaticData[i],propertyAliasMap=isInputData?directiveDef.inputs:directiveDef.outputs;for(var publicName in propertyAliasMap)if(propertyAliasMap.hasOwnProperty(publicName)){var internalName=propertyAliasMap[publicName],staticDirData=isInputData?data.inputs||(data.inputs={}):data.outputs||(data.outputs={}),hasProperty=staticDirData.hasOwnProperty(publicName);hasProperty?staticDirData[publicName].push(i,internalName):staticDirData[publicName]=[i,internalName]}}return data}function elementStyle(index,styleName,value,suffix){if(value!==NO_CHANGE){var lElement=data[index];null==value?renderer.removeStyle?renderer.removeStyle(lElement.native,styleName,RendererStyleFlags3.DashCase):lElement.native.style.removeProperty(styleName):renderer.setStyle?renderer.setStyle(lElement.native,styleName,suffix?stringify$1(value)+suffix:stringify$1(value),RendererStyleFlags3.DashCase):lElement.native.style.setProperty(styleName,suffix?stringify$1(value)+suffix:stringify$1(value))}}function text(index,value){ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex");var textNode=null!=value?renderer.createText?renderer.createText(stringify$1(value)):renderer.createTextNode(stringify$1(value)):null,node=createLNode(index,3,textNode);isParent=!1,appendChild(node.parent,textNode,currentView)}function textBinding(index,value){var existingNode=index<data.length&&data[index];existingNode&&existingNode.native?value!==NO_CHANGE&&(renderer.setValue?renderer.setValue(existingNode.native,stringify$1(value)):existingNode.native.textContent=stringify$1(value)):existingNode?(existingNode.native=renderer.createText?renderer.createText(stringify$1(value)):renderer.createTextNode(stringify$1(value)),insertChild(existingNode,currentView)):text(index,value)}function directive(index,directive,directiveDef,localName){var instance;if(null==directive)ngDevMode&&assertDataInRange(index),instance=data[index];else{ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex"),ngDevMode&&assertPreviousIsParent();var flags=previousOrParentNode.flags;if(0===(4092&flags)?flags=index<<12|4|3&flags:flags+=4,previousOrParentNode.flags=flags,ngDevMode&&assertDataInRange(index-1),Object.defineProperty(directive,NG_HOST_SYMBOL,{enumerable:!1,value:previousOrParentNode}),data[index]=instance=directive,index>=ngStaticData.length&&(ngStaticData[index]=directiveDef,localName)){ngDevMode&&assertNotNull(previousOrParentNode.staticData,"previousOrParentNode.staticData");var nodeStaticData=previousOrParentNode.staticData;(nodeStaticData.localNames||(nodeStaticData.localNames=[])).push(localName,index)}var diPublic_1=directiveDef.diPublic;diPublic_1&&diPublic_1(directiveDef);var staticData=previousOrParentNode.staticData;staticData&&staticData.attrs&&setInputsFromAttrs(instance,directiveDef.inputs,staticData)}return instance}function setInputsFromAttrs(instance,inputs,staticData){var directiveIndex=((4092&previousOrParentNode.flags)>>2)-1,initialInputData=staticData.initialInputs;(void 0===initialInputData||directiveIndex>=initialInputData.length)&&(initialInputData=generateInitialInputs(directiveIndex,inputs,staticData));var initialInputs=initialInputData[directiveIndex];if(initialInputs)for(var i=0;i<initialInputs.length;i+=2)instance[initialInputs[i]]=initialInputs[1|i]}function generateInitialInputs(directiveIndex,inputs,staticData){var initialInputData=staticData.initialInputs||(staticData.initialInputs=[]);initialInputData[directiveIndex]=null;for(var attrs=staticData.attrs,i=0;i<attrs.length;i+=2){var attrName=attrs[i],minifiedInputName=inputs[attrName];if(void 0!==minifiedInputName){(initialInputData[directiveIndex]||(initialInputData[directiveIndex]=[])).push(minifiedInputName,attrs[1|i])}}return initialInputData}function executeViewHooks(){if(null!=viewHookStartIndex){for(var checkIndex=viewHookStartIndex,writeIndex=checkIndex;checkIndex<data.length;)data[checkIndex+1].call(data[checkIndex+2]),16===data[checkIndex]&&(writeIndex<checkIndex&&(data[writeIndex]=data[checkIndex],data[writeIndex+1]=data[checkIndex+1],data[writeIndex+2]=data[checkIndex+2]),writeIndex+=3),checkIndex+=3;data.length=writeIndex}}function containerStart(index,template,tagName,attrs,localName){ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex");var comment=renderer.createComment(ngDevMode?"container":""),renderParent=null,currentParent=isParent?previousOrParentNode:previousOrParentNode.parent;ngDevMode&&assertNotEqual(currentParent,null,"currentParent"),appendChild(currentParent,comment,currentView)&&(renderParent=currentParent);var node=createLNode(index,0,comment,{views:[],nextIndex:0,renderParent:renderParent,template:null==template?null:template,next:null,parent:currentView});null==node.staticData&&(node.staticData=ngStaticData[index]=createNodeStatic(tagName||null,attrs||null,[],localName||null)),addToViewTree(node.data)}function containerEnd(){isParent?isParent=!1:(ngDevMode&&assertHasParent(),previousOrParentNode=previousOrParentNode.parent),ngDevMode&&assertNodeType(previousOrParentNode,0);var query=previousOrParentNode.query;query&&query.addNode(previousOrParentNode)}function containerRefreshStart(index){ngDevMode&&assertDataInRange(index),previousOrParentNode=data[index],ngDevMode&&assertNodeType(previousOrParentNode,0),isParent=!0,previousOrParentNode.data.nextIndex=0}function containerRefreshEnd(){isParent?isParent=!1:(ngDevMode&&assertNodeType(previousOrParentNode,2),ngDevMode&&assertHasParent(),previousOrParentNode=previousOrParentNode.parent),ngDevMode&&assertNodeType(previousOrParentNode,0);var container=previousOrParentNode;ngDevMode&&assertNodeType(container,0);for(var nextIndex=container.data.nextIndex;nextIndex<container.data.views.length;)removeView(container,nextIndex)}function viewStart(viewBlockId){var container=isParent?previousOrParentNode:previousOrParentNode.parent;ngDevMode&&assertNodeType(container,0);var containerState=container.data,views=containerState.views,existingView=!creationMode&&containerState.nextIndex<views.length&&views[containerState.nextIndex],viewUpdateMode=existingView&&viewBlockId===existingView.data.id;if(viewUpdateMode)previousOrParentNode=views[containerState.nextIndex++],ngDevMode&&assertNodeType(previousOrParentNode,2),isParent=!0,enterView(existingView.data,previousOrParentNode);else{var newViewState=createViewState(viewBlockId,renderer,initViewStaticData(viewBlockId,container));enterView(newViewState,createLNode(null,2,null,newViewState)),containerState.nextIndex++}return!viewUpdateMode}function initViewStaticData(viewIndex,parent){ngDevMode&&assertNodeType(parent,0);var containerStatic=parent.staticData.containerStatic;return(viewIndex>=containerStatic.length||null==containerStatic[viewIndex])&&(containerStatic[viewIndex]=[]),containerStatic[viewIndex]}function viewEnd(){isParent=!1;var viewNode=previousOrParentNode=currentView.node,container=previousOrParentNode.parent;ngDevMode&&assertNodeType(viewNode,2),ngDevMode&&assertNodeType(container,0);var containerState=container.data,previousView=containerState.nextIndex<=containerState.views.length?containerState.views[containerState.nextIndex-1]:null;(null==previousView||previousView.data.id!==viewNode.data.id)&&(insertView(container,viewNode,containerState.nextIndex-1),currentView.creationMode=!1),leaveView(currentView.parent),ngDevMode&&assertEqual(isParent,!1,"isParent"),ngDevMode&&assertNodeType(previousOrParentNode,2)}function addToViewTree(state){return currentView.tail?currentView.tail.next=state:currentView.child=state,currentView.tail=state,state}function bind(value){var different;return(different=creationMode)?("number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=value):((different=value!==NO_CHANGE&&isDifferent(data[bindingIndex],value))&&(data[bindingIndex]=value),bindingIndex++),different?value:NO_CHANGE}function bind1(prefix,value,suffix){return bind(value)===NO_CHANGE?NO_CHANGE:prefix+stringify$1(value)+suffix}function assertPreviousIsParent(){assertEqual(isParent,!0,"isParent")}function assertHasParent(){assertNotEqual(previousOrParentNode.parent,null,"isParent")}function assertDataInRange(index,arr){null==arr&&(arr=data),assertLessThan(arr?arr.length:0,index,"data.length")}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function renderComponent(componentType,opts){void 0===opts&&(opts={});var component,rendererFactory=opts.rendererFactory||domRendererFactory3,componentDef=componentType.ngComponentDef,hostNode=locateHostElement(rendererFactory,opts.host||componentDef.tag),oldView=enterView(createViewState(-1,rendererFactory.createRenderer(hostNode,componentDef.rendererType),[]),null);try{hostElement(hostNode,componentDef),component=directive(1,componentDef.n(),componentDef)}finally{leaveView(oldView)}return opts.features&&opts.features.forEach(function(feature){return feature(component,componentDef)}),detectChanges(component),component}function detectChanges(component){ngDevMode&&assertNotNull(component,"component");var hostNode=component[NG_HOST_SYMBOL];ngDevMode&&!hostNode&&createError("Not a directive instance",component),ngDevMode&&assertNotNull(hostNode.data,"hostNode.data"),renderComponentOrTemplate(hostNode,hostNode.view,component),isDirty=!1}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function defineComponent(componentDefinition){var def={type:componentDefinition.type,diPublic:null,n:componentDefinition.factory,tag:componentDefinition.tag||null,template:componentDefinition.template||null,r:componentDefinition.refresh||function(d,e){componentRefresh(d,e,componentDefinition.template)},h:componentDefinition.hostBindings||noop$1,inputs:invertObject(componentDefinition.inputs),outputs:invertObject(componentDefinition.outputs),methods:invertObject(componentDefinition.methods),rendererType:resolveRendererType2(componentDefinition.rendererType)||null},feature=componentDefinition.features;return feature&&feature.forEach(function(fn){return fn(def)}),def}function noop$1(){}function invertObject(obj){if(null==obj)return EMPTY$1;var newObj={};for(var minifiedKey in obj)newObj[obj[minifiedKey]]=minifiedKey;return newObj}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var Directive=makeDecorator("Directive",function(dir){return void 0===dir&&(dir={}),dir}),Component=makeDecorator("Component",function(c){return void 0===c&&(c={}),__assign({changeDetection:ChangeDetectionStrategy.Default},c)},Directive),Pipe=makeDecorator("Pipe",function(p){return __assign({pure:!0},p)}),Input=makePropDecorator("Input",function(bindingPropertyName){return{bindingPropertyName:bindingPropertyName}}),Output=makePropDecorator("Output",function(bindingPropertyName){return{bindingPropertyName:bindingPropertyName}}),HostBinding=makePropDecorator("HostBinding",function(hostPropertyName){return{hostPropertyName:hostPropertyName}}),HostListener=makePropDecorator("HostListener",function(eventName,args){return{eventName:eventName,args:args}}),CUSTOM_ELEMENTS_SCHEMA={name:"custom-elements"},NO_ERRORS_SCHEMA={name:"no-errors-schema"},NgModule=makeDecorator("NgModule",function(ngModule){return ngModule}),ViewEncapsulation={Emulated:0,Native:1,None:2};ViewEncapsulation[ViewEncapsulation.Emulated]="Emulated",ViewEncapsulation[ViewEncapsulation.Native]="Native",ViewEncapsulation[ViewEncapsulation.None]="None";/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
