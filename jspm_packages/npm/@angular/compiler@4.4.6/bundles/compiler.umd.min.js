...
(function(process) {
  !function(global, factory) {
    "object" == typeof exports && "undefined" != typeof module ? factory(exports, require('@angular/core')) : "function" == typeof define && define.amd ? define(["exports", "@angular/core"], factory) : factory((global.ng = global.ng || {}, global.ng.compiler = global.ng.compiler || {}), global.ng.core);
    ...
    function identifierModuleUrl(compileIdentifier) {
      var ref = compileIdentifier.reference;
      return ref instanceof StaticSymbol ? ref.filePath : "./" + _angular_core.ɵstringify(ref);
    }
    ...
    function templateSourceUrl(ngModuleType, compMeta, templateMeta) {
      var url;
      return url = templateMeta.isInline ? compMeta.type.reference instanceof StaticSymbol ? compMeta.type.reference.filePath + "." + compMeta.type.reference.name + ".html" : identifierName(ngModuleType) + "/" + identifierName(compMeta.type) + ".html" : templateMeta.templateUrl, sourceUrl(url);
    }
    function sharedStylesheetJitUrl(meta, id) {
      var pathParts = meta.moduleUrl.split(/\/\\/g);
      return sourceUrl("css/" + id + pathParts[pathParts.length - 1] + ".ngstyle.js");
    }
    ...
    function componentStillLoadingError(compType) {
      ...
      return error[_angular_core.ɵERROR_COMPONENT_TYPE] = compType, error;
    }
    ...
    function convertPropertyBinding(localResolver, implicitReceiver, expressionWithoutBuiltins, bindingId) {
      ...
      return stmts.push(currValExpr.set(outputExpr).toDeclStmt(null, [StmtModifier.Final])), new ConvertPropertyBindingResult(stmts, currValExpr);
    }
    ...
    function serializeSummaries(forJitCtx, summaryResolver, symbolResolver, symbols, types) {
      var toJsonSerializer = new ToJsonSerializer(symbolResolver, summaryResolver),
          forJitSerializer = new ForJitSerializer(forJitCtx, symbolResolver);
      ...
      for (var processedIndex = 0; processedIndex < toJsonSerializer.symbols.length; processedIndex++) {
        ...
        if (summaryResolver.isLibraryFile(symbol.filePath)) {
          ...
          summary && (summary.type && forJitSerializer.addLibType(summary.type), toJsonSerializer.addOrMergeSummary(summary));
        }
      }
      types.forEach(function(_a) {
        var summary = _a.summary,
            metadata = _a.metadata;
        if (forJitSerializer.addSourceType(summary, metadata), toJsonSerializer.addOrMergeSummary({
          symbol: summary.type.reference,
          metadata: null,
          type: summary
        }), summary.summaryKind === CompileSummaryKind.NgModule) {
          var ngModuleSummary = summary;
          ngModuleSummary.exportedDirectives.concat(ngModuleSummary.exportedPipes).forEach(function(id) {
            var symbol = id.reference;
            if (summaryResolver.isLibraryFile(symbol.filePath)) {
              var summary_1 = summaryResolver.resolveSummary(symbol);
              summary_1 && toJsonSerializer.addOrMergeSummary(summary_1);
            }
          });
        }
      });
      ...
      return forJitSerializer.serialize(exportAs), {
        json: json,
        exportAs: exportAs
      };
    }
    function deserializeSummaries(symbolCache, json) {
      return new FromJsonDeserializer(symbolCache).deserialize(json);
    }
    ...
    function analyzeNgModules(programStaticSymbols, host, metadataResolver) {
      ...
      return _analyzeNgModules(programStaticSymbols, _a.ngModules, _a.symbolsMissingModule, metadataResolver);
    }
    function analyzeAndValidateNgModules(programStaticSymbols, host, metadataResolver) {
      ...
      if (result.symbolsMissingModule && result.symbolsMissingModule.length) {
        throw syntaxError(result.symbolsMissingModule.map(function(s) {
    ...
    function _analyzeNgModules(programSymbols, ngModuleMetas, symbolsMissingModule, metadataResolver) {
      ...
      programSymbols.forEach(function(symbol) {
        ...
        filePaths.add(filePath), metadataResolver.isInjectable(symbol) && ngInjectablesByFile.set(filePath, (ngInjectablesByFile.get(filePath) || []).concat(symbol));
      }), ngModuleMetas.forEach(function(ngModuleMeta) {
        var srcFileUrl = ngModuleMeta.type.reference.filePath;
        filePaths.add(srcFileUrl), ngModulesByFile.set(srcFileUrl, (ngModulesByFile.get(srcFileUrl) || []).concat(ngModuleMeta.type.reference)), ngModuleMeta.declaredDirectives.forEach(function(dirIdentifier) {
          var fileUrl = dirIdentifier.reference.filePath;
          filePaths.add(fileUrl), ngDirectivesByFile.set(fileUrl, (ngDirectivesByFile.get(fileUrl) || []).concat(dirIdentifier.reference)), ngModuleByPipeOrDirective.set(dirIdentifier.reference, ngModuleMeta);
        }), ngModuleMeta.declaredPipes.forEach(function(pipeIdentifier) {
          var fileUrl = pipeIdentifier.reference.filePath;
          filePaths.add(fileUrl), ngPipesByFile.set(fileUrl, (ngPipesByFile.get(fileUrl) || []).concat(pipeIdentifier.reference)), ngModuleByPipeOrDirective.set(pipeIdentifier.reference, ngModuleMeta);
        });
      });
      var files = [];
      return filePaths.forEach(function(srcUrl) {
        var directives = ngDirectivesByFile.get(srcUrl) || [],
            pipes = ngPipesByFile.get(srcUrl) || [],
            ngModules = ngModulesByFile.get(srcUrl) || [],
            injectables = ngInjectablesByFile.get(srcUrl) || [];
        files.push({
          srcUrl: srcUrl,
    ...
    function extractProgramSymbols(staticSymbolResolver, files, host) {
      ...
      return files.filter(function(fileName) {
        ...
        staticSymbolResolver.getSymbolsOf(sourceFile).forEach(function(symbol) {
          ...
          symbolMeta && "error" != symbolMeta.__symbolic && staticSymbols.push(resolvedSymbol.symbol);
        });
      }), staticSymbols;
    }
      ...
      var ngModules = new Map,
          ...
          addNgModule = function(staticSymbol) {
            if (ngModules.has(staticSymbol) || !host.isSourceFile(staticSymbol.filePath))
              return !1;
            var ngModule = metadataResolver.getNgModuleMetadata(staticSymbol, !1);
            return ngModule && (ngModules.set(ngModule.type.reference, ngModule), ngModule.declaredDirectives.forEach(function(dir) {
    ...
    function expandedMessage(error) {
      switch (error.message) {
        ...
        case "Function call not supported":
          return (error.context && error.context.name ? "Calling function '" + error.context.name + "', f" : "F") + "unction calls are not supported. Consider replacing the function or lambda with a reference to an exported function";
    ...
    function positionalError(message, fileName, line, column) {
      ...
      return result.fileName = fileName, result.line = line, result.column = column, result;
    }
    ...
    function createAotCompiler(compilerHost, options) {
      var translations = options.translations || "",
          ...
          staticReflector = new StaticReflector(summaryResolver, symbolResolver),
          console = new _angular_core.ɵConsole,
          ...
          config = new CompilerConfig({
            ...
            useJit: !1,
            enableLegacyTemplate: !1 !== options.enableLegacyTemplate,
      ...
      return {
        compiler: new AotCompiler(config, compilerHost, staticReflector, resolver, tmplParser, new StyleCompiler(urlResolver), viewCompiler, new NgModuleCompiler(staticReflector), new TypeScriptEmitter, summaryResolver, options.locale || null, options.i18nFormat || null, options.enableSummariesForJit || null, symbolResolver),
        reflector: staticReflector
      };
    }
    function interpretStatements(statements) {
      var ctx = new _ExecutionContext(null, null, null, new Map);
      (new StatementInterpreter).visitAllStatements(statements, ctx);
      var result = {};
    ...
    function jitStatements(sourceUrl$$1, statements) {
      var converter = new JitEmitterVisitor,
          ctx = EmitterVisitorContext.createRoot();
      return converter.visitAllStatements(statements, ctx), converter.createReturnStmt(ctx), evalExpression(sourceUrl$$1, ctx, converter.getArgs());
    }
    ...
    function flattenSummaries(fn$$1, out) {
      return void 0 === out && (out = []), fn$$1().forEach(function(entry) {
        "function" == typeof entry ? flattenSummaries(entry, out) : out.push(entry);
      }), out;
    }
    ...
    var extendStatics = Object.setPrototypeOf || {__proto__: []} instanceof Array && function(d, b) {
      ...
      for (var p in b)
        ...
        VERSION = new _angular_core.Version("4.4.6"),
        TextAst = function() {
    ...
    var NullTemplateVisitor = function() {
      ...
      return NullTemplateVisitor.prototype.visitNgContent = function(ast, context) {}, NullTemplateVisitor.prototype.visitEmbeddedTemplate = function(ast, context) {}, NullTemplateVisitor.prototype.visitElement = function(ast, context) {}, NullTemplateVisitor.prototype.visitReference = function(ast, context) {}, NullTemplateVisitor.prototype.visitVariable = function(ast, context) {}, NullTemplateVisitor.prototype.visitEvent = function(ast, context) {}, NullTemplateVisitor.prototype.visitElementProperty = function(ast, context) {}, NullTemplateVisitor.prototype.visitAttr = function(ast, context) {}, NullTemplateVisitor.prototype.visitBoundText = function(ast, context) {}, NullTemplateVisitor.prototype.visitText = function(ast, context) {}, NullTemplateVisitor.prototype.visitDirective = function(ast, context) {}, NullTemplateVisitor.prototype.visitDirectiveProperty = function(ast, context) {}, NullTemplateVisitor;
        ...
        TagContentType = {};
    TagContentType.RAW_TEXT = 0, TagContentType.ESCAPABLE_RAW_TEXT = 1, TagContentType.PARSABLE_DATA = 2, TagContentType[TagContentType.RAW_TEXT] = "RAW_TEXT", TagContentType[TagContentType.ESCAPABLE_RAW_TEXT] = "ESCAPABLE_RAW_TEXT", TagContentType[TagContentType.PARSABLE_DATA] = "PARSABLE_DATA";
    var NAMED_ENTITIES = {
      Aacute: "Á",
      aacute: "á",
      Acirc: "Â",
          ...
          return SelectorMatcher.createNotMatcher = function(notSelectors) {
              ...
              for (var i = 0; i < attrs.length; i += 2) {
                ...
                if (isTerminal) {
                  var terminalMap = matcher._attrValueMap,
                      terminalValuesMap = terminalMap.get(name);
                  terminalValuesMap || (terminalValuesMap = new Map, terminalMap.set(name, terminalValuesMap)), this._addTerminal(terminalValuesMap, value, selectable);
                } else {
                  var partialMap = matcher._attrValuePartialMap,
                      partialValuesMap = partialMap.get(name);
                  partialValuesMap || (partialValuesMap = new Map, partialMap.set(name, partialValuesMap)), matcher = this._addPartial(partialValuesMap, value);
                }
              }
            ...
            if (attrs)
              for (var i = 0; i < attrs.length; i += 2) {
                var name = attrs[i],
                    value = attrs[i + 1],
                    terminalValuesMap = this._attrValueMap.get(name);
    ...
    var CompileStylesheetMetadata = function() {
      ...
      return CompileStylesheetMetadata;
        ...
        CompileReflector = function() {
          ...
          return CompileReflector.prototype.parameters = function(typeOrFunc) {}, CompileReflector.prototype.annotations = function(typeOrFunc) {}, CompileReflector.prototype.propMetadata = function(typeOrFunc) {}, CompileReflector.prototype.hasLifecycleHook = function(type, lcProperty) {}, CompileReflector.prototype.componentModuleUrl = function(type, cmpMetadata) {}, CompileReflector.prototype.resolveExternalReference = function(ref) {}, CompileReflector;
        }(),
        CompilerConfig = function() {
          function CompilerConfig(_a) {
            ...
            this.defaultEncapsulation = defaultEncapsulation, this.useJit = !!useJit, this.missingTranslation = missingTranslation || null, this.enableLegacyTemplate = !1 !== enableLegacyTemplate, this.preserveWhitespaces = preserveWhitespacesDefault(noUndefined(preserveWhitespaces));
          }
        ...
        TokenType = {};
    TokenType.Character = 0, TokenType.Identifier = 1, TokenType.Keyword = 2, TokenType.String = 3, TokenType.Operator = 4, TokenType.Number = 5, TokenType.Error = 6, TokenType[TokenType.Character] = "Character", TokenType[TokenType.Identifier] = "Identifier", TokenType[TokenType.Keyword] = "Keyword", TokenType[TokenType.String] = "String", TokenType[TokenType.Operator] = "Operator", TokenType[TokenType.Number] = "Number", TokenType[TokenType.Error] = "Error";
      ...
      return _ParseAST.prototype.peek = function(offset) {
        ...
        for (var bindings = [],
          ...
          if (keyIsVar)
            ...
            this.advance(), name = rawKey, key = this.expectTemplateBindingKey(), keyIsVar = !0;
          } else if (this.next !== EOF && !this.peekKeywordLet()) {
            var start_2 = this.inputIndex,
                ast = this.parsePipe(),
                source = this.input.substring(start_2 - this.offset, this.inputIndex - this.offset);
        ...
        ParseErrorLevel = {};
    ParseErrorLevel.WARNING = 0, ParseErrorLevel.ERROR = 1, ParseErrorLevel[ParseErrorLevel.WARNING] = "WARNING", ParseErrorLevel[ParseErrorLevel.ERROR] = "ERROR";
    ...
    var Token$1 = function() {
      ...
      return Token$1;
        ...
        _TreeBuilder = function() {
          ...
          return _TreeBuilder.prototype.build = function() {
            ...
            if (text.length > 0 && "\n" == text[0]) {
              ...
              null != parent && 0 == parent.children.length && this.getTagDefinition(parent.name).ignoreFirstLf && (text = text.substring(1));
            }
        ...
        Endian = {};
    Endian.Little = 0, Endian.Big = 1, Endian[Endian.Little] = "Little", Endian[Endian.Big] = "Big";
    var Message = function() {
      ...
      return Message;
        ...
        _VisitorMode = {};
    _VisitorMode.Extract = 0, _VisitorMode.Merge = 1, _VisitorMode[_VisitorMode.Extract] = "Extract", _VisitorMode[_VisitorMode.Merge] = "Merge";
    var _Visitor = function() {
      ...
      return _Visitor.prototype.extract = function(nodes, interpolationConfig) {
        ...
        Serializer = function() {
          function Serializer() {}
          return Serializer.prototype.write = function(messages, locale) {}, Serializer.prototype.load = function(content, url) {}, Serializer.prototype.digest = function(message) {}, Serializer.prototype.createNameMapper = function(message) {
            return null;
          }, Serializer;
        }(),
        ...
        _Visitor$1 = function() {
          function _Visitor$1() {}
          return _Visitor$1.prototype.visitTag = function(tag) {
            var _this = this,
        ...
        _WriteVisitor = function() {
          ...
          return _WriteVisitor.prototype.visitText = function(text, context) {
            ...
            var ctype = getCtypeForTag(ph.tag),
                startTagPh = new Tag("x", {
                  id: ph.startName,
                  ctype: ctype
                });
            if (ph.isVoid)
              return [startTagPh];
            var closeTagPh = new Tag("x", {
              id: ph.closeName,
              ctype: ctype
            });
            return [startTagPh].concat(this.serialize(ph.children), [closeTagPh]);
          }, _WriteVisitor.prototype.visitPlaceholder = function(ph, context) {
            return [new Tag("x", {id: ph.name})];
        ...
        XmlToI18n = function() {
          ...
          return XmlToI18n.prototype.convert = function(message, url) {
            ...
            if ("x" === el.name) {
              ...
              this._addError(el, '<x> misses the "id" attribute');
            } else
        ...
        _WriteVisitor$1 = function() {
          ...
          return _WriteVisitor$1.prototype.visitText = function(text, context) {
            return [new Text$2(text.value)];
        ...
        XmlToI18n$1 = function() {
          ...
          return XmlToI18n$1.prototype.convert = function(message, url) {
            var xmlIcu = (new XmlParser).parse(message, url, !0);
            return this._errors = xmlIcu.errors, {
        ...
        _Visitor$2 = function() {
          function _Visitor$2() {}
          return _Visitor$2.prototype.visitText = function(text, context) {
            return [new Text$2(text.value)];
            ...
            return [new Tag("ph", {name: ph.name})];
        ...
        XmlToI18n$2 = function() {
          ...
          return XmlToI18n$2.prototype.convert = function(message, url) {
            var xmlIcu = (new XmlParser).parse(message, url, !0);
            return this._errors = xmlIcu.errors, {
              i18nNodes: this._errors.length > 0 || 0 == xmlIcu.rootNodes.length ? [] : visitAll(this, xmlIcu.rootNodes),
              errors: this._errors
    ...
    HtmlParser.decorators = [{type: CompilerInjectable}], HtmlParser.ctorParameters = function() {
      return [];
    };
    ...
    var PRESERVE_WS_ATTR_NAME = "ngPreserveWhitespaces",
        ...
        ProviderElementContext = function() {
          function ProviderElementContext(viewContext, _parent, _isViewRoot, _directiveAsts, attrs, refs, isTemplate, contentQueryStartId, _sourceSpan) {
            ...
            }), this._queriedTokens.get(this.viewContext.reflector.resolveExternalReference(Identifiers.ViewContainerRef)) && (this._hasViewContainer = !0), Array.from(this._allProviders.values()).forEach(function(provider) {
            ...
            if (null != dep.token) {
              if (requestingProviderType === ProviderAstType.Directive || requestingProviderType === ProviderAstType.Component) {
                if (tokenReference(dep.token) === this.viewContext.reflector.resolveExternalReference(Identifiers.Renderer) || tokenReference(dep.token) === this.viewContext.reflector.resolveExternalReference(Identifiers.ElementRef) || tokenReference(dep.token) === this.viewContext.reflector.resolveExternalReference(Identifiers.ChangeDetectorRef) || tokenReference(dep.token) === this.viewContext.reflector.resolveExternalReference(Identifiers.TemplateRef))
                  return dep;
                tokenReference(dep.token) === this.viewContext.reflector.resolveExternalReference(Identifiers.ViewContainerRef) && (this._hasViewContainer = !0);
              }
        ...
        CSS_COMMENT_REGEXP = /\/\*[\s\S]+?\*\//g,
        URL_WITH_SCHEMA_REGEXP = /^([^:\/?#]+):/,
        BoundPropertyType = {};
    BoundPropertyType.DEFAULT = 0, BoundPropertyType.LITERAL_ATTR = 1, BoundPropertyType.ANIMATION = 2, BoundPropertyType[BoundPropertyType.DEFAULT] = "DEFAULT", BoundPropertyType[BoundPropertyType.LITERAL_ATTR] = "LITERAL_ATTR", BoundPropertyType[BoundPropertyType.ANIMATION] = "ANIMATION";
    var BoundProperty = function() {
      function BoundProperty(name, expression, type, sourceSpan) {
        this.name = name, this.expression = expression, this.type = type, this.sourceSpan = sourceSpan;
      }
      return Object.defineProperty(BoundProperty.prototype, "isLiteral", {
        get: function() {
              ...
              if ("attr" == parts[0]) {
                ...
                if (nsSeparatorIdx > -1) {
                  var ns = boundPropertyName.substring(0, nsSeparatorIdx),
                      name = boundPropertyName.substring(nsSeparatorIdx + 1);
                  boundPropertyName = mergeNsAndName(ns, name);
                }
                bindingType = PropertyBindingType.Attribute;
              } else
                "class" == parts[0] ? (boundPropertyName = parts[1], bindingType = PropertyBindingType.Class, securityContexts = [_angular_core.SecurityContext.NONE]) : "style" == parts[0] && (unit = parts.length > 2 ? parts[2] : null, boundPropertyName = parts[1], bindingType = PropertyBindingType.Style, securityContexts = [_angular_core.SecurityContext.STYLE]);
      ...
      return PreparsedElement;
        ...
        TemplateParser = function() {
          ...
          return TemplateParser.prototype.parse = function(component, template, directives, pipes, schemas, templateUrl, preserveWhitespaces) {
            ...
            var htmlParseResult = this._htmlParser.parse(template, templateUrl, !0, this.getInterpolationConfig(component));
    ...
    var DirectiveNormalizer = function() {
      ...
      return DirectiveNormalizer.prototype.clearCache = function() {
        ...
        null == encapsulation && (encapsulation = this._config.defaultEncapsulation);
        var styles = templateMetadataStyles.styles.concat(templateStyles.styles),
            styleUrls = templateMetadataStyles.styleUrls.concat(templateStyles.styleUrls);
        return encapsulation === _angular_core.ViewEncapsulation.Emulated && 0 === styles.length && 0 === styleUrls.length && (encapsulation = _angular_core.ViewEncapsulation.None), new CompileTemplateMetadata({
        ...
        DirectiveResolver = function() {
          ...
          return DirectiveResolver.prototype.isDirective = function(type) {
            ...
            for (var names = new Set,
              ...
              var binding = bindings[i],
                  name = this._extractPublicName(binding);
              names.has(name) || (names.add(name), reversedResult.push(binding));
            }
            ...
            var mergedInputs = this._dedupeBindings(directive.inputs ? directive.inputs.concat(inputs) : inputs),
                ...
                mergedHost = directive.host ? Object.assign({}, directive.host, host) : host,
                mergedQueries = directive.queries ? Object.assign({}, directive.queries, queries) : queries;
            return directive instanceof _angular_core.Component ? new _angular_core.Component({
    ...
    var SummaryResolver = function() {
      ...
      return SummaryResolver.prototype.isLibraryFile = function(fileName) {}, SummaryResolver.prototype.getLibraryFileName = function(fileName) {}, SummaryResolver.prototype.resolveSummary = function(reference) {}, SummaryResolver.prototype.getSymbolsOf = function(filePath) {}, SummaryResolver.prototype.getImportAs = function(reference) {}, SummaryResolver.prototype.addSummary = function(summary) {}, SummaryResolver;
    }(),
        JitSummaryResolver = function() {
          function JitSummaryResolver() {
            this._summaries = new Map;
          }
          return JitSummaryResolver.prototype.isLibraryFile = function(fileName) {
            return !1;
    ...
    JitSummaryResolver.decorators = [{type: CompilerInjectable}], JitSummaryResolver.ctorParameters = function() {
      return [];
    };
        ...
        CompileMetadataResolver = function() {
          ...
          return CompileMetadataResolver.prototype.getReflector = function() {
            ...
            if (metadata.isComponent) {
              ...
              return _angular_core.ɵisPromise(templateMeta) && isSync ? (this._reportError(componentStillLoadingError(directiveType), directiveType), null) : SyncAsync.then(templateMeta, createDirectiveMetadata);
            }
            return createDirectiveMetadata(null), null;
          }, CompileMetadataResolver.prototype.getNonNormalizedDirectiveMetadata = function(directiveType) {
            var _this = this;
            if (!(directiveType = _angular_core.resolveForwardRef(directiveType)))
              return null;
            ...
            if (dirMeta instanceof _angular_core.Component) {
              assertArrayOfStrings("styles", dirMeta.styles), assertArrayOfStrings("styleUrls", dirMeta.styleUrls), assertInterpolationSymbols("interpolation", dirMeta.interpolation);
            ...
            dirMeta instanceof _angular_core.Component ? (changeDetectionStrategy = dirMeta.changeDetection, dirMeta.viewProviders && (viewProviders = this._getProvidersMetadata(dirMeta.viewProviders, entryComponentMetadata, 'viewProviders for "' + stringifyType(directiveType) + '"', [], directiveType)), dirMeta.entryComponents && (entryComponentMetadata = flattenAndDedupeArray(dirMeta.entryComponents).map(function(type) {
              return _this._getEntryComponentMetadata(type);
            }).concat(entryComponentMetadata)), selector || (selector = this._schemaRegistry.getDefaultComponentElementName())) : selector || (this._reportError(syntaxError("Directive " + stringifyType(directiveType) + " has no selector, please add it!"), directiveType), selector = "error");
            var providers = [];
            ...
            if (!moduleSummary) {
              var moduleMeta = this.getNgModuleMetadata(moduleType, !1);
            ...
            meta.imports && flattenAndDedupeArray(meta.imports).forEach(function(importedType) {
              ...
              exportedModuleSummary ? exportedModules.push(exportedModuleSummary) : exportedNonModuleIdentifiers.push(_this._getIdentifierMetadata(exportedType));
            });
            ...
            return this.isDirective(type) ? "directive" : this.isPipe(type) ? "pipe" : this.isNgModule(type) ? "module" : type.provide ? "provider" : "value";
            ...
            return void 0 === dependencies && (dependencies = null), factory = _angular_core.resolveForwardRef(factory), {
              reference: factory,
            ...
            if (hasUnknownDeps) {
              ...
              throwOnUnknownDeps ? this._reportError(syntaxError(message), typeOrFunc) : this._console.warn("Warning: " + message + " This will become an error in Angular v5.x");
            }
    ...
    var _CompileValueConverter = function(_super) {
      ...
      return __extends(_CompileValueConverter, _super), _CompileValueConverter.prototype.visitOther = function(value, targetIdentifiers) {
        ...
        TypeModifier = {};
    TypeModifier.Const = 0, TypeModifier[TypeModifier.Const] = "Const";
    var Type$1 = function() {
    ...
    var Expression = function() {
      ...
      return Expression.prototype.visitExpression = function(visitor, context) {}, Expression.prototype.prop = function(name, sourceSpan) {
        ...
        BuiltinVar = {};
    BuiltinVar.This = 0, BuiltinVar.Super = 1, BuiltinVar.CatchError = 2, BuiltinVar.CatchStack = 3, BuiltinVar[BuiltinVar.This] = "This", BuiltinVar[BuiltinVar.Super] = "Super", BuiltinVar[BuiltinVar.CatchError] = "CatchError", BuiltinVar[BuiltinVar.CatchStack] = "CatchStack";
    var ReadVarExpr = function(_super) {
      ...
      return __extends(ReadVarExpr, _super), ReadVarExpr.prototype.visitExpression = function(visitor, context) {
        ...
        BuiltinMethod = {};
    BuiltinMethod.ConcatArray = 0, BuiltinMethod.SubscribeObservable = 1, BuiltinMethod.Bind = 2, BuiltinMethod[BuiltinMethod.ConcatArray] = "ConcatArray", BuiltinMethod[BuiltinMethod.SubscribeObservable] = "SubscribeObservable", BuiltinMethod[BuiltinMethod.Bind] = "Bind";
    var InvokeMethodExpr = function(_super) {
      ...
      return __extends(InvokeMethodExpr, _super), InvokeMethodExpr.prototype.visitExpression = function(visitor, context) {
        ...
        AssertNotNull = function(_super) {
          ...
          return __extends(AssertNotNull, _super), AssertNotNull.prototype.visitExpression = function(visitor, context) {
        ...
        StmtModifier = {};
    StmtModifier.Final = 0, StmtModifier.Private = 1, StmtModifier.Exported = 2, StmtModifier[StmtModifier.Final] = "Final", StmtModifier[StmtModifier.Private] = "Private", StmtModifier[StmtModifier.Exported] = "Exported";
    var Statement = function() {
      ...
      return Statement.prototype.visitStatement = function(visitor, context) {}, Statement.prototype.hasModifier = function(modifier) {
        ...
        CommentStmt = function(_super) {
          ...
          return __extends(CommentStmt, _super), CommentStmt.prototype.visitStatement = function(visitor, context) {
        ...
        AstTransformer$1 = function() {
          ...
          return AstTransformer$1.prototype.transformExpr = function(expr, context) {
            return expr;
        ...
        RecursiveAstVisitor$1 = function() {
          ...
          return RecursiveAstVisitor$1.prototype.visitReadVarExpr = function(ast, context) {
            return ast;
    ...
    var SourceMapGenerator = function() {
      ...
      return SourceMapGenerator.prototype.addSource = function(url, content) {
        ...
        TypeScriptEmitter = function() {
          ...
          return TypeScriptEmitter.prototype.emitStatementsAndContext = function(srcFilePath, genFilePath, stmts, preamble, emitSourceMaps) {
            ...
            return void 0 === preamble && (preamble = ""), this.emitStatementsAndContext(srcFilePath, genFilePath, stmts, preamble).sourceText;
          }, TypeScriptEmitter;
        }(),
        _TsEmitterVisitor = function(_super) {
          function _TsEmitterVisitor() {
            var _this = _super.call(this, !1) || this;
            return _this.typeExpression = 0, _this.importsWithPrefixes = new Map, _this.reexports = new Map, _this;
          }
    ...
    var BOOLEAN = "boolean",
        ...
        DomElementSchemaRegistry = function(_super) {
          ...
          return __extends(DomElementSchemaRegistry, _super), DomElementSchemaRegistry.prototype.hasProperty = function(tagName, propName, schemaMetas) {
            ...
            return ctx || ((ctx = SECURITY_SCHEMA["*|" + propName]) || _angular_core.SecurityContext.NONE);
    ...
    DomElementSchemaRegistry.decorators = [{type: CompilerInjectable}], DomElementSchemaRegistry.ctorParameters = function() {
      return [];
    };
    var ShadowCss = function() {
      ...
      return ShadowCss.prototype.shimCssText = function(cssText, selector, hostSelector) {
        ...
        return cssText = stripComments(cssText), cssText = this._insertDirectives(cssText), this._scopeCssText(cssText, selector, hostSelector) + sourceMappingUrl;
        ...
        for (var res,
            ...
            startIndex = 0,
            sep = /( |>|\+|~(?!=))\s*/g,
            scopeAfter = selector.indexOf(_polyfillHostNoCombinator); null !== (res = sep.exec(selector)); ) {
          var separator = res[1],
              part = selector.slice(startIndex, res.index).trim();
          scopedSelector += (startIndex >= scopeAfter ? _scopeSelectorPart(part) : part) + " " + separator + " ", startIndex = sep.lastIndex;
        }
        ...
        StyleCompiler = function() {
          ...
          return StyleCompiler.prototype.compileComponent = function(outputCtx, comp) {
            ...
            return this._compileStyles(outputCtx, comp, stylesheet, !1);
            ...
            return shim ? this._shadowCss.shimCssText(style$$1, "_ngcontent-%COMP%", "_nghost-%COMP%") : style$$1;
          }, StyleCompiler;
        }();
    ...
    var ConvertActionBindingResult = function() {
      ...
      return ConvertActionBindingResult;
        ...
        _Mode = {};
    _Mode.Statement = 0, _Mode.Expression = 1, _Mode[_Mode.Statement] = "Statement", _Mode[_Mode.Expression] = "Expression";
    ...
    var LOG_VAR$1 = variable("_l"),
        ...
        ViewBuilder = function() {
          ...
          return Object.defineProperty(ViewBuilder.prototype, "viewName", {
            ...
            var _a = this._createNodeExpressions(),
                ...
                viewFlags = 0;
            this.parent || this.component.changeDetection !== _angular_core.ChangeDetectionStrategy.OnPush || (viewFlags |= 2);
            ...
            handlers.forEach(function(_a) {
              ...
              var _c = elementEventNameAndTarget(eventAst, dirAst),
                  ...
                  eventName = _c.name,
                  fullEventName = _angular_core.ɵelementEventFullName(eventTarget, eventName);
        ...
        ToJsonSerializer = function(_super) {
          function ToJsonSerializer(symbolResolver, summaryResolver) {
            var _this = _super.call(this) || this;
            return _this.symbolResolver = symbolResolver, _this.summaryResolver = summaryResolver, _this.symbols = [], _this.indexBySymbol = new Map, _this.processedSummaryBySymbol = new Map, _this.processedSummaries = [], _this;
          }
          return __extends(ToJsonSerializer, _super), ToJsonSerializer.prototype.addOrMergeSummary = function(summary) {
            var symbolMeta = summary.metadata;
            if (symbolMeta && "class" === symbolMeta.__symbolic) {
              var clone_1 = {};
            ...
            return {
              json: JSON.stringify({
                ...
                symbols: this.symbols.map(function(symbol, index) {
                  ...
                  var importAs = void 0;
                  return _this.summaryResolver.isLibraryFile(symbol.filePath) && (importAs = symbol.name + "_" + index, exportAs.push({
                    symbol: symbol,
                    exportAs: importAs
                  })), {
                    __symbol: index,
            ...
            if (value instanceof StaticSymbol) {
              var baseSymbol = this.symbolResolver.getStaticSymbol(value.filePath, value.name),
                  index = this.indexBySymbol.get(baseSymbol);
              return null == index && (index = this.indexBySymbol.size, this.indexBySymbol.set(baseSymbol, index), this.symbols.push(baseSymbol)), {
                __symbol: index,
        ...
        ForJitSerializer = function() {
          function ForJitSerializer(outputCtx, symbolResolver) {
            this.outputCtx = outputCtx, this.symbolResolver = symbolResolver, this.data = new Map;
          }
          return ForJitSerializer.prototype.addSourceType = function(summary, metadata) {
            ...
            Array.from(this.data.values()).forEach(function(_a) {
              ...
              if (summary.summaryKind === CompileSummaryKind.NgModule) {
                ...
                summary.modules.forEach(function(mod) {
        ...
        FromJsonDeserializer = function(_super) {
          function FromJsonDeserializer(symbolCache) {
            var _this = _super.call(this) || this;
            return _this.symbolCache = symbolCache, _this;
          }
          return __extends(FromJsonDeserializer, _super), FromJsonDeserializer.prototype.deserialize = function(json) {
            var _this = this,
                data = JSON.parse(json),
                importAs = [];
            return this.symbols = [], data.symbols.forEach(function(serializedSymbol) {
              var symbol = _this.symbolCache.get(serializedSymbol.filePath, serializedSymbol.name);
              _this.symbols.push(symbol), serializedSymbol.importAs && importAs.push({
                symbol: symbol,
                importAs: serializedSymbol.importAs
              });
            }), {
              summaries: visitValue(data.summaries, this, null),
              importAs: importAs
            };
        ...
        AotCompiler = function() {
          ...
          return AotCompiler.prototype.clearCache = function() {
            ...
            var _this = this,
                ...
                analyzeResult = analyzeAndValidateNgModules(programSymbols, this._host, this._metadataResolver);
            ...
            return flatten(analyzeResult.files.map(function(file) {
              return _this._compileStubFile(file.srcUrl, file.directives, file.pipes, file.ngModules, !1);
            }));
          }, AotCompiler.prototype.emitPartialStubs = function(analyzeResult) {
            var _this = this;
            return flatten(analyzeResult.files.map(function(file) {
              return _this._compileStubFile(file.srcUrl, file.directives, file.pipes, file.ngModules, !0);
            }));
            ...
            var _this = this,
                fileSuffix = splitTypescriptSuffix(srcFileUrl, !0)[1],
                generatedFiles = [],
                ngFactoryOutputCtx = this._createOutputContext(ngfactoryFilePath(srcFileUrl, !0)),
                jitSummaryOutputCtx = this._createOutputContext(summaryForJitFileName(srcFileUrl, !0));
            ngModules.forEach(function(ngModuleReference) {
              _this._ngModuleCompiler.createStub(ngFactoryOutputCtx, ngModuleReference), createForJitStub(jitSummaryOutputCtx, ngModuleReference);
            });
            var partialJitStubRequired = !1,
                partialFactoryStubRequired = !1;
            return directives.forEach(function(dirType) {
              var compMeta = _this._metadataResolver.getDirectiveMetadata(dirType);
              partialJitStubRequired = !0, compMeta.isComponent && (compMeta.template.externalStylesheets.forEach(function(stylesheetMeta) {
                ...
                _createTypeReferenceStub(styleContext, Identifiers.ComponentFactory), generatedFiles.push(_this._codegenSourceModule(stylesheetMeta.moduleUrl, styleContext));
              }), partialFactoryStubRequired = !0);
            }), (partialFactoryStubRequired || !partial) && ngFactoryOutputCtx.statements.length <= 0 && _createTypeReferenceStub(ngFactoryOutputCtx, Identifiers.ComponentFactory), (partialJitStubRequired || !partial || pipes && pipes.length > 0) && jitSummaryOutputCtx.statements.length <= 0 && _createTypeReferenceStub(jitSummaryOutputCtx, Identifiers.ComponentFactory), generatedFiles.push(this._codegenSourceModule(srcFileUrl, ngFactoryOutputCtx)), this._enableSummariesForJit && generatedFiles.push(this._codegenSourceModule(srcFileUrl, jitSummaryOutputCtx)), generatedFiles;
          }, AotCompiler.prototype._compileImplFile = function(srcFileUrl, ngModuleByPipeOrDirective, directives, pipes, ngModules, injectables) {
            var _this = this,
                fileSuffix = splitTypescriptSuffix(srcFileUrl, !0)[1],
                generatedFiles = [],
                outputCtx = this._createOutputContext(ngfactoryFilePath(srcFileUrl, !0));
            if (generatedFiles.push.apply(generatedFiles, this._createSummary(srcFileUrl, directives, pipes, ngModules, injectables, outputCtx)), ngModules.forEach(function(ngModuleType) {
            ...
            var _this = this,
                ...
                typeData = ngModules.map(function(ref) {
                  return {
                ...
                forJitOutputCtx = this._createOutputContext(summaryForJitFileName(srcFileUrl, !0)),
                _a = serializeSummaries(forJitOutputCtx, this._summaryResolver, this._symbolResolver, symbolSummaries, typeData),
                json = _a.json;
            ...
            return this._enableSummariesForJit ? [summaryJson, this._codegenSourceModule(srcFileUrl, forJitOutputCtx)] : [summaryJson];
            ...
            this._translationFormat && providers.push({
            ...
            var _this = this,
                ...
                _a = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, ngModule.schemas, templateSourceUrl(ngModule.type, compMeta, compMeta.template), preserveWhitespaces),
            ...
            return {
              ...
              importExpr: function(symbol, typeParams) {
                if (void 0 === typeParams && (typeParams = null), !(symbol instanceof StaticSymbol))
                  throw new Error("Internal error: unknown identifier " + JSON.stringify(symbol));
                var arity = _this._symbolResolver.getTypeArity(symbol) || 0,
                    _a = _this._symbolResolver.getImportAs(symbol) || symbol,
                    filePath = _a.filePath,
                    name = _a.name,
                    members = _a.members,
                    importModule = _this._symbolResolver.fileNameToModuleName(filePath, genFilePath),
                    selfReference = _this._symbolResolver.fileNameToModuleName(genFilePath, genFilePath),
            ...
            var outputCtx = this._createOutputContext(_stylesModuleUrl(stylesheetMetadata.moduleUrl, this._styleCompiler.needsStyleShim(compMeta), fileSuffix)),
                compiledStylesheet = this._styleCompiler.compileStyles(outputCtx, compMeta, stylesheetMetadata);
            return _resolveStyleStatements(this._symbolResolver, compiledStylesheet, this._styleCompiler.needsStyleShim(compMeta), fileSuffix), this._codegenSourceModule(srcFileUrl, outputCtx);
        ...
        StaticReflector = function() {
          ...
          return StaticReflector.prototype.componentModuleUrl = function(typeOrFunc) {
            ...
            var importSymbol = this.getStaticSymbol(ref.moduleName, ref.name),
                rootSymbol = this.findDeclaration(ref.moduleName, ref.name);
            return importSymbol != rootSymbol && this.symbolResolver.recordImportAs(rootSymbol, importSymbol), rootSymbol;
            ...
            return resolvedSymbol && resolvedSymbol.metadata instanceof StaticSymbol ? this.findSymbolDeclaration(resolvedSymbol.metadata) : symbol;
          }, StaticReflector.prototype.annotations = function(type) {
            var _this = this,
                annotations = this.annotationCache.get(type);
            if (!annotations) {
              annotations = [];
            ...
            function simplifyInContext(context, value, depth, references) {
              ...
              function simplifyCall(functionSymbol, targetFunction, args) {
                  ...
                  try {
                    ...
                    if (value_1 && (0 != depth || "error" != value_1.__symbolic)) {
                      ...
                      var result_1,
                          oldScope = scope;
                      try {
                        scope = functionScope.done(), result_1 = simplifyInContext(functionSymbol, value_1, depth + 1, references);
                      } finally {
                        scope = oldScope;
                      }
                      return result_1;
                ...
                return 0 === depth ? IGNORE : simplify({
                  __symbolic: "error",
              ...
              function simplify(expression) {
                ...
                if (expression instanceof StaticSymbol) {
                  ...
                  return null != declarationValue ? simplifyInContext(staticSymbol, declarationValue, depth + 1, references) : staticSymbol;
                }
                if (expression) {
                    ...
                    switch (expression.__symbolic) {
                      ...
                      case "index":
                        var indexTarget = simplifyInContext(context, expression.expression, depth, 0),
                            index = simplifyInContext(context, expression.index, depth, 0);
                      ...
                      case "select":
                        ...
                        if (selectTarget instanceof StaticSymbol) {
                          ...
                          return null != declarationValue ? simplifyInContext(selectContext, declarationValue, depth + 1, references) : selectContext;
                        }
                        return selectTarget && isPrimitive(member) ? simplifyInContext(selectContext, selectTarget[member], depth + 1, references) : null;
                      case "reference":
                        var name = expression.name,
                            localValue = scope.resolve(name);
                      ...
                      case "call":
                        if ((staticSymbol = simplifyInContext(context, expression.expression, depth + 1, 0)) instanceof StaticSymbol) {
                          ...
                          return simplifyCall(staticSymbol, resolveReferenceValue(staticSymbol), argExpressions);
                        }
                        return IGNORE;
                      case "error":
                        var message = produceErrorMessage(expression);
                        return expression.line ? (message = message + " (position " + (expression.line + 1) + ":" + (expression.character + 1) + " in the original .ts file)", self.reportError(positionalError(message, context.filePath, expression.line, expression.character), context)) : self.reportError(new Error(message), context), IGNORE;
                      case "ignore":
                        return expression;
                    }
                    return null;
              ...
              try {
                ...
                var members = context.members.length ? "." + context.members.join(".") : "",
                    message = e.message + ", resolving symbol " + context.name + members + " in " + context.filePath;
                if (e.fileName)
            ...
            var _this = this,
                ...
                calling = new Map,
                result = this.errorRecorder ? function(context, value) {
                  try {
                    return simplifyInContext(context, value, 0, 0);
                  } catch (e) {
                    _this.reportError(e, context);
                  }
                }(context, value) : simplifyInContext(context, value, 0, 0);
        ...
        BindingScope = function() {
          ...
          return BindingScope.prototype.resolve = function(name) {}, BindingScope.build = function() {
            var current = new Map;
            return {
      ...
      return __extends(PopulatedScope, _super), PopulatedScope.prototype.resolve = function(name) {
        ...
        StaticSymbolResolver = function() {
          ...
          return StaticSymbolResolver.prototype.resolveSymbol = function(staticSymbol) {
            ...
            return result || ((result = this._resolveSymbolFromSummary(staticSymbol)) ? result : (this._createSymbolsOf(staticSymbol.filePath), result = this.resolvedSymbols.get(staticSymbol)));
          }, StaticSymbolResolver.prototype.getImportAs = function(staticSymbol) {
            if (staticSymbol.members.length) {
              var baseSymbol = this.getStaticSymbol(staticSymbol.filePath, staticSymbol.name),
                  baseImportAs = this.getImportAs(baseSymbol);
            ...
            if (isGeneratedFile(staticSymbol.filePath))
              return null;
            for (var resolvedSymbol = this.resolveSymbol(staticSymbol); resolvedSymbol && resolvedSymbol.metadata instanceof StaticSymbol; )
            ...
            return this._createSymbolsOf(filePath), this.resolvedSymbols.forEach(function(resolvedSymbol) {
            ...
            if (!this.resolvedFilePaths.has(filePath)) {
              ...
              if (metadata.importAs && this.knownFileNameToModuleNames.set(filePath, metadata.importAs), metadata.metadata) {
                ...
                ReferenceTransformer = function(_super) {
                  ...
                  return __extends(ReferenceTransformer, _super), ReferenceTransformer.prototype.visitStringMap = function(map, functionParams) {
                    ...
                    var module_1 = map.module,
                        name = map.name ? unescapeIdentifier(map.name) : map.name;
                    if (!name)
                      return null;
                    var filePath = void 0;
                    return module_1 ? (filePath = self.resolveModule(module_1, sourceSymbol.filePath), filePath ? self.getStaticSymbol(filePath, name) : {
                      __symbolic: "error",
                      message: "Could not resolve " + module_1 + " relative to " + sourceSymbol.filePath + "."
                    }) : functionParams.indexOf(name) >= 0 ? {
                      __symbolic: "reference",
                      name: name
                    } : topLevelSymbolNames.has(name) ? self.getStaticSymbol(topLevelPath, name) : void 0;
                  }, ReferenceTransformer;
                }(ValueTransformer),
                transformedMeta = visitValue(metadata, new ReferenceTransformer, []);
            return transformedMeta instanceof StaticSymbol ? this.createExport(sourceSymbol, transformedMeta) : new ResolvedStaticSymbol(sourceSymbol, transformedMeta);
            ...
            if (!moduleMetadata) {
              ...
              if (moduleMetadata || (moduleMetadata = {
                ...
                module: module,
                metadata: {}
              }), 3 != moduleMetadata.version) {
                var errorMessage = 2 == moduleMetadata.version ? "Unsupported metadata version " + moduleMetadata.version + " for module " + module + ". This module should be compiled with a newer version of ngc" : "Metadata version mismatch for module " + module + ", found version " + moduleMetadata.version + ", expected 3";
            ...
            return filePath ? this.getStaticSymbol(filePath, symbolName) : (this.reportError(new Error("Could not resolve module " + module + (containingFile ? " relative to $ {\n            containingFile\n          } " : ""))), this.getStaticSymbol("ERROR:" + module, symbolName));
        ...
        AotSummaryResolver = function() {
          ...
          return AotSummaryResolver.prototype.isLibraryFile = function(filePath) {
            ...
            return summary || (this._loadSummaryFile(staticSymbol.filePath), summary = this.summaryCache.get(staticSymbol)), summary;
            ...
            var _this = this;
            if (!this.loadedFilePaths.has(filePath) && (this.loadedFilePaths.add(filePath), this.isLibraryFile(filePath))) {
        ...
        JitEmitterVisitor = function(_super) {
          function JitEmitterVisitor() {
            var _this = _super.apply(this, arguments) || this;
            return _this._evalArgNames = [], _this._evalArgValues = [], _this._evalExportedVars = [], _this;
          }
        ...
        JitCompiler = function() {
          ...
          return Object.defineProperty(JitCompiler.prototype, "injector", {
            get: function() {
            ...
            var _this = this;
            this.clearCache(), flattenSummaries(summaries).forEach(function(summary) {
            ...
            return SyncAsync.then(this._loadModules(moduleType, isSync), function() {
              ...
              return _this._compileComponents(moduleType, componentFactories), new _angular_core.ModuleWithComponentFactories(_this._compileModule(moduleType), componentFactories);
            });
            ...
            if (!ngModuleFactory) {
              var moduleMeta_1 = this._metadataResolver.getNgModuleMetadata(moduleType),
                  extraProviders = [this._metadataResolver.getProviderMetadata(new ProviderMeta(_angular_core.Compiler, {useFactory: function() {
                      return new ModuleBoundCompiler(_this, moduleMeta_1.type.reference);
                    }}))],
                  outputCtx = createOutputContext(),
                  compileResult = this._ngModuleCompiler.compile(outputCtx, moduleMeta_1, extraProviders);
              ngModuleFactory = this._compilerConfig.useJit ? jitStatements(ngModuleJitUrl(moduleMeta_1), outputCtx.statements)[compileResult.ngModuleFactoryVar] : interpretStatements(outputCtx.statements)[compileResult.ngModuleFactoryVar], this._compiledNgModuleCache.set(moduleMeta_1.type.reference, ngModuleFactory);
            }
            ...
            if (!compiledTemplate) {
              ...
              var componentFactory = compMeta.componentFactory,
                  ...
                  hostMeta = createHostComponentMeta(hostClass, compMeta, _angular_core.ɵgetComponentViewDefinitionFactory(componentFactory));
            ...
            if (!template.isCompiled) {
              ...
              var evalResult,
                  ...
                  _a = this._templateParser.parse(compMeta, compMeta.template.template, directives, pipes, template.ngModule.schemas, templateSourceUrl(template.ngModule.type, template.compMeta, template.compMeta.template), preserveWhitespaces),
                  parsedTemplate = _a.template,
                  usedPipes = _a.pipes,
              ...
              evalResult = this._compilerConfig.useJit ? jitStatements(templateJitUrl(template.ngModule.type, template.compMeta), outputContext.statements) : interpretStatements(outputContext.statements);
            ...
            return this._resolveStylesCompileResult(result, externalStylesheetsByModuleUrl), this._compilerConfig.useJit ? jitStatements(sharedStylesheetJitUrl(result.meta, this._sharedStylesheetCount++), result.outputCtx.statements)[result.stylesVar] : interpretStatements(result.outputCtx.statements)[result.stylesVar];
          }, JitCompiler;
        }();
    ...
    var CompiledTemplate = function() {
      ...
      return CompiledTemplate.prototype.compiled = function(viewClass, rendererType) {
        ...
        JitReflector = function() {
          function JitReflector() {
            this.reflectionCapabilities = new _angular_core.ɵReflectionCapabilities;
          }
          return JitReflector.prototype.componentModuleUrl = function(type, cmpMetadata) {
            var moduleId = cmpMetadata.moduleId;
            if ("string" == typeof moduleId) {
              return getUrlScheme(moduleId) ? moduleId : "package:" + moduleId;
            }
            if (null !== moduleId && void 0 !== moduleId)
        ...
        JitCompilerFactory = function() {
          ...
          return JitCompilerFactory.prototype.createCompiler = function(options) {
            ...
            return _angular_core.ReflectiveInjector.resolveAndCreate([COMPILER_PROVIDERS, {
              provide: CompilerConfig,
              useFactory: function() {
              ...
              deps: []
            }, opts.providers]).get(_angular_core.Compiler);
          }, JitCompilerFactory;
        }();
