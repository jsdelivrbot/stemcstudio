  ...
  module.exports = function(grunt) {
    ...
    var config = {
      ...
      concat: {
        ...
        dist: {
          src: ['src/options.js', 'src/showdown.js', 'src/helpers.js', 'src/converter.js', 'src/subParsers/*.js', 'src/loader.js'],
      ...
      conventionalGithubReleaser: {release: {options: {
            auth: {
              type: 'oauth',
              token: process.env.GH_TOEKN
            },
            changelogOpts: {preset: 'angular'}
          }}},
      simplemocha: {
        node: {
          src: 'test/node/**/*.js',
    ...
    grunt.registerTask('performancejs', function() {
      'use strict';
      var perf = require('./test/node/performance');
      perf.runTests();
      perf.generateLogs();
    });
    grunt.registerTask('single-test', function(grep) {
      'use strict';
      grunt.config.merge({simplemocha: {single: {options: {grep: grep}}}});
