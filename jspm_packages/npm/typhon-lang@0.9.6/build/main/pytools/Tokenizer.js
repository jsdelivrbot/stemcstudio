...
var Tokenizer = (function() {
  function Tokenizer(interactive, callback) {
    ...
    this.doneFunc = function doneOrFailed() {
      ...
      if (this.callback(Tokens_1.Tokens.T_ENDMARKER, '', [this.lnum, 0], [this.lnum, 0], ''))
        return 'done';
      return 'failed';
    };
  }
  Tokenizer.prototype.generateTokens = function(line) {
    ...
    if (this.contstr.length > 0) {
      if (!line) {
        throw new TokenError_1.TokenError("EOF in multi-line string", this.strstart[0], this.strstart[1]);
      }
      this.endprog.lastIndex = 0;
      endmatch = this.endprog.test(line);
      if (endmatch) {
        pos = end = this.endprog.lastIndex;
        if (this.callback(Tokens_1.Tokens.T_STRING, this.contstr + line.substring(0, end), this.strstart, [this.lnum, end], this.contline + line))
          return 'done';
        this.contstr = '';
        this.needcont = false;
        this.contline = undefined;
      } else if (this.needcont && line.substring(line.length - 2) !== "\\\n" && line.substring(line.length - 3) !== "\\\r\n") {
        if (this.callback(Tokens_1.Tokens.T_ERRORTOKEN, this.contstr + line, this.strstart, [this.lnum, line.length], this.contline)) {
          return 'done';
        }
        this.contstr = '';
        this.contline = undefined;
      ...
      if ("#\r\n".indexOf(line.charAt(pos)) !== -1) {
        if (line.charAt(pos) === '#') {
          ...
          if (this.callback(Tokens_1.Tokens.T_COMMENT, comment_token, [this.lnum, pos], [this.lnum, pos + comment_token.length], line)) {
            return 'done';
          }
          if (this.callback(Tokens_1.Tokens.T_NL, line.substring(nl_pos), [this.lnum, nl_pos], [this.lnum, line.length], line)) {
            return 'done';
          }
          return false;
        } else {
          if (this.callback(Tokens_1.Tokens.T_NL, line.substring(pos), [this.lnum, pos], [this.lnum, line.length], line))
            return 'done';
          if (!this.interactive)
            return false;
        }
      ...
      if (column > this.indents[this.indents.length - 1]) {
        ...
        if (this.callback(Tokens_1.Tokens.T_INDENT, line.substring(0, pos), [this.lnum, 0], [this.lnum, pos], line))
          return 'done';
      }
      while (column < this.indents[this.indents.length - 1]) {
        ...
        if (this.callback(Tokens_1.Tokens.T_DEDENT, '', [this.lnum, pos], [this.lnum, pos], line)) {
          return 'done';
        }
      }
    } else {
    ...
    while (pos < max) {
      ...
      if (pseudomatch) {
        ...
        if (this.numchars.indexOf(initial) !== -1 || (initial === '.' && token !== '.')) {
          if (this.callback(Tokens_1.Tokens.T_NUMBER, token, spos, epos, line))
            return 'done';
        } else if (initial === '\r' || initial === '\n') {
          var newl = Tokens_1.Tokens.T_NEWLINE;
          if (this.parenlev > 0)
            newl = Tokens_1.Tokens.T_NL;
          if (this.callback(newl, token, spos, epos, line))
            return 'done';
        } else if (initial === '#') {
          if (this.callback(Tokens_1.Tokens.T_COMMENT, token, spos, epos, line))
            return 'done';
          ...
          if (token[token.length - 1] === '\n') {
            ...
            if (this.callback(Tokens_1.Tokens.T_STRING, token, spos, epos, line))
              return 'done';
          }
        } else if (this.namechars.indexOf(initial) !== -1) {
          if (this.callback(Tokens_1.Tokens.T_NAME, token, spos, epos, line))
            return 'done';
        } else if (initial === '\\') {
          if (this.callback(Tokens_1.Tokens.T_NL, token, spos, [this.lnum, pos], line))
            return 'done';
          this.continued = true;
        } else {
          if ('([{'.indexOf(initial) !== -1)
            ...
            this.parenlev -= 1;
          if (this.callback(Tokens_1.Tokens.T_OP, token, spos, epos, line))
            return 'done';
        }
      } else {
        if (this.callback(Tokens_1.Tokens.T_ERRORTOKEN, line.charAt(pos), [this.lnum, pos], [this.lnum, pos + 1], line))
          return 'done';
        pos += 1;
      }
    }
