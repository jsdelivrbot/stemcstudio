...
import { fileExtensionIs, removeExtension } from './fileExtensionIs';
...
export class DefaultLanguageServiceHost implements ts.LanguageServiceHost {
     ...
     *
     */
    private addScript(fileName: string, content: string, version = 1): ScriptInfo {
        const script = new ScriptInfo(content, version);
        this.scripts[fileName] = script;
        return script;
    }

    /**
     * Ensures a mapping from the specified module name to a fileName.
     * The fileName, if any, that was mapped is returned.
     */
    ensureModuleMapping(moduleName: string, fileName: string): string {
        ...
        const previousFileName = this.moduleNameToFileName[moduleName];
        this.moduleNameToFileName[moduleName] = fileName;
        return previousFileName;
    }

    /**
     * Applies the delta to the specified file and returns the new version number of the file.
     */
    ...
    /**
     * Removes the mapping from the specified module name to a fileName.
     * The fileName that was mapped is returned.
     */
    removeModuleMapping(moduleName: string): string {
        // console.lg(`removeModuleMapping(moduleName=${moduleName})`);
    ...
    /**
     * Removes the specified script from the cache of scripts.
     * Returns true if the script was removed, false if the script does not exist (idempotency).
     */
    ...
    /**
     * Ensures the specified script exists in the cache of scripts.
     * Returns true if the script was added, false if the script exists and was updated (idempotency).
     * The version number is bumped in the process.
     */
    ...
    setScriptContentAndVersionNumber(fileName: string, content: string, version: number): boolean {
        const script = this.scripts[fileName];
        if (script) {
    ...
    getScriptVersionNumber(fileName: string): number | undefined {
        const script = this.scripts[fileName];
        if (script) {
            return script.version;
        }
    ...
    getLineAndColumn(fileName: string, position): { line: number, column: number } {
        const script = this.scripts[fileName];
        if (script) {
    ...
    public lineAndColumnToIndex(fileName: string, lineAndColumn: { line: number; column: number }): number {
        const script = this.scripts[fileName];
        if (script) {
    ...
    resolveModuleNames(moduleNames: string[], containingFile: string): ts.ResolvedModuleFull[] {
        ...
        const ns = moduleNames.map((moduleName) => {
            ...
            const fileName = fileNames.find((candidateFileName) => {
                ...
                if (isRelativeToBaseURI) {
                    ...
                    if (fileExtensionIs(simpleName, '.js')) {
                        const basicName = removeExtension(simpleName, '.js');
                        if (candidateFileName.indexOf(`${basicName}.ts`) >= 0) {
                            return true;
                        }
                    }
                    else if (fileExtensionIs(simpleName, '.jsx')) {
                        const basicName = removeExtension(simpleName, '.jsx');
        ...
        // console.lg(`resolveModuleNames(moduleNames=${JSON.stringify(moduleNames)}, containingFile=${JSON.stringify(containingFile)}) => ${JSON.stringify(ns)}`);
        return ns;
    }
}
