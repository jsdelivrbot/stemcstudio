...
import { ApplyDeltaRequest } from '../mode/typescript/LanguageServiceEvents';
...
import { GetOutputFilesResponse } from '../mode/typescript/LanguageServiceEvents';
...
export class LanguageServiceWorker {
    ...
    constructor(private sender: WorkerCallback) {
        ...
        sender.on<ApplyDeltaRequest>(EVENT_APPLY_DELTA, (message) => {
            const { fileName, delta, callbackId } = message.data;
            try {
                if (this.trace) {
                    console.log(`${EVENT_APPLY_DELTA}(${fileName}, ${JSON.stringify(delta)})`);
                }
                const version = this.dispatcher.applyDelta(fileName, delta);
                this.resolve(EVENT_APPLY_DELTA, version, callbackId);
            }
            catch (reason) {
        ...
        sender.on<GetOutputFilesRequest>(EVENT_GET_OUTPUT_FILES, (message) => {
            ...
            try {
                ...
                const version = this.dispatcher.getScriptVersionNumber(fileName);
                const outputFiles = this.dispatcher.getOutputFiles(fileName);
                this.resolve<GetOutputFilesResponse>(EVENT_GET_OUTPUT_FILES, { fileName, outputFiles, version }, callbackId);
            }
            catch (reason) {
