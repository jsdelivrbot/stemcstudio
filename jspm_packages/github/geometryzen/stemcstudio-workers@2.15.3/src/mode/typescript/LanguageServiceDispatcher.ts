...
import { transpileModule as transpilePythonModule } from 'typhon-typescript';
...
export class LanguageServiceDispatcher {
     ...
     * @param tsHost
     */
    constructor(private readonly jsHost: DefaultLanguageServiceHost, private readonly pyHost: DefaultLanguageServiceHost, private readonly tsHost: DefaultLanguageServiceHost) {
        this.tsDocumentRegistry = new DocumentRegistryInspector(ts.createDocumentRegistry(useCaseSensitiveFileNames));
    }
    /**
     * Applies the delta to the file and returns the new version number of the file.
     */
    applyDelta(fileName: string, delta: Delta): number {
    ...
    getScriptVersionNumber(fileName: string): number {
        if (isTypeScriptFileName(fileName)) {
    ...
    private ensurePythonLanguageService(): PythonLanguageService {
        if (!this.pyLanguageService) {
            this.pyLanguageService = new PythonLanguageService(this.pyHost/*, this.tsDocumentRegistry*/);
        }
        return this.pyLanguageService;
    }
    private mapTsDiagnosticsToPy(tsFileName: string, tsDiagnostics: Diagnostic[], pyFileName: string): Diagnostic[] {
        const pyLS = this.ensurePythonLanguageService();
        const tsHost = this.tsHost;
        const pyHost = this.pyHost;
        return tsDiagnostics.map((tsDiagnostic) => {

            const category = tsDiagnostic.category;
            const code = tsDiagnostic.code;
            const length = tsDiagnostic.length;
            const message = tsDiagnostic.message;

            const tsLineAndColumn = tsHost.getLineAndColumn(tsFileName, tsDiagnostic.start);
            const sourceMap = pyLS.getSourceMap(pyFileName);
            const pyLineAndColumn = sourceMap.getSourcePosition(tsLineAndColumn);
            const pyStart = pyHost.lineAndColumnToIndex(pyFileName, pyLineAndColumn);

            return { category, code, length, message, start: pyStart };
        });
    ...
    getSyntaxErrors(fileName: string): Diagnostic[] {
        if (isTypeScriptFileName(fileName)) {
            ...
            const diagnostics = tsLS.getSyntacticDiagnostics(fileName);
    ...
    getLintErrors(fileName: string, configuration: IConfigurationFile): Diagnostic[] {
        if (isTypeScriptFileName(fileName)) {
            const tsLS = this.ensureTypeScriptLanguageService();
            this.synchronizeFiles();
            const linter = new TypeScriptLinter({ fix: false }, tsLS);
            ...
            // Linting is currently done by the standard worker.
            return [];
        }
        else if (isPythonFileName(fileName)) {
    ...
    getOutputFiles(fileName: string): ts.OutputFile[] {
        ...
        // const t3 = Date.now();
        // console.lg(`synchronize  took ${t2 - t1} ms`);
        // console.lg(`output Files took ${t3 - t2} ms`);
        return outputFiles;
    }
    ...
    private getAtPosition<T>(fileName: string, position: number, noMappingValue: T, alias: string, callback: (tsFileName: string, tsPosition: number) => T): T {
        ...
        if (isTypeScriptFileName(fileName)) {
            ...
            const pyLineAndColumn = this.pyHost.getLineAndColumn(fileName, position);
            if (pyLineAndColumn) {
                const pyLS = this.ensurePythonLanguageService();
                const sourceMap = pyLS.getSourceMap(fileName);
                if (sourceMap) {
                    const tsLineAndColumn = sourceMap.getTargetPosition(pyLineAndColumn);
                    if (tsLineAndColumn) {
                        const tsFileName = getTargetFileName(fileName);
                        const tsIndex = this.tsHost.lineAndColumnToIndex(tsFileName, tsLineAndColumn);
    ...
    private synchronizeFiles(): void {
        // JavaScript
        for (const jsFileName of this.jsHost.getScriptFileNames()) {
            const sourceVersion = this.jsHost.getScriptVersionNumber(jsFileName);
            const tsFileName = getTargetFileName(jsFileName);
            const targetVersion = this.tsHost.getScriptVersionNumber(tsFileName);
            if (targetNeedsUpdate(sourceVersion, targetVersion)) {
                const sourceText = this.jsHost.getScriptContent(jsFileName);
                this.tsHost.setScriptContent(tsFileName, sourceText);
            }
        }
        // Python
        const pyLS = this.ensurePythonLanguageService();
        for (const pyFileName of this.pyHost.getScriptFileNames()) {
            const sourceVersion = this.pyHost.getScriptVersionNumber(pyFileName);
            const tsFileName = getTargetFileName(pyFileName);
            const targetVersion = this.tsHost.getScriptVersionNumber(tsFileName);
            if (targetNeedsUpdate(sourceVersion, targetVersion)) {
                const sourceText = this.pyHost.getScriptContent(pyFileName);
    ...
    private outputFiles(fileName: string): ts.OutputFile[] {
        ...
        if (isTypeScriptFileName(fileName)) {
            ...
            const output: ts.TranspileOutput = transpileModule(program, sourceFile, this.tsHost.getCustomTransformers());
...
function isTypeScriptFileName(fileName: string) {
    return fileExtensionIs(fileName, '.ts') || fileExtensionIs(fileName, '.tsx') || fileExtensionIs(fileName, '.d.ts');
}
...
function isPythonFileName(fileName: string) {
    // return ['.py', '.pyx', '.d.py'].map(function (extension: '.py') { return fileExtensionIs(fileName, extension); }).reduce(function (a, b) { return a || b; }, false);
    return fileExtensionIs(fileName, '.py') || fileExtensionIs(fileName, '.pyx') || fileExtensionIs(fileName, '.d.py');
}
...
/**
 * Computes the TypeScript fileName with a `ts` or `tsx` extension.
 */
function getTargetFileName(fileName: string) {
    if (fileExtensionIs(fileName, '.js')) {
        const end = fileName.lastIndexOf('.js');
        return `${fileName.substring(0, end)}.ts`;
    }
    else if (fileExtensionIs(fileName, '.jsx')) {
        const end = fileName.lastIndexOf('.jsx');
        return `${fileName.substring(0, end)}.tsx`;
    }
    else if (fileExtensionIs(fileName, '.py')) {
        const end = fileName.lastIndexOf('.py');
        return `${fileName.substring(0, end)}.ts`;
    }
    else if (fileExtensionIs(fileName, '.pyx')) {
        const end = fileName.lastIndexOf('.pyx');
        return `${fileName.substring(0, end)}.tsx`;
    }
    else {
