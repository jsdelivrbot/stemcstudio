...
const namedEntityPrefixes: { [entityKey: string]: boolean } = {};
        ...
        if (typeof ch === 'string') {
            ...
            if (ch === '#') {
                ...
                if (typeof ch === 'string' && isDigit(ch)) {
                    ...
                    while (ch !== InputStream.EOF && isDigit(ch as string)) {
                        code += ch;
                        ch = buffer.char();
                    }
            ...
            if ((ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z')) {
                ...
                if (ch === ';' || !additionalAllowedCharacter || !((typeof ch === 'string' && isAlphaNumeric(ch)) || ch === '=')) {
...
export const EntityParser = new EntityParserClass();
