...
import { STMTLIST } from 'src/mode/glsl/parser';

describe("glsl/parse", function () {
    describe("basic", function () {
        const node = <Node>parse('void main(void) {gl_Position = vec4(0.0, 0.0, 0.0, 1.0);}');
        it("(program)", function () {
            expect(node.mode).toBe(STMTLIST);
    ...
    describe("attribute", function () {
        const node = <Node>parse('attribute vec3 aVertexPosition; void main(void) {gl_Position = aVertexPosition;}');
        it("(program)", function () {
    ...
    describe("uniform", function () {
        var node = <Node>parse('uniform mat4 projectionMatrix; void main(void) {gl_Position = aVertexPosition;}');
        it("(program)", function () {
            expect(node.mode).toBe(2);
            expect(node.type).toBe('stmtlist');
            expect(node.children.length).toBe(2);
    ...
    describe("varying", function () {
        var node = <Node>parse('varying highp vec4 vColor; void main(void) {gl_Position = aVertexPosition;}');
        it("(program)", function () {
            expect(node.mode).toBe(2);
            expect(node.type).toBe('stmtlist');
            expect(node.children.length).toBe(2);
    ...
    // This fails on Chrome, but works with PhantomJS.
    xdescribe("syntax", function () {
        it("", function () {
            try {
                parse('vary ing highp vec4 vColor; void main(void) {gl_Position = aVertexPosition;}');
            }
            catch (e) {
