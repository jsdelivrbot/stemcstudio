...
export class TreeBuilder {
    ...
    adoptionAgencyEndTag(name) {
        ...
        while (outerLoopCounter++ < outerIterationLimit) {
            ...
            while (innerLoopCounter++ < innerIterationLimit) {
                ...
                const clone = this.createElement(node.namespaceURI, node.localName, node.attributes);
                const newNode = new StackItem(node.namespaceURI, node.localName, node.attributes, clone);
            ...
            const clone = this.createElement("http://www.w3.org/1999/xhtml", formattingElement.localName, formattingElement.attributes);
            const formattingClone = new StackItem(formattingElement.namespaceURI, formattingElement.localName, formattingElement.attributes, clone);
     ...
     */
    ensureNoahsArkCondition(item: StackItem) {
        const kNoahsArkCapacity = 3;
        ...
        for (let i = this.activeFormattingElements.length - 1; i >= 0; i--) {
            const candidate = this.activeFormattingElements[i];
            if (candidate === Marker)
                break;
        ...
        var attributes: Attr[] = item.attributes;
        for (let i = 0; i < attributes.length; i++) {
            var attribute = attributes[i];

            for (let j = 0; j < candidates.length; j++) {
                const candidate = candidates[j];
                const candidateAttribute = getAttribute(candidate, attribute.nodeName);
        ...
        for (let i = kNoahsArkCapacity - 1; i < candidates.length; i++)
