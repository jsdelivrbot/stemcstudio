...
export const IDENT = 0;
export const STMT = 1;
export const STMTLIST = 2;
let STRUCT = 3;
let FUNCTION = 4;
let FUNCTIONARGS = 5;
let DECL = 6;
...
// map of stmt types (mode) to human readable strings.
const stmt_type = [
    'ident'
    , 'stmt'
    , 'stmtlist'
...
export default function parser(): (data: Token) => Node | Node[] {

    function reader(data: Token): Node | Node[] {
        if (data === null) {
            return end(), program
        }
    ...
    function special_fake(x: Node) {
        state.unshift(x)
        state.shift()
    }

    function special_unshift(_node: Node, add_child?: boolean) {
    ...
    function special_shift() {
        var _node: Node = [].shift.call(this);
        var okay = check[this.length];
        var emit = false
    ...
    function parse_decl() {
        function invariant_or_not(): any {  // FIXME
            if (token.data === 'invariant') {
                ...
                state.fake(mknode(PLACEHOLDER, { data: '', position: token.position, type: void 0 }))
                return Advance
            }
        }
    ...
    function decl(flags: number) {
        var _: Node = mknode(DECL, token, node)  // FIXME
        _.flags = flags
        return _
    }

    function struct(allow_assign?: boolean, allow_comma?: boolean) {
        var _: Node = mknode(STRUCT, token, node)  // FIXME
        _.allow_assign = allow_assign === undefined ? true : allow_assign
        _.allow_comma = allow_comma === undefined ? true : allow_comma
        return _
    }

    function expr(arg0?: string, arg1?: string) {
        const n: Node = mknode(EXPR, token, node)  // FIXME?
    ...
    function unexpected(str: string) {
        ...
        const e = new Error((str || 'unexpected ' + state) + ' at line ' + state[0].token.line);
        e['line'] = line;
        e['column'] = column;
        throw e;
    }
    ...
    function assert(type: string, data?: any) {
        return assert_null_string_or_array(type, token.type) && assert_null_string_or_array(data, token.data)
    }
    ...
    function stative(arg0?: any, arg1?: any, arg2?: any, arg3?: any, arg4?: any, arg5?: any, arg6?: any, arg7?: any, arg8?: any, arg9?: any, argA?: any, argB?: any) {
        ...
        return function (): number | void {
            var current: { stage?: number } = state[0]
    ...
    function advance(op: string, t?: string) {
        ...
        return function () {
            ...
            const last_node: Node = children[children.length - 1]
    ...
    function advance_expr(until: string) {
        return function () {
    ...
    function advance_ident(declare: boolean) {
        return declare ? function () {
            ...
            var s: Node = Object.create(state.scope.find(token.data))
            s.token = token
    ...
    function maybe_stmtlist(skip: number) {
        return function () {
    ...
    state.create_node = function () {
        var n: Node = mknode(IDENT, token)  // FIXME
...
function mknode(mode: number, sourcetoken: Token, unused?: any): Node {  // FIXME: What is the third argument?
    const children: Node[] = []
    return {
        mode: mode
        , token: sourcetoken
