...
import { IDENT, STMT, STMTLIST } from 'src/mode/glsl/parser';
...
describe("glsl/parse", function () {
    ...
    describe("syntax", function () {
        it("", function () {
            try {
                const node = parse('vary ing highp vec4 vColor; void main(void) {gl_Position = aVertexPosition;}');
            }
            catch (e) {
                expect(e.message).toBe("did not use all tokens");
                expect(e.line).toBe(91);
