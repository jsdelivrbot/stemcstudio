...
// DGH: It may just make sense to replace this function.
import { loadRules } from "./ruleLoader";
// import { loadRules } from "./browserRuleLoader";
...
class Linter {
    ...
    */
    ...
    public lint(fileName: string/*, source: string*/, configuration: IConfigurationFile = DEFAULT_CONFIG): void {
        ...
        // DGH: Will need to get some IRule(s)!
        // const enabledRules: IRule[] = [];
    ...
    */
    ...
    private getEnabledRules(sourceFile: ts.SourceFile, configuration: IConfigurationFile = DEFAULT_CONFIG, isJs: boolean): IRule[] {
        const configurationRules: { [name: string]: boolean } = isJs ? configuration.jsRules : configuration.rules;
        ...
        const rulesDirectories = arrayify(this.options.rulesDirectory).concat(arrayify(configuration.rulesDirectory));
