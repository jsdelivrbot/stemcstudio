...
import { hasModifier, isBlockScopedVariable, isNodeFlagSet } from '../language/utils';
...
class NoVarKeywordWalker extends RuleWalker {
    public visitVariableStatement(node: ts.VariableStatement) {
        if (!hasModifier(node.modifiers, ts.SyntaxKind.DeclareKeyword)
            && !isBlockScopedVariable(node)) {
    ...
    private handleInitializerNode(node: ts.VariableDeclarationList | ts.Expression | undefined) {
        if (node && node.kind === ts.SyntaxKind.VariableDeclarationList &&
            !(isNodeFlagSet(node, ts.NodeFlags.Let) || isNodeFlagSet(node, ts.NodeFlags.Const))) {
