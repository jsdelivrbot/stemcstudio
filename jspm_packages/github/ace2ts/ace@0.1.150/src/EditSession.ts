...
export default class EditSession implements EventBus<EditSession> {
  ...
  */
  ...
  /*
  private importMode(modeName: string, options?: {}): Promise<LanguageMode> {
      ...
      if (!this.$mode) {
          this.$onChangeMode(new TextMode('', []), true, function(err: any) {
              if (!err) {
                  // Do nothing.
              }
              else {
      ...
      return new Promise<LanguageMode>((success, fail) => {
          if (this.$modes[modeName] && !options) {
              // We've already got that mode cached, use it.
              success(this.$modes[modeName]);
          }
          else {
              if (this.$modes[modeName] && !options) {
                  success(this.$modes[modeName]);
