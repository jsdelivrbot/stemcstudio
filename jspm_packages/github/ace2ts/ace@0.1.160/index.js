...
function main(element0, element1, element2) {
    ...
    var workerImports = systemImports.concat(['/jspm_packages/github/ace2ts/ace-workers@0.1.54/dist/ace-workers.js']);
    var typescriptServices = ['/assets/js/typescriptServices.js'];

    var editor0 = ace.edit(element0);
//    editor0.setLanguageMode(ace.createCssMode('/assets/js/worker.js', workerImports), function(err) {
...
//    });
    ...
    editor1.setLanguageMode(ace.createTypeScriptMode('/assets/js/worker.js', workerImports), function(err) {
        console.log("TypeScript setLanguageMode completed, err => " + err);
    });
...
//    });
    workspace.init(function(err) {
        console.log("workspace init completed, err => " + err);
    });
    ...
    workspace.attachEditor('one.ts', editor1, function(err) {
        console.log("one is attached")
    });
    workspace.attachEditor('two.ts', editor2, function(err) {
        console.log("two is attached")
    });
