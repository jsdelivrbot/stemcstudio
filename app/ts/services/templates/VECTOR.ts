...
export function VECTOR(tabString: string, dims: number): string {
    ...
    lines.push(_ + "__add__(rhs: any): Vector");
    lines.push(_ + "__radd__(lhs: any): Vector");
    lines.push(_ + "__sub__(rhs: any): Vector");
    lines.push(_ + "__rsub__(lhs: any): Vector");
    lines.push(_ + "__mul__(rhs: any): Vector");
    lines.push(_ + "__rmul__(lhs: any): Vector");
    lines.push(_ + "__div__(rhs: any): Vector");
    lines.push(_ + "__rdiv__(lhs: any): Vector");
    lines.push("}");
    //
    lines.push("");
    ...
    lines.push(_ + _ + "__add__(rhs: any): Vector {");
    ...
    lines.push(_ + _ + _ + _ + "throw new Error(`+(this: Vector, rhs: Vector), rhs (${typeof rhs}) must be a Vector.`)");
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + "},");
    lines.push(_ + _ + "__radd__(lhs: any): Vector {");
    lines.push(_ + _ + _ + "if (isCartesianVector(lhs)) {");
    lines.push(_ + _ + _ + _ + `return cvec(${coords.map((coord) => `lhs.${coord} + ${coord}`).join(', ')})`);
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + _ + "else {");
    lines.push(_ + _ + _ + _ + "throw new Error(`+(lhs: Vector, this: Vector), lhs (${typeof lhs}) must be a Vector.`)");
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + "},");
    lines.push(_ + _ + "__sub__(rhs: any): Vector {");
    lines.push(_ + _ + _ + "if (isCartesianVector(rhs)) {");
    lines.push(_ + _ + _ + _ + `return cvec(${coords.map((coord) => `${coord} - rhs.${coord}`).join(', ')})`);
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + _ + "else {");
    ...
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + "},");
    lines.push(_ + _ + "__rsub__(lhs: any): Vector {");
    lines.push(_ + _ + _ + "if (isCartesianVector(lhs)) {");
    lines.push(_ + _ + _ + _ + `return cvec(${coords.map((coord) => `lhs.${coord} - ${coord}`).join(', ')})`);
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + _ + "else {");
    ...
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + "},");
    lines.push(_ + _ + "__mul__(rhs: any): Vector {");
    lines.push(_ + _ + _ + "if (typeof rhs === 'number') {");
    lines.push(_ + _ + _ + _ + `return cvec(${coords.map((coord) => `${coord} * rhs`).join(', ')})`);
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + _ + "else {");
    lines.push(_ + _ + _ + _ + "throw new Error(`*(this: Vector, rhs: number), rhs (${typeof rhs}) must be a number.`)");
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + "},");
    lines.push(_ + _ + "__rmul__(lhs: any): Vector {");
    lines.push(_ + _ + _ + "if (typeof lhs === 'number') {");
    lines.push(_ + _ + _ + _ + `return cvec(${coords.map((coord) => `lhs * ${coord}`).join(', ')})`);
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + _ + "else {");
    lines.push(_ + _ + _ + _ + "throw new Error(`*(lhs: number, this: Vector), lhs (${typeof lhs}) must be a number.`)");
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + "},");
    lines.push(_ + _ + "__div__(rhs: any): Vector {");
    lines.push(_ + _ + _ + "if (typeof rhs === 'number') {");
    lines.push(_ + _ + _ + _ + `return cvec(${coords.map((coord) => `${coord} / rhs`).join(', ')})`);
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + _ + "else {");
    ...
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + "},");
    lines.push(_ + _ + "__rdiv__(lhs: any): Vector {");
    lines.push(_ + _ + _ + "if (typeof lhs === 'number') {");
    lines.push(_ + _ + _ + _ + `return cvec(${coords.map((coord) => `lhs / ${coord}`).join(', ')})`);
    lines.push(_ + _ + _ + "}");
    lines.push(_ + _ + _ + "else {");
