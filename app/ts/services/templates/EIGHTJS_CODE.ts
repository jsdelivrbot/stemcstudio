export default function(width: number, height: number, canvasId: string, refChange: boolean, useStats: boolean): string {
  const lines = [
    ...
    "  private controls: EIGHT.ViewController",
    "  constructor(controls: EIGHT.ViewController, trail: EIGHT.Trail) {",
    ...
    "",
    "const controls = new EIGHT.TrackballControls(camera, window)",
    "",
    ...
    "",
    "  ball.X = model.A * (cos(τ * t / model.T) * e1 + sin(τ * t / model.T) * e3)",
    "  trail.snapshot()",
    "",
    "  cube.R = exp(-model.Ω * t/2)",
    "  rod.R = exp(-model.Ω * t/2)",
    "",
    "  arrow.X = rod.X",
    "  arrow.h = rod.axis",
    "",
