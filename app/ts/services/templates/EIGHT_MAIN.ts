...
export function EIGHT_MAIN(tabString: string): string {
    ...
    lines.push("import { e1, e2, zero } from './vector'");
    lines.push("");
    ...
    lines.push(_ + _ + "model.body.position = model.body.position + e1 * Δt");
    lines.push(_ + "}");
    lines.push(_ + "if (kbd.pressed['left']) {");
    lines.push(_ + _ + "model.body.position = model.body.position - e1 * Δt");
    lines.push(_ + "}");
    lines.push(_ + "if (kbd.pressed['up']) {");
    lines.push(_ + _ + "model.body.position = model.body.position + e2 * Δt");
    lines.push(_ + "}");
    ...
    lines.push("");
    lines.push(_ + "model.body.position = zero");
    lines.push("");
    ...
    lines.push("");
    lines.push(_ + "model.timestamp = now");
    lines.push("");
