...
export default class DMP {
     ...
     * @returns Three element Array, containing the
     *     encoded text1, the encoded text2 and the array of unique strings.  The
     *     zeroth element of the array of unique strings is intentionally blank.
     */
    private diff_linesToChars_(text1: string, text2: string): LinesToCharsResult {
    ...
    /* private */ diff_map(text1: string, text2: string): Diff[] {
        ...
        let v_map1: { [index: number]: { [key: string]: boolean } }[] = [];
        let v_map2: { [index: number]: { [key: string]: boolean } }[] = [];
     ...
     * @returns Array of diff tuples.
     */
    private diff_path2(v_map: { [index: number]: { [key: string]: boolean } }[], text1: string, text2: string): Diff[] {
        const path: Diff[] = [];
        let pathLength = 0;
        let x = text1.length;
        let y = text2.length;
    ...
    diff_halfMatch_(text1: string, text2: string): string[] {
         ...
         * @returns {Array.<string>?} Five element Array, containing the prefix of
    ...
    private match_bitap_(text: string, pattern: string, loc: number): number {
         ...
         * @private
         */
        function match_bitapScore(e: number, x: number) {
     ...
     */
    private match_alphabet_(pattern: string): { [key: string]: number } {
     ...
     */
    patch_make(a: string | Diff[], opt_b?: string, opt_c?): Patch[] {
        let text1: string;
        let diffs: Diff[];
    ...
    patch_fromText(textline: string): Patch[] {
        ...
        while (textPointer < text.length) {
            ...
            while (textPointer < text.length) {
                ...
                const encodedURI = text[textPointer].substring(1);
                try {
                    const line = decodeURI(encodedURI);
                    if (sign === '-') {
                        // Deletion.
                    ...
                    // Malformed URI sequence.
                    throw new Error('Illegal escape in patch_fromText: ' + encodedURI);
                }
            }
        }
        return patches;
