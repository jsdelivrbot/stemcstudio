...
angular.module('app').controller('doodle-controller', [
  ...
  function(
    ...
    */
    ...
    const systemImports = ['/jspm_packages/system.js', '/js/jspm.config.js'];
    ...
    scope.doUpload = function(label?: string, value?: number) {
      ...
      if (token) {
        const data = doodleToGist(doodles.current());
    ...
    scope.doShare = function() {
      ...
      const dialog = <HTMLDialogElement>document.getElementById('share-dialog');
    ...
    function closure(options: IOption[]): IOption[] {
      const nameSet = new compsci.StringSet();
    ...
    function init() {
      ...
      if (gistId) {
        if (doodles.current().gistId !== gistId) {
          const token = cookie.getItem(GITHUB_TOKEN_COOKIE_NAME);
          cloud.downloadGist(token, gistId, function(err: any, doodle: mathdoodle.IDoodle) {
            if (!err) {
