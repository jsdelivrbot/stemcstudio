...
import { Replacement, IRuleMetadata, RuleFailure } from '../language/rule/rule';
import { dedent } from '../utils';
import { AbstractWalker } from '../language/walker/walker';
...
class NoInferrableTypesWalker extends AbstractWalker<IOptions> {
        ...
        if (node.type != null && node.initializer != null) {
            ...
            if (failure != null) {
                this.addFailureAtNode(node.type,
                    ...
                    this.createFix(Replacement.deleteFromTo(node.name.end, node.type.end)),
                );
            }
        }
    }
