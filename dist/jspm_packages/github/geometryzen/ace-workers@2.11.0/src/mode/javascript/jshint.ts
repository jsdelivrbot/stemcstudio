...
function checkOption(name: string, t: IToken): boolean {
    name = name.trim();
...
function isIdentifier(tkn: IToken, value: string): boolean {
    if (!tkn) {
        return false;
    }
...
function isReserved(token: IToken): boolean {
    if (!token.reserved) {
        return false;
    }
    var meta = token.meta;
...
function supplant(str: string, data: { [name: string]: (number | string) }): string {
    // FIXME: The typing of the replacer function.
    return str.replace(/\{([^{}]*)\}/g, function replacer(a: any, b: any): any {
        const r = data[b];
        return typeof r === "string" || typeof r === "number" ? r : a;
    });
}

function combine<D, S>(dest: D, src: S): void {
...
function processenforceall(): void {
    if (state.option.enforceall) {
        for (const enforceopt in bool.enforcing) {
            if (state.option[enforceopt] === void 0 &&
                !noenforceall[enforceopt]) {
                state.option[enforceopt] = true;
            }
        }
        for (const relaxopt in bool.relaxing) {
 ...
 */
function quit(code: string, token: IToken, a?: number | string, b?: number | string): void {
    var percentage = Math.floor((token.line / state.lines.length) * 100);
    var message = errors[code].desc;

    const exception: JSHintError = {
        name: "JSHintError",
        line: token.line,
    ...
    exception.reason = supplant(message, exception as any) + " (" + percentage + "% scanned).";

    throw exception;
}

function removeIgnoredMessages(): void {
    const ignored: { [line: number]: boolean } = state.ignoredLines;
...
function warning(code: string, t?: IToken, a?: number | string, b?: number | string, c?: string, d?: string): JSHintError {
    ...
    warn.reason = supplant(msg.desc, warn as any);
...
function advance(id?: string, t?: IToken): void {
...
function isInfix(token: IToken): boolean {
...
function isBeginOfExpr(prev: IToken): boolean {
...
function block(ordinary: boolean, stmt?: boolean, isfunc?: boolean, isfatarrow?: boolean, iscase?: boolean) {
    ...
    if (state.tokens.next.id === "{") {
        ...
        if (<string>state.tokens.next.id !== "}") {
...
function destructuringPatternRecursive(options): { id: string; token: IToken }[] {
    var ids: { id: string; token: IToken; }[];
    const identifiers: { id: string; token: IToken; }[] = [];
...
function blockVariableStatement(type, statement, context) {
    ...
    let tokens: { id: string; token: IToken }[];
    var lone;
    var value: IToken;
    var letblock;
...
blockstmt("if", function () {
    ...
    if (state.tokens.next.id === "else") {
        advance("else");
        if (<string>state.tokens.next.id === "if" || <string>state.tokens.next.id === "switch") {
            statement();
        } else {
            block(true, true);
        }
...
stmt("export", function () {
    ...
    if (state.tokens.next.type === "default") {
        ...
        state.nameStack.set(state.tokens.next);
        advance("default");
        const exportType = state.tokens.next.id;
    ...
    if (state.tokens.next.value === "{") {
        ...
        for (; ;) {
            ...
            if (<string>state.tokens.next.value === "as") {
                advance("as");
                if (!state.tokens.next.identifier) {
                    error("E030", state.tokens.next, state.tokens.next.value);
                }
            ...
            if (<string>state.tokens.next.value === ",") {
                advance(",");
            }
            else if (<string>state.tokens.next.value === "}") {
                advance("}");
                break;
            }
            else {
        ...
        if (<string>state.tokens.next.value === "from") {
 ...
 */
function saveAccessor(accessorType: 'get' | 'set', props, name, tkn, isClass?: boolean, isStatic?: boolean) {
 ...
 */
function checkPunctuators(token: IToken, values: string[]): boolean {
 ...
 * @returns {boolean}
 */
function checkPunctuator(token: IToken, value) {
...
export interface IJSHINT {
    (s: string, o: JSHintOptions, g?): boolean;
    errors: JSHintError[];
    /**
     * I don't think this is being used.
     */
    undefs: any[];
    internals: { id: string; elem: string; value: any }[];
 ...
 */
export const JSHINT: IJSHINT = <any>function lint(s: string, o: JSHintOptions, g): boolean {
        ...
        if (o.predef) {
            o.predef.forEach(function (item) {
                if (item[0] === "-") {
                    ...
                    const prop = Object.getOwnPropertyDescriptor(o.predef, item);
                    predefined[item] = prop ? prop.value : false;
                }
            });
        }

        each<boolean>(o.exported || null, function (value, item) {
            exported[item] = true;
        });
    ...
    if (o && o.ignoreDelimiters) {
        ...
        o.ignoreDelimiters.forEach(function (delimiterPair) {
            ...
            const reIgnoreStr = escapeRegex(delimiterPair.start) +
                "[\\s\\S]*?" +
                escapeRegex(delimiterPair.end);

            const reIgnore = new RegExp(reIgnoreStr, "ig");
    ...
    try {
        ...
        if (err && err.name === "JSHintError") {
            const nt: IToken = state.tokens.next;
            JSHINT.errors.push({
                scope: "(main)",
                raw: err.raw,
                code: err.code,
    ...
    if (JSHINT.scope === "(main)") {
        ...
        for (const k of JSHINT.internals) {
            o.scope = k.elem;
            JSHINT(k.value, o, g);
        }
    }
...
// Modules.
JSHINT.addModule = function (func: Function) {
