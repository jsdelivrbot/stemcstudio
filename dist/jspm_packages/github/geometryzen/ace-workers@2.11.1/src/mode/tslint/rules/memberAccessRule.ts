...
import { childOfKind, hasModifier } from '../language/utils';
...
export class MemberAccessWalker extends RuleWalker {
    ...
    private validateVisibilityModifiers(node: ts.ClassElement) {
        ...
        const hasAnyVisibilityModifiers = hasModifier(
            node.modifiers,
        ...
        if (!hasAnyVisibilityModifiers) {
            ...
            if (node.kind === ts.SyntaxKind.MethodDeclaration) {
                ...
                publicOnly = true;
                end = childOfKind(node, ts.SyntaxKind.ConstructorKeyword)!.getEnd();
