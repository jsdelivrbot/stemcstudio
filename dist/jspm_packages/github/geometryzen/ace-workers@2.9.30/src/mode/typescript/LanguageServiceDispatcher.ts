...
export class LanguageServiceDispatcher {
    ...
    getCompletionsAtPosition(fileName: string, position: number): ts.CompletionEntry[] {
        const completionInfo: ts.CompletionInfo = this.ensureTypeScriptLanguageService().getCompletionsAtPosition(fileName, position);
        if (completionInfo) {
    ...
    getDefinitionAtPosition(fileName: string, position: number): ts.DefinitionInfo[] {
        const definitionInfo: ts.DefinitionInfo[] = this.ensureTypeScriptLanguageService().getDefinitionAtPosition(fileName, position);
        if (definitionInfo) {
    ...
    getQuickInfoAtPosition(fileName: string, position: number): ts.QuickInfo {
        if (isTypeScriptFileName(fileName)) {
            ...
            // console.warn(`getQuickInfo('${fileName}') is not yet implemented.`);
            return void 0;
        }
        else {
    ...
    private synchronizeFiles(): void {
        for (const pyFileName of this.pyLanguageServiceHost.getScriptFileNames()) {
            ...
            if (targetNeedsUpdate(sourceVersion, targetVersion)) {
                ...
                const { code } = transpilePythonModule(sourceText);
