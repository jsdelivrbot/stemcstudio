...
export function register(linter: Linter): void {
  // Check for properties named __proto__. This special property was
  // deprecated and then re-introduced for ES6.

  linter.on("Identifier", function style_scanProto(data: Data) {
  ...
  linter.on("Identifier", function style_scanIterator(data: Data) {
  ...
  linter.on("Identifier", function style_scanCamelCase(data: Data) {
  ...
  linter.on("String", function style_scanQuotes(data: Data) {
    const quotmark = linter.getOption("quotmark");
    let code: string;
  ...
  linter.on("Number", function style_scanNumbers(data: Data) {
  ...
  linter.on("String", function style_scanJavaScriptURLs(data: Data) {
    const re = /^(?:javascript|jscript|ecmascript|vbscript|livescript)\s*:/i;
