...
import { EVENT_ENSURE_SCRIPT } from '../../src/mode/LanguageServiceEvents';
...
import { EnsureScriptRequest, RemoveScriptRequest } from '../../src/mode/LanguageServiceEvents';
...
class MockLanguageServiceProxy {
    ...
    ensureScript(fileName: string, content: string, callbackId: number): void {
        const callback = this.sender.callbacks[EVENT_ENSURE_SCRIPT];
        const data: EnsureScriptRequest = { fileName, content, callbackId };
        callback({ data });
    }
...
describe("LanguageServiceWorker", function () {
    ...
    describe("ensureScript", function () {
        ...
        it("should acknowledge", function () {
            ...
            const resolution = sender.resolutions[callbackId];
            expect(resolution).toBeDefined();
            expect(resolution.eventName).toBe(EVENT_ENSURE_SCRIPT);
        });
    });
    ...
    describe("getOutputFiles", function () {
        ...
        it("should acknowledge", function () {
            ...
            expect(resolution1.eventName).toBe(EVENT_ENSURE_SCRIPT);
