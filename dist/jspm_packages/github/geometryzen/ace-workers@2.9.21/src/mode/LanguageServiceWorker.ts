...
import { GetScriptContentRequest } from './LanguageServiceEvents';
...
import { SetScriptContentRequest } from './LanguageServiceEvents';
    ...
    constructor(private sender: WorkerCallback) {
        ...
        sender.on<SetTraceRequest>(EVENT_SET_TRACE, (message) => {
            ...
            try {
                ...
                // Make the updates.
                this.trace = newTrace;
                this.documentRegistry_.trace = newTrace;
        ...
        sender.on(EVENT_DEFAULT_LIB_CONTENT, (message: { data: { content: string; callbackId: number } }) => {
            ...
            try {
                ...
                const added = this.lsHost.setScriptContent(this.lsHost.getDefaultLibFileName({}), content);
        ...
        sender.on<GetScriptContentRequest>(EVENT_GET_SCRIPT_CONTENT, (message) => {
            ...
            try {
                ...
                const content = this.lsHost.getScriptContent(fileName);
        ...
        sender.on<SetScriptContentRequest>(EVENT_SET_SCRIPT_CONTENT, (message) => {
            ...
            try {
                ...
                const added = this.lsHost.setScriptContent(fileName, content);
        ...
        sender.on<RemoveScriptRequest>(EVENT_REMOVE_SCRIPT, (message) => {
            ...
            try {
                ...
                const removed = this.lsHost.removeScript(fileName);
        ...
        sender.on<GetDefinitionAtPositionRequest>(EVENT_GET_DEFINITION_AT_POSITION, (message) => {
            ...
            try {
                ...
                const definitionInfo: ts.DefinitionInfo[] = this.ensureLS().getDefinitionAtPosition(fileName, position);
                if (definitionInfo) {
                    this.resolve(EVENT_GET_DEFINITION_AT_POSITION, definitionInfo, callbackId);
                }
                else {
                    this.resolve(EVENT_GET_DEFINITION_AT_POSITION, [], callbackId);
                }
            }
            catch (reason) {
