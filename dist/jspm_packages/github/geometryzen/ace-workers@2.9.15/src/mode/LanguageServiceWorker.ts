    ...
    constructor(private sender: WorkerCallback) {
        ...
        sender.on(EVENT_DEFAULT_LIB_CONTENT, (message: { data: { content: string; callbackId: number } }) => {
            ...
            try {
                ...
                const added = this.lsHost.ensureScript(this.lsHost.getDefaultLibFileName({}), content);
        ...
        sender.on<EnsureScriptRequest>(EVENT_ENSURE_SCRIPT, (message) => {
            ...
            try {
                ...
                const added = this.lsHost.ensureScript(fileName, content);
                this.resolve(EVENT_ENSURE_SCRIPT, added, callbackId);
            }
            catch (reason) {
