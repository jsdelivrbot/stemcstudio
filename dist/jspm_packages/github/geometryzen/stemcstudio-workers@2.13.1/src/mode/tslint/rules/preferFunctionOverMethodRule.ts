...
class PreferFunctionOverMethodWalker extends RuleWalker {
    ...
    private shouldWarnForModifiers(node: ts.MethodDeclaration): boolean {
        if (hasModifier(node.modifiers, ts.SyntaxKind.StaticKeyword, ts.SyntaxKind.AbstractKeyword)) {
            return false;
        }
...
function methodVisibility(node: ts.MethodDeclaration): Visibility {
    ...
    else if (hasModifier(node.modifiers, ts.SyntaxKind.ProtectedKeyword)) {
