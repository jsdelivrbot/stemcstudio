...
import { doesIntersect } from '../language/utils';
    ...
    public apply(sourceFile: ts.SourceFile): RuleFailure[] {
        ...
        for (let i = 0; i < lineStarts.length - 1; ++i) {
            ...
            if ((to - from - 1) > lineLimit && !((to - from - 2) === lineLimit && source[to - 2] === "\r")) {
                ...
                const ruleFailure = new RuleFailure(sourceFile, from, to - 1, errorString, this.getOptions().ruleName);
                if (!doesIntersect(ruleFailure, disabledIntervals)) {
