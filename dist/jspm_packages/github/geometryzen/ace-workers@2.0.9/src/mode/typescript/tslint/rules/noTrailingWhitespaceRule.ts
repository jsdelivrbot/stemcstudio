...
import { IRuleMetadata, Replacement, RuleFailure } from '../language/rule/rule';
import { forEachToken } from '../language/utils';
import { dedent } from '../utils';
import { WalkContext } from '../language/walker/walkContext';
...
function walk(ctx: WalkContext<IgnoreOption>) {
    let lastSeenWasWhitespace = false;
    let lastSeenWhitespacePosition = 0;
    forEachToken(ctx.sourceFile, false, (fullText, kind, pos) => {
...
function reportFailure(ctx: WalkContext<IgnoreOption>, start: number, end: number) {
    ctx.addFailure(start, end, Rule.FAILURE_STRING, ctx.createFix(Replacement.deleteFromTo(start, end)));
}
