...
export default class LanguageServiceWorker {
    ...
    constructor(private sender: WorkerCallback) {
        ...
        sender.on<EnsureModuleMappingRequest>(EVENT_ENSURE_MODULE_MAPPING, (message) => {
            ...
            try {
                ...
                this.lsHost.ensureModuleMapping(moduleName, fileName);
                this.resolve(EVENT_ENSURE_MODULE_MAPPING, void 0, callbackId);
            }
            catch (reason) {
        ...
        sender.on<RemoveModuleMappingRequest>(EVENT_REMOVE_MODULE_MAPPING, (message) => {
            ...
            try {
                ...
                this.lsHost.removeModuleMapping(moduleName);
                this.resolve(EVENT_REMOVE_MODULE_MAPPING, void 0, callbackId);
            }
            catch (reason) {
        ...
        sender.on<SetOperatorOverloadingRequest>(EVENT_SET_OPERATOR_OVERLOADING, (message) => {
            ...
            try {
                ...
                this.setOperatorOverloading(operatorOverloading);
                this.resolve(EVENT_SET_OPERATOR_OVERLOADING, this.lsHost.operatorOverloading, callbackId);
            }
            catch (reason) {
        ...
        sender.on<SetTsConfigRequest>(EVENT_SET_TS_CONFIG, (message) => {
            ...
            try {
                if (this.trace) {
                    console.log(`${EVENT_SET_TS_CONFIG}(${JSON.stringify(settings, null, 2)})`);
                }
                try {
                    const compilerOptions = compilerOptionsFromTsConfig(settings);
                    this.setCompilationSettings(compilerOptions);
                    this.resolve(EVENT_SET_TS_CONFIG, this.lsHost.getCompilationSettings(), callbackId);
                }
                catch (e) {
    ...
    getOperatorOverloading(): boolean {
     ...
     */
    setOperatorOverloading(operatorOverloading: boolean): void {
