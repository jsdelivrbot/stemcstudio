System.register(["./typescript/DefaultLanguageServiceHost", "./typescript/DocumentRegistryInspector", "./typescript/tslint/linter"], function (exports_1, context_1) {
    "use strict";
                ...
                function LanguageServiceWorker(sender) {
                    ...
                    sender.on(EVENT_GET_LINT_ERRORS, function (message) {
                        ...
                        try {
                            ...
                            var program = _this.ensureLS().getProgram();
                            var linter = new linter_1.default({ fix: false }, program);
                            var configuration = {};
                            var rules = {};
                            rules['comment-format'] = true;
                            rules['eofline'] = true;
                            ...
                            var errors = ruleFailures.map(function (ruleFailure) {
                                var start = ruleFailure.getStartPosition().getPosition();
                                var end = ruleFailure.getEndPosition().getPosition();
                                var length = end - start;
                                return { message: ruleFailure.getFailure(), start: start, length: length };
                            });
                            _this.resolve(EVENT_GET_LINT_ERRORS, errors, callbackId);
                        }
                        catch (reason) {
