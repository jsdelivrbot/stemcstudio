...
(function(process) {
  (function(root, factory) {
    ...
    function $translatePartialLoader() {
      ...
      this.$get = ['$rootScope', '$injector', '$q', '$http', '$log', function($rootScope, $injector, $q, $http, $log) {
          ...
          return $q.all(loaders).then(function() {
            ...
            if (structureHasBeenChangedWhileLoading) {
              if (!options.__retries) {
                ...
                $log.warn('The partial loader has detected a multiple structure change (with addPort/removePart) ' + 'while loading translations. You should consider using promises of $translate.use(lang) and ' + '$translate.refresh(). Also parts should be added/removed right before an explicit refresh ' + 'if possible.');
              }
            }
            var table = {};
            prioritizedParts = getPrioritizedParts();
