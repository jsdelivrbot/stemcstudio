...
!function(global, factory) {
    ...
    if (rawParams.length > 0) {
      ...
      params.forEach(function(param) {
        var eqIdx = param.indexOf("="),
            _a = eqIdx == -1 ? [codec.decodeKey(param), ""] : [codec.decodeKey(param.slice(0, eqIdx)), codec.decodeValue(param.slice(eqIdx + 1))],
            key = _a[0],
            val = _a[1],
            list = map$$1.get(key) || [];
        list.push(val), map$$1.set(key, list);
      });
    }
    return map$$1;
  }
  ...
  function interceptingHandler(backend, interceptors) {
    return void 0 === interceptors && (interceptors = []), interceptors ? interceptors.reduceRight(function(next, interceptor) {
  ...
  var HttpHandler = function() {
    ...
    return HttpHandler.prototype.handle = function(req) {}, HttpHandler;
  }(),
      HttpBackend = function() {
        function HttpBackend() {}
        return HttpBackend.prototype.handle = function(req) {}, HttpBackend;
      }(),
      HttpUrlEncodingCodec = function() {
        function HttpUrlEncodingCodec() {}
        return HttpUrlEncodingCodec.prototype.encodeKey = function(k) {
      ...
      HttpParams = function() {
        function HttpParams(options) {
          void 0 === options && (options = {}), this.updates = null, this.cloneFrom = null, this.encoder = options.encoder || new HttpUrlEncodingCodec, this.map = options.fromString ? paramParser(options.fromString, this.encoder) : null;
        }
            ...
            switch (update.op) {
              ...
              case "d":
                ...
                var base_1 = _this.map.get(update.param) || [],
                    idx = base_1.indexOf(update.value);
                idx !== -1 && base_1.splice(idx, 1), base_1.length > 0 ? _this.map.set(update.param, base_1) : _this.map.delete(update.param);
            }
          }), this.cloneFrom = null);
        }, HttpParams;
      }(),
      HttpHeaders = function() {
        function HttpHeaders(headers) {
          var _this = this;
          this.normalizedNames = new Map, this.lazyUpdate = null, headers ? "string" == typeof headers ? this.lazyInit = function() {
            _this.headers = new Map, headers.split("\n").forEach(function(line) {
              ...
              if (index > 0) {
                ...
                _this.maybeSetNormalizedName(name, key), _this.headers.has(key) ? _this.headers.get(key).push(value) : _this.headers.set(key, [value]);
              }
            });
            ...
            _this.headers = new Map, Object.keys(headers).forEach(function(name) {
              var values = headers[name],
                  key = name.toLowerCase();
              "string" == typeof values && (values = [values]), values.length > 0 && (_this.headers.set(key, values), _this.maybeSetNormalizedName(name, key));
            });
        ...
        return HttpHeaders.prototype.has = function(name) {
          ...
          switch (update.op) {
            ...
            case "s":
              ...
              if ("string" == typeof value && (value = [value]), 0 === value.length)
                return;
            ...
            case "d":
              ...
              if (toDelete_1) {
                ...
                }), 0 === existing.length ? (this.headers.delete(key), this.normalizedNames.delete(key)) : this.headers.set(key, existing);
              } else
                this.headers.delete(key), this.normalizedNames.delete(key);
          }
      ...
      HttpRequest = function() {
        function HttpRequest(method, url, third, fourth) {
          this.url = url, this.body = null, this.reportProgress = !1, this.withCredentials = !1, this.responseType = "json", this.method = method.toUpperCase();
          var options;
          if (mightHaveBody(this.method) || fourth ? (this.body = third || null, options = fourth) : options = third, options && (this.reportProgress = !!options.reportProgress, this.withCredentials = !!options.withCredentials, options.responseType && (this.responseType = options.responseType), options.headers && (this.headers = options.headers), options.params && (this.params = options.params)), this.headers || (this.headers = new HttpHeaders), this.params) {
            var params = this.params.toString();
            if (0 === params.length)
              this.urlWithParams = url;
            else {
              var qIdx = url.indexOf("?"),
                  sep = qIdx === -1 ? "?" : qIdx < url.length - 1 ? "&" : "";
              this.urlWithParams = url + sep + params;
            }
          } else
            this.params = new HttpParams, this.urlWithParams = url;
        }
  ...
  var HttpResponseBase = function() {
    ...
    return HttpResponseBase;
      ...
      HttpHeaderResponse = function(_super) {
        function HttpHeaderResponse(init) {
          void 0 === init && (init = {});
          var _this = _super.call(this, init) || this;
          return _this.type = HttpEventType.ResponseHeader, _this;
        }
        return tslib_1.__extends(HttpHeaderResponse, _super), HttpHeaderResponse.prototype.clone = function(update) {
      ...
      HttpResponse = function(_super) {
        function HttpResponse(init) {
          void 0 === init && (init = {});
          var _this = _super.call(this, init) || this;
          return _this.type = HttpEventType.Response, _this.body = init.body || null, _this;
        }
        return tslib_1.__extends(HttpResponse, _super), HttpResponse.prototype.clone = function(update) {
      ...
      HttpErrorResponse = function(_super) {
        function HttpErrorResponse(init) {
          var _this = _super.call(this, init, 0, "Unknown Error") || this;
          return _this.name = "HttpErrorResponse", _this.ok = !1, _this.status >= 200 && _this.status < 300 ? _this.message = "Http failure during parsing for " + (init.url || "(unknown url)") : _this.message = "Http failure response for " + (init.url || "(unknown url)") + ": " + init.status + " " + init.statusText, _this.error = init.error || null, _this;
        }
        return tslib_1.__extends(HttpErrorResponse, _super), HttpErrorResponse;
      }(HttpResponseBase),
      HttpClient = function() {
        ...
        return HttpClient.prototype.request = function(first, url, options) {
          ...
          var req;
          req = first instanceof HttpRequest ? first : new HttpRequest(first, url, options.body || null, {
            headers: options.headers,
            params: options.params,
            reportProgress: options.reportProgress,
              ...
              switch (req.responseType) {
                case "arraybuffer":
                  return rxjs_operator_map.map.call(res$, function(res) {
                    if (null !== res.body && !(res.body instanceof ArrayBuffer))
  ...
  HttpClient.decorators = [{type: _angular_core.Injectable}], HttpClient.ctorParameters = function() {
  ...
  var HttpInterceptorHandler = function() {
    ...
    return HttpInterceptorHandler.prototype.handle = function(req) {
      ...
      HTTP_INTERCEPTORS = new _angular_core.InjectionToken("HTTP_INTERCEPTORS"),
  ...
  NoopInterceptor.decorators = [{type: _angular_core.Injectable}], NoopInterceptor.ctorParameters = function() {
    return [];
  };
  var nextRequestId = 0,
      ...
      JsonpCallbackContext = function() {
        function JsonpCallbackContext() {}
        return JsonpCallbackContext;
      }(),
      JsonpClientBackend = function() {
        function JsonpClientBackend(callbackMap, document) {
          this.callbackMap = callbackMap, this.document = document;
        }
        return JsonpClientBackend.prototype.nextCallback = function() {
          ...
          return new rxjs_Observable.Observable(function(observer) {
            var callback = _this.nextCallback(),
                url = req.urlWithParams.replace(/=JSONP_CALLBACK(&|$)/, "=" + callback + "$1"),
                node = _this.document.createElement("script");
            node.src = url;
            var body = null,
                finished = !1,
                cancelled = !1;
            _this.callbackMap[callback] = function(data) {
              delete _this.callbackMap[callback], cancelled || (body = data, finished = !0);
            };
            var cleanup = function() {
              node.parentNode && node.parentNode.removeChild(node), delete _this.callbackMap[callback];
            },
                onLoad = function(event) {
                  if (!cancelled) {
                    if (cleanup(), !finished)
                      return void observer.error(new HttpErrorResponse({
                        url: url,
                        status: 0,
                ...
                onError = function(error) {
                  cancelled || (cleanup(), observer.error(new HttpErrorResponse({
                    error: error,
                    status: 0,
                    statusText: "JSONP Error",
                    url: url
            ...
            return node.addEventListener("load", onLoad), node.addEventListener("error", onError), _this.document.body.appendChild(node), observer.next({type: HttpEventType.Sent}), function() {
              cancelled = !0, node.removeEventListener("load", onLoad), node.removeEventListener("error", onError), cleanup();
            };
          });
        }, JsonpClientBackend;
      }();
  JsonpClientBackend.decorators = [{type: _angular_core.Injectable}], JsonpClientBackend.ctorParameters = function() {
  ...
  var JsonpInterceptor = function() {
    ...
    return JsonpInterceptor.prototype.intercept = function(req, next) {
      return "JSONP" === req.method ? this.jsonp.handle(req) : next.handle(req);
    }, JsonpInterceptor;
  }();
  JsonpInterceptor.decorators = [{type: _angular_core.Injectable}], JsonpInterceptor.ctorParameters = function() {
  ...
  var HttpXhrBackend = function() {
    ...
    return HttpXhrBackend.prototype.handle = function(req) {
      ...
      return new rxjs_Observable.Observable(function(observer) {
        ...
        req.responseType && (xhr.responseType = req.responseType.toLowerCase());
        var reqBody = req.serializeBody(),
            ...
            onLoad = function() {
              var _a = partialFromXhr(),
                  ...
                  url = _a.url,
                  body = null;
              204 !== status && (body = "undefined" == typeof xhr.response ? xhr.responseText : xhr.response, "string" == typeof body && (body = body.replace(XSSI_PREFIX, ""))), 0 === status && (status = body ? 200 : 0);
              var ok = status >= 200 && status < 300;
              if (ok && "string" == typeof body && "json" === req.responseType)
                try {
        ...
        return xhr.addEventListener("load", onLoad), xhr.addEventListener("error", onError), req.reportProgress && (xhr.addEventListener("progress", onDownProgress), null !== reqBody && xhr.upload && xhr.upload.addEventListener("progress", onUpProgress)), xhr.send(reqBody), observer.next({type: HttpEventType.Sent}), function() {
  ...
  HttpXhrBackend.decorators = [{type: _angular_core.Injectable}], HttpXhrBackend.ctorParameters = function() {
    return [{type: XhrFactory}];
  };
  var XSRF_COOKIE_NAME = new _angular_core.InjectionToken("XSRF_COOKIE_NAME"),
      XSRF_HEADER_NAME = new _angular_core.InjectionToken("XSRF_HEADER_NAME"),
      HttpXsrfTokenExtractor = function() {
        function HttpXsrfTokenExtractor() {}
        return HttpXsrfTokenExtractor.prototype.getToken = function() {}, HttpXsrfTokenExtractor;
      }(),
      HttpXsrfCookieExtractor = function() {
        ...
        return HttpXsrfCookieExtractor.prototype.getToken = function() {
          ...
          return cookieString !== this.lastCookieString && (this.parseCount++, this.lastToken = _angular_common.ɵparseCookieValue(cookieString, this.cookieName), this.lastCookieString = cookieString), this.lastToken;
        }, HttpXsrfCookieExtractor;
      }();
  HttpXsrfCookieExtractor.decorators = [{type: _angular_core.Injectable}], HttpXsrfCookieExtractor.ctorParameters = function() {
    return [{
      type: void 0,
      decorators: [{
        type: _angular_core.Inject,
  ...
  var HttpXsrfInterceptor = function() {
    ...
    return HttpXsrfInterceptor.prototype.intercept = function(req, next) {
      ...
      if ("GET" === req.method || "HEAD" === req.method || lcUrl.startsWith("http://") || lcUrl.startsWith("https://"))
        return next.handle(req);
      var token = this.tokenService.getToken();
      return null === token || req.headers.has(this.headerName) || (req = req.clone({headers: req.headers.set(this.headerName, token)})), next.handle(req);
    }, HttpXsrfInterceptor;
  }();
  HttpXsrfInterceptor.decorators = [{type: _angular_core.Injectable}], HttpXsrfInterceptor.ctorParameters = function() {
