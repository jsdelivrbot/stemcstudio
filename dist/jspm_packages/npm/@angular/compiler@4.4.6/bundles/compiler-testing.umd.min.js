...
!function(global, factory) {
  ...
  function removeMetadata(metadata, remove, references) {
    ...
    for (var prop in remove)
      !function(prop) {
        ...
        removeValue instanceof Array ? removeValue.forEach(function(value) {
    ...
    for (var prop in metadata)
      !function(prop) {
        ...
        propValue instanceof Array ? metadata[prop] = propValue.filter(function(value) {
          return !removeObjects.has(_propHashKey(prop, value, references));
        }) : removeObjects.has(_propHashKey(prop, propValue, references)) && (metadata[prop] = void 0);
      }(prop);
  }
  function addMetadata(metadata, add) {
    for (var prop in add) {
      var addValue = add[prop],
          propValue = metadata[prop];
      null != propValue && propValue instanceof Array ? metadata[prop] = propValue.concat(addValue) : metadata[prop] = addValue;
    }
  }
  ...
  function _propHashKey(propName, propValue, references) {
    var replacer = function(key, value) {
      return "function" == typeof value && (value = _serializeReference(value, references)), value;
    };
    return propName + ":" + JSON.stringify(propValue, replacer);
  }
  function _serializeReference(ref, references) {
    var id = references.get(ref);
    return id || (id = "" + _angular_core.ɵstringify(ref) + _nextReferenceId++, references.set(ref, id)), id;
  }
  function _valueProps(obj) {
    var props = [];
    Object.keys(obj).forEach(function(prop) {
      prop.startsWith("_") || props.push(prop);
    });
    for (var proto = obj; proto = Object.getPrototypeOf(proto); )
      Object.keys(proto).forEach(function(protoProp) {
        var desc = Object.getOwnPropertyDescriptor(proto, protoProp);
        !protoProp.startsWith("_") && desc && "get" in desc && props.push(protoProp);
      });
    return props;
  }
  var extendStatics = Object.setPrototypeOf || {__proto__: []} instanceof Array && function(d, b) {
    ...
    for (var p in b)
      ...
      MockSchemaRegistry = function() {
        function MockSchemaRegistry(existingProperties, attrPropMapping, existingElements, invalidProperties, invalidAttributes) {
          this.existingProperties = existingProperties, this.attrPropMapping = attrPropMapping, this.existingElements = existingElements, this.invalidProperties = invalidProperties, this.invalidAttributes = invalidAttributes;
        }
        return MockSchemaRegistry.prototype.hasProperty = function(tagName, property, schemas) {
          ...
          return this.invalidProperties.indexOf(name) > -1 ? {
            error: !0,
          ...
          return this.invalidAttributes.indexOf(name) > -1 ? {
            error: !0,
      ...
      MockDirectiveResolver = function(_super) {
        function MockDirectiveResolver(_injector, reflector) {
          ...
          return _this._injector = _injector, _this._directives = new Map, _this._providerOverrides = new Map, _this._viewProviderOverrides = new Map, _this._views = new Map, _this._inlineTemplates = new Map, _this;
        }
        return __extends(MockDirectiveResolver, _super), Object.defineProperty(MockDirectiveResolver.prototype, "_compiler", {
          get: function() {
          ...
          if (metadata || (metadata = _super.prototype.resolve.call(this, type, throwIfNotFound)), !metadata)
            return null;
          var providerOverrides = this._providerOverrides.get(type),
              viewProviderOverrides = this._viewProviderOverrides.get(type),
              providers = metadata.providers;
          if (null != providerOverrides) {
          ...
          if (metadata instanceof _angular_core.Component) {
            ...
            var view = this._views.get(type) || metadata,
                ...
                inlineTemplate = this._inlineTemplates.get(type);
            return inlineTemplate ? templateUrl = void 0 : inlineTemplate = view.template, new _angular_core.Component({
          ...
          this._directives.set(type, metadata), this._clearCacheFor(type);
        }, MockDirectiveResolver.prototype.setProvidersOverride = function(type, providers) {
          this._providerOverrides.set(type, providers), this._clearCacheFor(type);
        }, MockDirectiveResolver.prototype.setViewProvidersOverride = function(type, viewProviders) {
          this._viewProviderOverrides.set(type, viewProviders), this._clearCacheFor(type);
        }, MockDirectiveResolver.prototype.setView = function(component, view) {
          this._views.set(component, view), this._clearCacheFor(component);
        }, MockDirectiveResolver.prototype.setInlineTemplate = function(component, template) {
          this._inlineTemplates.set(component, template), this._clearCacheFor(component);
        }, MockDirectiveResolver;
      }(_angular_compiler.DirectiveResolver);
  MockDirectiveResolver.decorators = [{type: _angular_core.Injectable}], MockDirectiveResolver.ctorParameters = function() {
  ...
  var MockNgModuleResolver = function(_super) {
    function MockNgModuleResolver(_injector, reflector) {
      var _this = _super.call(this, reflector) || this;
      return _this._injector = _injector, _this._ngModules = new Map, _this;
    }
    return __extends(MockNgModuleResolver, _super), MockNgModuleResolver.prototype.setNgModule = function(type, metadata) {
      this._ngModules.set(type, metadata), this._clearCacheFor(type);
    }, MockNgModuleResolver.prototype.resolve = function(type, throwIfNotFound) {
      return void 0 === throwIfNotFound && (throwIfNotFound = !0), this._ngModules.get(type) || _super.prototype.resolve.call(this, type, throwIfNotFound);
  ...
  MockNgModuleResolver.decorators = [{type: _angular_core.Injectable}], MockNgModuleResolver.ctorParameters = function() {
    return [{type: _angular_core.Injector}, {type: _angular_compiler.CompileReflector}];
  };
  var MockPipeResolver = function(_super) {
    function MockPipeResolver(_injector, refector) {
      var _this = _super.call(this, refector) || this;
      return _this._injector = _injector, _this._pipes = new Map, _this;
    }
    return __extends(MockPipeResolver, _super), Object.defineProperty(MockPipeResolver.prototype, "_compiler", {
      get: function() {
        return this._injector.get(_angular_core.Compiler);
      },
      enumerable: !0,
      ...
      this._pipes.set(type, metadata), this._clearCacheFor(type);
    }, MockPipeResolver.prototype.resolve = function(type, throwIfNotFound) {
      void 0 === throwIfNotFound && (throwIfNotFound = !0);
      var metadata = this._pipes.get(type);
      return metadata || (metadata = _super.prototype.resolve.call(this, type, throwIfNotFound)), metadata;
    }, MockPipeResolver;
  }(_angular_compiler.PipeResolver);
  MockPipeResolver.decorators = [{type: _angular_core.Injectable}], MockPipeResolver.ctorParameters = function() {
    return [{type: _angular_core.Injector}, {type: _angular_compiler.CompileReflector}];
  };
  var _nextReferenceId = 0,
      MetadataOverrider = function() {
        function MetadataOverrider() {
          this._references = new Map;
        }
        return MetadataOverrider.prototype.overrideMetadata = function(metadataClass, oldMetadata, override) {
          var props = {};
          if (oldMetadata && _valueProps(oldMetadata).forEach(function(prop) {
          ...
          return override.remove && removeMetadata(props, override.remove, this._references), override.add && addMetadata(props, override.add), new metadataClass(props);
        }, MetadataOverrider;
      }(),
      TestingCompilerFactoryImpl = function() {
        function TestingCompilerFactoryImpl(_compilerFactory) {
          this._compilerFactory = _compilerFactory;
        }
        return TestingCompilerFactoryImpl.prototype.createTestingCompiler = function(options) {
  ...
  TestingCompilerFactoryImpl.decorators = [{type: _angular_core.Injectable}], TestingCompilerFactoryImpl.ctorParameters = function() {
  ...
  var TestingCompilerImpl = function() {
    function TestingCompilerImpl(_compiler, _directiveResolver, _pipeResolver, _moduleResolver, _metadataResolver) {
      this._compiler = _compiler, this._directiveResolver = _directiveResolver, this._pipeResolver = _pipeResolver, this._moduleResolver = _moduleResolver, this._metadataResolver = _metadataResolver, this._overrider = new MetadataOverrider;
    }
    return Object.defineProperty(TestingCompilerImpl.prototype, "injector", {
      get: function() {
      ...
      var oldMetadata = this._moduleResolver.resolve(ngModule, !1);
      ...
      var oldMetadata = this._directiveResolver.resolve(directive, !1);
      ...
      var oldMetadata = this._directiveResolver.resolve(component, !1);
      ...
      var oldMetadata = this._pipeResolver.resolve(pipe, !1);
      ...
      platformCoreDynamicTesting = _angular_core.createPlatformFactory(_angular_compiler.platformCoreDynamic, "coreDynamicTesting", [{
