...
(function(process) {
  !function(global, factory) {
    ...
    function validateNode(route, fullPath) {
      ...
      if (void 0 === route.redirectTo && !route.component && !route.children && !route.loadChildren)
      ...
      if ("" === route.path && void 0 !== route.redirectTo && void 0 === route.pathMatch) {
        throw new Error("Invalid configuration of route '{path: \"" + fullPath + '", redirectTo: "' + route.redirectTo + "\"}': please provide 'pathMatch'. The default value of 'pathMatch' is 'prefix', but often the intent is to use 'full'.");
      }
      if (void 0 !== route.pathMatch && "full" !== route.pathMatch && "prefix" !== route.pathMatch)
    ...
    function forEach(map$$1, callback) {
      for (var prop in map$$1)
        map$$1.hasOwnProperty(prop) && callback(map$$1[prop], prop);
    }
    function waitForMap(obj, fn) {
      ...
      var concat$ = rxjs_operator_concatAll.concatAll.call(rxjs_observable_of.of.apply(void 0, waitHead.concat(waitTail))),
          last$ = rxjs_operator_last.last.call(concat$);
    ...
    function runCanLoadGuard(moduleInjector, route) {
      ...
      return canLoad && 0 !== canLoad.length ? andObservables(rxjs_operator_map.map.call(rxjs_observable_from.from(canLoad), function(injectionToken) {
        var guard = moduleInjector.get(injectionToken);
        return wrapIntoObservable(guard.canLoad ? guard.canLoad(route) : guard(route));
      })) : rxjs_observable_of.of(!0);
    }
    function match(segmentGroup, route, segments) {
      if ("" === route.path)
        return "full" === route.pathMatch && (segmentGroup.hasChildren() || segments.length > 0) ? {
          matched: !1,
          consumedSegments: [],
          lastChild: 0,
    ...
    function createEmptyStateSnapshot(urlTree, rootComponent) {
      var emptyParams = {},
          emptyData = {},
          emptyQueryParams = {},
          activated = new ActivatedRouteSnapshot([], emptyParams, emptyQueryParams, "", emptyData, PRIMARY_OUTLET, rootComponent, null, urlTree.root, -1, {});
    ...
    function inheritedParamsDataResolve(route, paramsInheritanceStrategy) {
      ...
      if ("always" !== paramsInheritanceStrategy)
        for (inheritingStartingFrom = pathFromRoot.length - 1; inheritingStartingFrom >= 1; ) {
          var current = pathFromRoot[inheritingStartingFrom],
              parent_1 = pathFromRoot[inheritingStartingFrom - 1];
          if (current.routeConfig && "" === current.routeConfig.path)
            inheritingStartingFrom--;
          else {
            if (parent_1.component)
              break;
            inheritingStartingFrom--;
          }
        }
    ...
    function advanceActivatedRoute(route) {
      if (route.snapshot) {
        ...
        route.snapshot = nextSnapshot, shallowEqual(currentSnapshot.queryParams, nextSnapshot.queryParams) || route.queryParams.next(nextSnapshot.queryParams), currentSnapshot.fragment !== nextSnapshot.fragment && route.fragment.next(nextSnapshot.fragment), shallowEqual(currentSnapshot.params, nextSnapshot.params) || route.params.next(nextSnapshot.params), shallowEqualArrays(currentSnapshot.url, nextSnapshot.url) || route.url.next(nextSnapshot.url), shallowEqual(currentSnapshot.data, nextSnapshot.data) || route.data.next(nextSnapshot.data);
      } else
        route.snapshot = route._futureSnapshot, route.data.next(route._futureSnapshot.data);
    }
    ...
    function createRouterState(routeReuseStrategy, curr, prevState) {
      var root = createNode(routeReuseStrategy, curr._root, prevState ? prevState._root : void 0);
    ...
    function createNode(routeReuseStrategy, curr, prevState) {
      ...
      if (routeReuseStrategy.retrieve(curr.value)) {
        ...
        return setFutureSnapshotsOfActivatedRoutes(curr, tree), tree;
      }
    ...
    function computeNavigation(commands) {
      if ("string" == typeof commands[0] && 1 === commands.length && "/" === commands[0])
    ...
    function findStartingPosition(nav, tree, route) {
      if (nav.isAbsolute)
        return new Position(tree.root, !0, 0);
      if (-1 === route.snapshot._lastPathIndex)
        return new Position(route.snapshot._urlSegment, !0, 0);
    ...
    function getOutlets(commands) {
      return "object" != typeof commands[0] ? (_a = {}, _a[PRIMARY_OUTLET] = commands, _a) : void 0 === commands[0].outlets ? (_b = {}, _b[PRIMARY_OUTLET] = commands, _b) : commands[0].outlets;
      var _a,
          _b;
    }
    function updateSegmentGroup(segmentGroup, startIndex, commands) {
      if (segmentGroup || (segmentGroup = new UrlSegmentGroup([], {})), 0 === segmentGroup.segments.length && segmentGroup.hasChildren())
        return updateSegmentGroupChildren(segmentGroup, startIndex, commands);
      var m = prefixedWith(segmentGroup, startIndex, commands),
          slicedCommands = commands.slice(m.commandIndex);
      if (m.match && m.pathIndex < segmentGroup.segments.length) {
        var g = new UrlSegmentGroup(segmentGroup.segments.slice(0, m.pathIndex), {});
        return g.children[PRIMARY_OUTLET] = new UrlSegmentGroup(segmentGroup.segments.slice(m.pathIndex), segmentGroup.children), updateSegmentGroupChildren(g, 0, slicedCommands);
      }
    ...
    function prefixedWith(segmentGroup, startIndex, commands) {
      for (var currentCommandIndex = 0,
        ...
        if (currentPathIndex > 0 && void 0 === curr)
          break;
    ...
    function createNewSegmentGroup(segmentGroup, startIndex, commands) {
      for (var paths = segmentGroup.segments.slice(0, startIndex),
        ...
        if (0 === i && isMatrixParams(commands[0])) {
          ...
          paths.push(new UrlSegment(p.path, commands[0])), i++;
        } else {
          var curr = getPath(commands[i]),
              next = i < commands.length - 1 ? commands[i + 1] : null;
          curr && next && isMatrixParams(next) ? (paths.push(new UrlSegment(curr, stringify(next))), i += 2) : (paths.push(new UrlSegment(curr, {})), i++);
        }
      }
    ...
    function recognize(rootComponentType, config, urlTree, url, paramsInheritanceStrategy) {
      return void 0 === paramsInheritanceStrategy && (paramsInheritanceStrategy = "emptyOnly"), new Recognizer(rootComponentType, config, urlTree, url, paramsInheritanceStrategy).recognize();
    }
    ...
    function match$1(segmentGroup, route, segments) {
      if ("" === route.path) {
        if ("full" === route.pathMatch && (segmentGroup.hasChildren() || segments.length > 0))
          throw new NoMatch$1;
        return {
          consumedSegments: [],
          lastChild: 0,
    ...
    function getSourceSegmentGroup(segmentGroup) {
      for (var s = segmentGroup; s._sourceSegment; )
        s = s._sourceSegment;
      return s;
    }
    function getPathIndexShift(segmentGroup) {
      for (var s = segmentGroup,
          res = s._segmentIndexShift ? s._segmentIndexShift : 0; s._sourceSegment; )
        s = s._sourceSegment, res += s._segmentIndexShift ? s._segmentIndexShift : 0;
      return res - 1;
    }
    ...
    function addEmptyPathsToChildrenIfNeeded(segmentGroup, slicedSegments, routes, children) {
      for (var res = {},
        ...
        if (emptyPathMatch(segmentGroup, slicedSegments, r) && !children[getOutlet$1(r)]) {
          ...
          s._sourceSegment = segmentGroup, s._segmentIndexShift = segmentGroup.segments.length, res[getOutlet$1(r)] = s;
        }
      }
      return __assign({}, children, res);
    }
    function createChildrenForEmptyPaths(segmentGroup, consumedSegments, routes, primarySegment) {
      var res = {};
      res[PRIMARY_OUTLET] = primarySegment, primarySegment._sourceSegment = segmentGroup, primarySegment._segmentIndexShift = consumedSegments.length;
      for (var _i = 0,
          routes_2 = routes; _i < routes_2.length; _i++) {
        var r = routes_2[_i];
        if ("" === r.path && getOutlet$1(r) !== PRIMARY_OUTLET) {
          var s = new UrlSegmentGroup([], {});
          s._sourceSegment = segmentGroup, s._segmentIndexShift = consumedSegments.length, res[getOutlet$1(r)] = s;
        }
      }
      return res;
    }
    ...
    function setupRouter(ref, urlSerializer, contexts, location, injector, loader, compiler, config, opts, urlHandlingStrategy, routeReuseStrategy) {
      ...
      if (urlHandlingStrategy && (router.urlHandlingStrategy = urlHandlingStrategy), routeReuseStrategy && (router.routeReuseStrategy = routeReuseStrategy), opts.errorHandler && (router.errorHandler = opts.errorHandler), opts.enableTracing) {
      ...
      return opts.onSameUrlNavigation && (router.onSameUrlNavigation = opts.onSameUrlNavigation), opts.paramsInheritanceStrategy && (router.paramsInheritanceStrategy = opts.paramsInheritanceStrategy), router;
    }
    ...
    var extendStatics = Object.setPrototypeOf || {__proto__: []} instanceof Array && function(d, b) {
      ...
      for (var p in b)
        ...
        RouterEvent = function() {
          function RouterEvent(id, url) {
            this.id = id, this.url = url;
          }
          return RouterEvent;
        }(),
        NavigationStart = function(_super) {
          function NavigationStart() {
            return null !== _super && _super.apply(this, arguments) || this;
          }
          return __extends(NavigationStart, _super), NavigationStart.prototype.toString = function() {
            return "NavigationStart(id: " + this.id + ", url: '" + this.url + "')";
          }, NavigationStart;
        }(RouterEvent),
        NavigationEnd = function(_super) {
          function NavigationEnd(id, url, urlAfterRedirects) {
            var _this = _super.call(this, id, url) || this;
            return _this.urlAfterRedirects = urlAfterRedirects, _this;
          }
          return __extends(NavigationEnd, _super), NavigationEnd.prototype.toString = function() {
            return "NavigationEnd(id: " + this.id + ", url: '" + this.url + "', urlAfterRedirects: '" + this.urlAfterRedirects + "')";
          }, NavigationEnd;
        }(RouterEvent),
        NavigationCancel = function(_super) {
          function NavigationCancel(id, url, reason) {
            var _this = _super.call(this, id, url) || this;
            return _this.reason = reason, _this;
          }
          return __extends(NavigationCancel, _super), NavigationCancel.prototype.toString = function() {
            return "NavigationCancel(id: " + this.id + ", url: '" + this.url + "')";
          }, NavigationCancel;
        }(RouterEvent),
        NavigationError = function(_super) {
          function NavigationError(id, url, error) {
            var _this = _super.call(this, id, url) || this;
            return _this.error = error, _this;
          }
          return __extends(NavigationError, _super), NavigationError.prototype.toString = function() {
            return "NavigationError(id: " + this.id + ", url: '" + this.url + "', error: " + this.error + ")";
          }, NavigationError;
        }(RouterEvent),
        RoutesRecognized = function(_super) {
          function RoutesRecognized(id, url, urlAfterRedirects, state) {
            var _this = _super.call(this, id, url) || this;
            return _this.urlAfterRedirects = urlAfterRedirects, _this.state = state, _this;
          }
          return __extends(RoutesRecognized, _super), RoutesRecognized.prototype.toString = function() {
            return "RoutesRecognized(id: " + this.id + ", url: '" + this.url + "', urlAfterRedirects: '" + this.urlAfterRedirects + "', state: " + this.state + ")";
          }, RoutesRecognized;
        }(RouterEvent),
        GuardsCheckStart = function(_super) {
        ...
        UrlSegmentGroup = function() {
          function UrlSegmentGroup(segments, children) {
            var _this = this;
            this.segments = segments, this.children = children, this.parent = null, forEach(children, function(v, k) {
        ...
        DefaultUrlSerializer = function() {
          ...
          return DefaultUrlSerializer.prototype.parse = function(url) {
            var p = new UrlParser(url);
            return new UrlTree(p.parseRootSegment(), p.parseQueryParams(), p.parseFragment());
          }, DefaultUrlSerializer.prototype.serialize = function(tree) {
            return "/" + serializeSegment(tree.root, !0) + serializeQueryParams(tree.queryParams) + ("string" == typeof tree.fragment ? "#" + encodeURI(tree.fragment) : "");
        ...
        UrlParser = function() {
          ...
          return UrlParser.prototype.parseRootSegment = function() {
            ...
            var segments = [];
            for (this.peekStartsWith("(") || segments.push(this.parseSegment()); this.peekStartsWith("/") && !this.peekStartsWith("//") && !this.peekStartsWith("/("); )
              this.capture("/"), segments.push(this.parseSegment());
            var children = {};
            this.peekStartsWith("/(") && (this.capture("/"), children = this.parseParens(!0));
            var res = {};
            return this.peekStartsWith("(") && (res = this.parseParens(!1)), (segments.length > 0 || Object.keys(children).length > 0) && (res[PRIMARY_OUTLET] = new UrlSegmentGroup(segments, children)), res;
            ...
            for (var params = {}; this.consumeOptional(";"); )
            ...
            if (key) {
              ...
              var decodedKey = decode(key),
                  decodedVal = decode(value);
              if (params.hasOwnProperty(decodedKey)) {
                var currentVal = params[decodedKey];
                Array.isArray(currentVal) || (currentVal = [currentVal], params[decodedKey] = currentVal), currentVal.push(decodedVal);
              } else
            ...
            for (this.capture("("); !this.consumeOptional(")") && this.remaining.length > 0; ) {
              ...
              path.indexOf(":") > -1 ? (outletName = path.substr(0, path.indexOf(":")), this.capture(outletName), this.capture(":")) : allowPrimary && (outletName = PRIMARY_OUTLET);
              var children = this.parseChildren();
              segments[outletName] = 1 === Object.keys(children).length ? children[PRIMARY_OUTLET] : new UrlSegmentGroup([], children), this.consumeOptional("//");
            }
            return segments;
        ...
        ApplyRedirects = function() {
          ...
          return ApplyRedirects.prototype.apply = function() {
            ...
            return rxjs_operator_catch._catch.call(first$, function(e, _) {
              if (e instanceof rxjs_util_EmptyError.EmptyError || "EmptyError" === e.name) {
            ...
            var _this = this,
                newTree = this.applyRedirectCommands([], route.redirectTo, {});
            return route.redirectTo.startsWith("/") ? absoluteRedirect(newTree) : rxjs_operator_mergeMap.mergeMap.call(this.lineralizeSegments(route, newTree), function(newSegments) {
            ...
            var _this = this;
            if ("**" === route.path)
              return route.loadChildren ? rxjs_operator_map.map.call(this.configLoader.load(ngModule.injector, route), function(cfg) {
                return route._loadedConfig = cfg, new UrlSegmentGroup(segments, {});
              }) : rxjs_observable_of.of(new UrlSegmentGroup(segments, {}));
            ...
            return rxjs_operator_mergeMap.mergeMap.call(childConfig$, function(routerConfig) {
              ...
              var expanded$ = _this.expandSegment(childModule, segmentGroup, childConfig, slicedSegments, PRIMARY_OUTLET, !0);
            ...
            var _this = this;
            return route.children ? rxjs_observable_of.of(new LoadedRouterConfig(route.children, ngModule)) : route.loadChildren ? void 0 !== route._loadedConfig ? rxjs_observable_of.of(route._loadedConfig) : rxjs_operator_mergeMap.mergeMap.call(runCanLoadGuard(ngModule.injector, route), function(shouldLoad) {
        ...
        ActivatedRouteSnapshot = function() {
          function ActivatedRouteSnapshot(url, params, queryParams, fragment, data, outlet, component, routeConfig, urlSegment, lastPathIndex, resolve) {
            this.url = url, this.params = params, this.queryParams = queryParams, this.fragment = fragment, this.data = data, this.outlet = outlet, this.component = component, this.routeConfig = routeConfig, this._urlSegment = urlSegment, this._lastPathIndex = lastPathIndex, this._resolve = resolve;
          }
        ...
        Navigation = function() {
          function Navigation(isAbsolute, numberOfDoubleDots, commands) {
            if (this.isAbsolute = isAbsolute, this.numberOfDoubleDots = numberOfDoubleDots, this.commands = commands, isAbsolute && commands.length > 0 && isMatrixParams(commands[0]))
        ...
        PreActivation = function() {
          function PreActivation(future, curr, moduleInjector, forwardEvent) {
            this.future = future, this.curr = curr, this.moduleInjector = moduleInjector, this.forwardEvent = forwardEvent, this.canActivateChecks = [], this.canDeactivateChecks = [];
          }
          return PreActivation.prototype.initialize = function(parentContexts) {
            ...
            if (!this.isDeactivating() && !this.isActivating())
              return rxjs_observable_of.of(!0);
            ...
            var _this = this,
                prevChildren = nodeChildrenAsMap(currNode);
            futureNode.children.forEach(function(c) {
              _this.setupRouteGuards(c, prevChildren[c.value.outlet], contexts, futurePath.concat([c.value])), delete prevChildren[c.value.outlet];
            ...
            var future = futureNode.value,
                ...
                context = parentContexts ? parentContexts.getContext(futureNode.value.outlet) : null;
            if (curr && future.routeConfig === curr.routeConfig) {
              ...
              if (shouldRunGuardsAndResolvers ? this.canActivateChecks.push(new CanActivate(futurePath)) : (future.data = curr.data, future._resolvedData = curr._resolvedData), future.component ? this.setupChildRouteGuards(futureNode, currNode, context ? context.children : null, futurePath) : this.setupChildRouteGuards(futureNode, currNode, parentContexts, futurePath), shouldRunGuardsAndResolvers) {
                var outlet = context.outlet;
                this.canDeactivateChecks.push(new CanDeactivate(outlet.component, curr));
              }
            } else
              curr && this.deactivateRouteAndItsChildren(currNode, context), this.canActivateChecks.push(new CanActivate(futurePath)), future.component ? this.setupChildRouteGuards(futureNode, null, context ? context.children : null, futurePath) : this.setupChildRouteGuards(futureNode, null, parentContexts, futurePath);
            ...
            forEach(children, function(node, childName) {
              r.component ? context ? _this.deactivateRouteAndItsChildren(node, context.children.getContext(childName)) : _this.deactivateRouteAndItsChildren(node, null) : _this.deactivateRouteAndItsChildren(node, context);
            }), r.component && context && context.outlet && context.outlet.isActivated ? this.canDeactivateChecks.push(new CanDeactivate(context.outlet.component, r)) : this.canDeactivateChecks.push(new CanDeactivate(null, r));
            ...
            return null !== snapshot && this.forwardEvent && this.forwardEvent(new ActivationStart(snapshot)), rxjs_observable_of.of(!0);
          }, PreActivation.prototype.fireChildActivationStart = function(snapshot) {
            return null !== snapshot && this.forwardEvent && this.forwardEvent(new ChildActivationStart(snapshot)), rxjs_observable_of.of(!0);
          }, PreActivation.prototype.runCanActivate = function(future) {
            var _this = this,
                canActivate = future.routeConfig ? future.routeConfig.canActivate : null;
            return canActivate && 0 !== canActivate.length ? andObservables(rxjs_operator_map.map.call(rxjs_observable_from.from(canActivate), function(c) {
              var observable,
                  guard = _this.getToken(c, future);
              return observable = wrapIntoObservable(guard.canActivate ? guard.canActivate(future, _this.future) : guard(future, _this.future)), rxjs_operator_first.first.call(observable);
            })) : rxjs_observable_of.of(!0);
            ...
            return andObservables(rxjs_operator_map.map.call(rxjs_observable_from.from(canActivateChildGuards), function(d) {
              return andObservables(rxjs_operator_map.map.call(rxjs_observable_from.from(d.guards), function(c) {
                var observable,
                    guard = _this.getToken(c, d.node);
                return observable = wrapIntoObservable(guard.canActivateChild ? guard.canActivateChild(future, _this.future) : guard(future, _this.future)), rxjs_operator_first.first.call(observable);
              }));
            }));
            ...
            var canDeactivate$ = rxjs_operator_mergeMap.mergeMap.call(rxjs_observable_from.from(canDeactivate), function(c) {
              var observable,
                  guard = _this.getToken(c, curr);
              return observable = wrapIntoObservable(guard.canDeactivate ? guard.canDeactivate(component, curr, _this.curr, _this.future) : guard(component, curr, _this.curr, _this.future)), rxjs_operator_first.first.call(observable);
            });
            ...
            return rxjs_operator_map.map.call(this.resolveNode(resolve, future), function(resolvedData) {
              return future._resolvedData = resolvedData, future.data = __assign({}, future.data, inheritedParamsDataResolve(future, paramsInheritanceStrategy).resolve), null;
            });
            ...
            var data = {},
                runningResolvers$ = rxjs_operator_mergeMap.mergeMap.call(rxjs_observable_from.from(keys), function(key) {
            ...
            return wrapIntoObservable(resolver.resolve ? resolver.resolve(future, this.future) : resolver(future, this.future));
          }, PreActivation.prototype.getToken = function(token, snapshot) {
            var config = closestLoadedConfig(snapshot);
            return (config ? config.module.injector : this.moduleInjector).get(token);
          }, PreActivation;
        }(),
        NoMatch$1 = function() {
          function NoMatch() {}
        ...
        Recognizer = function() {
          function Recognizer(rootComponentType, config, urlTree, url, paramsInheritanceStrategy) {
            this.rootComponentType = rootComponentType, this.config = config, this.urlTree = urlTree, this.url = url, this.paramsInheritanceStrategy = paramsInheritanceStrategy;
          }
          return Recognizer.prototype.recognize = function() {
            try {
              ...
              return this.inheritParamsAndData(routeState._root), rxjs_observable_of.of(routeState);
            } catch (e) {
              return new rxjs_Observable.Observable(function(obs) {
                return obs.error(e);
              });
        ...
        RouterConfigLoader = function() {
          ...
          return RouterConfigLoader.prototype.load = function(parentInjector, route) {
            ...
            var _this = this;
            return "string" == typeof loadChildren ? rxjs_observable_fromPromise.fromPromise(this.loader.load(loadChildren)) : rxjs_operator_mergeMap.mergeMap.call(wrapIntoObservable(loadChildren()), function(t) {
        ...
        Router = function() {
          function Router(rootComponentType, urlSerializer, rootContexts, location, injector, loader, compiler, config) {
            var _this = this;
            this.rootComponentType = rootComponentType, this.urlSerializer = urlSerializer, this.rootContexts = rootContexts, this.location = location, this.config = config, this.navigations = new rxjs_BehaviorSubject.BehaviorSubject(null), this.navigationId = 0, this.events = new rxjs_Subject.Subject, this.errorHandler = defaultErrorHandler, this.navigated = !1, this.hooks = {
              beforePreactivation: defaultRouterHook,
              afterPreactivation: defaultRouterHook
            }, this.urlHandlingStrategy = new DefaultUrlHandlingStrategy, this.routeReuseStrategy = new DefaultRouteReuseStrategy, this.onSameUrlNavigation = "ignore", this.paramsInheritanceStrategy = "emptyOnly";
          ...
          return Router.prototype.resetRootComponentType = function(rootComponentType) {
            ...
            var _this = this;
            this.locationSubscription || (this.locationSubscription = this.location.subscribe(Zone.current.wrap(function(change) {
              var rawUrlTree = _this.urlSerializer.parse(change.url),
                  source = "popstate" === change.type ? "popstate" : "hashchange";
              setTimeout(function() {
            ...
            validateConfig(config), this.config = config, this.navigated = !1;
            ...
            _angular_core.isDevMode() && preserveQueryParams && console && console.warn && console.warn("preserveQueryParams is deprecated, use queryParamsHandling instead.");
            ...
            return null !== q && (q = this.removeEmptyProps(q)), createUrlTree(a, this.currentUrlTree, commands, q, f);
          }, Router.prototype.navigateByUrl = function(url, extras) {
            void 0 === extras && (extras = {skipLocationChange: !1});
            var urlTree = url instanceof UrlTree ? url : this.parseUrl(url),
                mergedTree = this.urlHandlingStrategy.merge(urlTree, this.rawUrlTree);
            return this.scheduleNavigation(mergedTree, "imperative", extras);
          }, Router.prototype.navigate = function(commands, extras) {
            return void 0 === extras && (extras = {skipLocationChange: !1}), validateCommands(commands), this.navigateByUrl(this.createUrlTree(commands, extras), extras);
            ...
            return Object.keys(params).reduce(function(result, key) {
              ...
              return null !== value && void 0 !== value && (result[key] = value), result;
            }, {});
            ...
            if (lastNavigation && "imperative" !== source && "imperative" === lastNavigation.source && lastNavigation.rawUrl.toString() === rawUrl.toString())
              return Promise.resolve(!0);
            if (lastNavigation && "hashchange" == source && "popstate" === lastNavigation.source && lastNavigation.rawUrl.toString() === rawUrl.toString())
              return Promise.resolve(!0);
            if (lastNavigation && "popstate" == source && "hashchange" === lastNavigation.source && lastNavigation.rawUrl.toString() === rawUrl.toString())
              return Promise.resolve(!0);
            var resolve = null,
                reject = null,
            ...
            ("reload" === this.onSameUrlNavigation || urlTransition) && this.urlHandlingStrategy.shouldProcessUrl(rawUrl) ? (this.events.next(new NavigationStart(id, this.serializeUrl(url))), Promise.resolve().then(function(_) {
              return _this.runNavigate(url, rawUrl, !!extras.skipLocationChange, !!extras.replaceUrl, id, null);
            }).then(resolve, reject)) : urlTransition && this.rawUrlTree && this.urlHandlingStrategy.shouldProcessUrl(this.rawUrlTree) ? (this.events.next(new NavigationStart(id, this.serializeUrl(url))), Promise.resolve().then(function(_) {
            ...
            var _this = this;
            return id !== this.navigationId ? (this.events.next(new NavigationCancel(id, this.serializeUrl(url), "Navigation ID " + id + " is not equal to the current navigation id " + this.navigationId)), Promise.resolve(!1)) : new Promise(function(resolvePromise, rejectPromise) {
              var urlAndSnapshot$;
              if (precreatedState)
                ...
                var moduleInjector = _this.ngModule.injector,
                    redirectsApplied$ = applyRedirects(moduleInjector, _this.configLoader, _this.urlSerializer, url, _this.config);
                urlAndSnapshot$ = rxjs_operator_mergeMap.mergeMap.call(redirectsApplied$, function(appliedUrl) {
                  return rxjs_operator_map.map.call(recognize(_this.rootComponentType, _this.config, appliedUrl, _this.serializeUrl(appliedUrl), _this.paramsInheritanceStrategy), function(snapshot) {
                    return _this.events.next(new RoutesRecognized(id, _this.serializeUrl(url), _this.serializeUrl(appliedUrl), snapshot)), {
              ...
              var preActivation,
                  ...
                  preactivationSetup$ = rxjs_operator_map.map.call(beforePreactivationDone$, function(_a) {
                    var appliedUrl = _a.appliedUrl,
                        snapshot = _a.snapshot,
                        moduleInjector = _this.ngModule.injector;
                    return preActivation = new PreActivation(snapshot, _this.routerState.snapshot, moduleInjector, function(evt) {
                  ...
                  preactivationCheckGuards$ = rxjs_operator_mergeMap.mergeMap.call(preactivationSetup$, function(_a) {
                    var appliedUrl = _a.appliedUrl,
                        snapshot = _a.snapshot;
                    return _this.navigationId !== id ? rxjs_observable_of.of(!1) : (_this.triggerEvent(new GuardsCheckStart(id, _this.serializeUrl(url), appliedUrl, snapshot)), rxjs_operator_map.map.call(preActivation.checkGuards(), function(shouldActivate) {
                      return _this.triggerEvent(new GuardsCheckEnd(id, _this.serializeUrl(url), appliedUrl, snapshot, shouldActivate)), {
                        appliedUrl: appliedUrl,
                        snapshot: snapshot,
                  ...
                  preactivationResolveData$ = rxjs_operator_mergeMap.mergeMap.call(preactivationCheckGuards$, function(p) {
                    return _this.navigationId !== id ? rxjs_observable_of.of(!1) : p.shouldActivate && preActivation.isActivating() ? (_this.triggerEvent(new ResolveStart(id, _this.serializeUrl(url), p.appliedUrl, p.snapshot)), rxjs_operator_map.map.call(preActivation.resolveData(_this.paramsInheritanceStrategy), function() {
                      return _this.triggerEvent(new ResolveEnd(id, _this.serializeUrl(url), p.appliedUrl, p.snapshot)), p;
                  ...
                  routerState$ = rxjs_operator_map.map.call(preactivationDone$, function(_a) {
                    ...
                    if (shouldActivate) {
                      return {
                        appliedUrl: appliedUrl,
                        state: createRouterState(_this.routeReuseStrategy, snapshot, _this.routerState),
                        shouldActivate: shouldActivate
                      };
                    }
                    return {
                  ...
                  storedState = _this.routerState,
                  storedUrl = _this.currentUrlTree;
              routerState$.forEach(function(_a) {
                var appliedUrl = _a.appliedUrl,
                    state = _a.state;
                if (!_a.shouldActivate || id !== _this.navigationId)
                  return void(navigationIsSuccessful = !1);
                if (_this.currentUrlTree = appliedUrl, _this.rawUrlTree = _this.urlHandlingStrategy.merge(_this.currentUrlTree, rawUrl), _this.routerState = state, !skipLocationChange) {
                  var path = _this.urlSerializer.serialize(_this.rawUrlTree);
                  _this.location.isCurrentPathEqualTo(path) || replaceUrl ? _this.location.replaceState(path) : _this.location.go(path);
                }
                new ActivateRoutes(_this.routeReuseStrategy, state, storedState, function(evt) {
                  return _this.triggerEvent(evt);
                }).activate(_this.rootContexts), navigationIsSuccessful = !0;
              }).then(function() {
                navigationIsSuccessful ? (_this.navigated = !0, _this.events.next(new NavigationEnd(id, _this.serializeUrl(url), _this.serializeUrl(_this.currentUrlTree))), resolvePromise(!0)) : (_this.resetUrlToCurrentUrlTree(), _this.events.next(new NavigationCancel(id, _this.serializeUrl(url), "")), resolvePromise(!1));
              }, function(e) {
                if (isNavigationCancelingError(e))
                  _this.navigated = !0, _this.resetStateAndUrl(storedState, storedUrl, rawUrl), _this.events.next(new NavigationCancel(id, _this.serializeUrl(url), e.message)), resolvePromise(!1);
                else {
                  _this.resetStateAndUrl(storedState, storedUrl, rawUrl), _this.events.next(new NavigationError(id, _this.serializeUrl(url), e));
                  try {
            ...
            this.routerState = storedState, this.currentUrlTree = storedUrl, this.rawUrlTree = this.urlHandlingStrategy.merge(this.currentUrlTree, rawUrl), this.resetUrlToCurrentUrlTree();
        ...
        ActivateRoutes = function() {
          function ActivateRoutes(routeReuseStrategy, futureState, currState, forwardEvent) {
            this.routeReuseStrategy = routeReuseStrategy, this.futureState = futureState, this.currState = currState, this.forwardEvent = forwardEvent;
          }
          return ActivateRoutes.prototype.activate = function(parentContexts) {
            var futureRoot = this.futureState._root,
                currRoot = this.currState ? this.currState._root : null;
            this.deactivateChildRoutes(futureRoot, currRoot, parentContexts), advanceActivatedRoute(this.futureState.root), this.activateChildRoutes(futureRoot, currRoot, parentContexts);
            ...
            if (context) {
              var children = nodeChildrenAsMap(route),
                  contexts_1 = route.value.component ? context.children : parentContexts;
              forEach(children, function(v, k) {
                return _this.deactivateRouteAndItsChildren(v, contexts_1);
              }), context.outlet && (context.outlet.deactivate(), context.children.onOutletDeactivated());
            }
            ...
            if (advanceActivatedRoute(future), future === curr)
              ...
              if (this.routeReuseStrategy.shouldAttach(future.snapshot)) {
                ...
                this.routeReuseStrategy.store(future.snapshot, null), context.children.onOutletReAttached(stored.contexts), context.attachRef = stored.componentRef, context.route = stored.route.value, context.outlet && context.outlet.attach(stored.componentRef, stored.route.value), advanceActivatedRouteNodeAndItsChildren(stored.route);
              } else {
                var config = parentLoadedConfig(future.snapshot),
                    cmpFactoryResolver = config ? config.module.componentFactoryResolver : null;
                context.route = future, context.resolver = cmpFactoryResolver, context.outlet && context.outlet.activateWith(future, cmpFactoryResolver), this.activateChildRoutes(futureNode, null, context.children);
              }
            } else
        ...
        RouterLink = function() {
          function RouterLink(router, route, tabIndex, renderer, el) {
            this.router = router, this.route = route, this.commands = [], null == tabIndex && renderer.setAttribute(el.nativeElement, "tabindex", "0");
          }
          return Object.defineProperty(RouterLink.prototype, "routerLink", {
            set: function(commands) {
              this.commands = null != commands ? Array.isArray(commands) ? commands : [commands] : [];
            },
            enumerable: !0,
            configurable: !0
            ...
            return this.router.navigateByUrl(this.urlTree, extras), !0;
        ...
        RouterLinkWithHref = function() {
          function RouterLinkWithHref(router, route, locationStrategy) {
            var _this = this;
            this.router = router, this.route = route, this.locationStrategy = locationStrategy, this.commands = [], this.subscription = router.events.subscribe(function(s) {
              s instanceof NavigationEnd && _this.updateTargetUrlAndHref();
            });
          }
          return Object.defineProperty(RouterLinkWithHref.prototype, "routerLink", {
            set: function(commands) {
              this.commands = null != commands ? Array.isArray(commands) ? commands : [commands] : [];
            },
            enumerable: !0,
            ...
            if (0 !== button || ctrlKey || metaKey || shiftKey)
              return !0;
        ...
        RouterLinkActive = function() {
          function RouterLinkActive(router, element, renderer, cdr) {
            var _this = this;
            this.router = router, this.element = element, this.renderer = renderer, this.cdr = cdr, this.classes = [], this.isActive = !1, this.routerLinkActiveOptions = {exact: !1}, this.subscription = router.events.subscribe(function(s) {
              s instanceof NavigationEnd && _this.update();
            });
          }
          return RouterLinkActive.prototype.ngAfterContentInit = function() {
            var _this = this;
            ...
            this.links && this.linksWithHrefs && this.router.navigated && Promise.resolve().then(function() {
              ...
              _this.isActive !== hasActiveLinks && (_this.isActive = hasActiveLinks, _this.classes.forEach(function(c) {
                hasActiveLinks ? _this.renderer.addClass(_this.element.nativeElement, c) : _this.renderer.removeClass(_this.element.nativeElement, c);
              }));
            });
            ...
            linksWithHrefs: [{
              type: _angular_core.ContentChildren,
              args: [RouterLinkWithHref, {descendants: !0}]
            }],
            routerLinkActiveOptions: [{type: _angular_core.Input}],
            routerLinkActive: [{type: _angular_core.Input}]
          }, RouterLinkActive;
        }(),
        OutletContext = function() {
          function OutletContext() {
            this.outlet = null, this.route = null, this.resolver = null, this.children = new ChildrenOutletContexts, this.attachRef = null;
          }
          return OutletContext;
        }(),
        ChildrenOutletContexts = function() {
          function ChildrenOutletContexts() {
            this.contexts = new Map;
          }
          return ChildrenOutletContexts.prototype.onChildOutletCreated = function(childName, outlet) {
            ...
            var context = this.getContext(childName);
            return context || (context = new OutletContext, this.contexts.set(childName, context)), context;
        ...
        RouterOutlet = function() {
          function RouterOutlet(parentContexts, location, resolver, name, changeDetector) {
            this.parentContexts = parentContexts, this.location = location, this.resolver = resolver, this.changeDetector = changeDetector, this.activated = null, this._activatedRoute = null, this.activateEvents = new _angular_core.EventEmitter, this.deactivateEvents = new _angular_core.EventEmitter, this.name = name || PRIMARY_OUTLET, parentContexts.onChildOutletCreated(this.name, this);
          }
          return RouterOutlet.prototype.ngOnDestroy = function() {
            ...
            if (!this.activated) {
              ...
              context && context.route && (context.attachRef ? this.attach(context.attachRef, context.route) : this.activateWith(context.route, context.resolver || null));
            }
            ...
            this.activated = this.location.createComponent(factory, this.location.length, injector), this.changeDetector.markForCheck(), this.activateEvents.emit(this.activated.instance);
          ...
          return RouterPreloader.prototype.setUpPreloading = function() {
            ...
            for (var res = [],
              ...
              if (route.loadChildren && !route.canLoad && route._loadedConfig) {
                ...
                route.loadChildren && !route.canLoad ? res.push(this.preloadConfig(ngModule, route)) : route.children && res.push(this.processRoutes(ngModule, route.children));
            }
        ...
        ROUTER_PROVIDERS = [_angular_common.Location, {
        ...
        RouterModule = function() {
          ...
          return RouterModule.forRoot = function(routes, config) {
            return {
        ...
        RouterInitializer = function() {
          ...
          return RouterInitializer.prototype.appInitializer = function() {
            var _this = this;
            return this.injector.get(_angular_common.LOCATION_INITIALIZED, Promise.resolve(null)).then(function() {
              var resolve = null,
                  res = new Promise(function(r) {
                    return resolve = r;
                  }),
                  router = _this.injector.get(Router),
                  opts = _this.injector.get(ROUTER_CONFIGURATION);
            ...
            var opts = this.injector.get(ROUTER_CONFIGURATION),
                ...
                ref = this.injector.get(_angular_core.ApplicationRef);
            bootstrappedComponentRef === ref.components[0] && (this.isLegacyEnabled(opts) ? router.initialNavigation() : this.isLegacyDisabled(opts) && router.setUpLocationChangeListener(), preloader.setUpPreloading(), router.resetRootComponentType(ref.componentTypes[0]), this.resultOfPreactivationDone.next(null), this.resultOfPreactivationDone.complete());
        ...
        ROUTER_INITIALIZER = new _angular_core.InjectionToken("Router Initializer"),
        VERSION = new _angular_core.Version("5.2.3");
