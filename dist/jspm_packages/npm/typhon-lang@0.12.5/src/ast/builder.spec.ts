...
import { Module, FunctionDef, Name, Arguments, Statement, Attribute, FunctionParamDef } from './types';
    ...
    */
    ...
    describe("Function testing", function () {
        ...
        it('def foo(x):\n pass', function () {
            ...
            const ast = new Module(astFromParse(cst));
            const dump = astDump(ast);
            expect(dump).toBe('Module(body=[FunctionDef(name=foo,args=Arguments(args=[FunctionParamDef(name=Name(id=x,ctx=Param()),type=None)],vararg=None,kwarg=None,defaults=[]),body=[Pass()],returnType=None,decorator_list=[])])');
        });
        ...
        it('def foo(x: num):\n pass', function () {
            ...
            const ast = new Module(astFromParse(cst));
            const dump = astDump(ast);
            expect(dump).toBe('Module(body=[FunctionDef(name=foo,args=Arguments(args=[FunctionParamDef(name=Name(id=test,ctx=Param()),type=Name(id=num,ctx=Load()))],vararg=None,kwarg=None,defaults=[]),body=[Pass()],returnType=None,decorator_list=[])])');
        });
        ...
        it('def foo(x) -> GG:\n pass', function () {
            ...
            if (ast instanceof Module) {
                ...
                if (body instanceof FunctionDef) {
                    ...
                    if (args instanceof Arguments) {
                        const intoArgs: FunctionParamDef[] = args.args;
                        const arg0id = intoArgs[0].name.id;
            ...
            expect(dump).toBe('Module(body=[FunctionDef(name=foo,args=Arguments(args=[FunctionParamDef(name=Name(id=x,ctx=Param()),type=None)],vararg=None,kwarg=None,defaults=[]),body=[Pass()],returnType=Name(id=GG,ctx=Load()),decorator_list=[])])');
        });
        ...
        it('def foo(x, y):\n pass', function () {
            ...
            const ast = new Module(astFromParse(cst));
            const dump = astDump(ast);
            expect(dump).toBe('Module(body=[FunctionDef(name=foo,args=Arguments(args=[FunctionParamDef(name=Name(id=x,ctx=Param()),type=None),FunctionParamDef(name=Name(id=y,ctx=Param()),type=None)],vararg=None,kwarg=None,defaults=[]),body=[Pass()],returnType=None,decorator_list=[])])');
        });
