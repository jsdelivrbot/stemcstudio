...
var warningMessages = require('../lib/warning_messages.json!systemjs-json');
...
tap.test("empty object", function(t) {
  ...
  t.same(warnings, [warningMessages.missingDescription, warningMessages.missingRepository, warningMessages.missingReadme, warningMessages.missingLicense]);
  t.end();
});
tap.test("core module name", function(t) {
  ...
  var expect = [safeFormat(warningMessages.conflictingName, 'http'), warningMessages.nonEmailUrlBugsString, warningMessages.emptyNormalizedBugs, warningMessages.nonUrlHomepage, warningMessages.missingLicense];
...
tap.test("urls required", function(t) {
  ...
  var expect = [warningMessages.missingDescription, warningMessages.missingRepository, warningMessages.nonUrlBugsUrlField, warningMessages.nonEmailBugsEmailField, warningMessages.emptyNormalizedBugs, warningMessages.missingReadme, warningMessages.missingLicense, warningMessages.nonEmailUrlBugsString, warningMessages.emptyNormalizedBugs, warningMessages.nonUrlHomepage, warningMessages.missingLicense];
  t.same(warnings, expect);
  t.end();
});
tap.test("homepage field must start with a protocol.", function(t) {
  ...
  var expect = [warningMessages.missingDescription, warningMessages.missingRepository, warningMessages.missingReadme, warningMessages.missingProtocolHomepage, warningMessages.missingLicense];
  t.same(warnings, expect);
...
tap.test("license field should be a valid SPDX expression", function(t) {
  ...
  var a;
  normalize(a = {license: 'Apache 2'}, warn);
  var expect = [warningMessages.missingDescription, warningMessages.missingRepository, warningMessages.missingReadme, warningMessages.invalidLicense];
  t.same(warnings, expect);
  t.end();
});
tap.test("gist bugs url", function(t) {
  var d = {repository: "git@gist.github.com:123456.git"};
  normalize(d);
  t.same(d.repository, {
    type: 'git',
    url: 'git+ssh://git@gist.github.com/123456.git'
  });
...
tap.test("treat visionmedia/express as github repo", function(t) {
  ...
  t.same(d.repository, {
    type: "git",
    url: "git+https://github.com/visionmedia/express.git"
  });
  t.end();
});
