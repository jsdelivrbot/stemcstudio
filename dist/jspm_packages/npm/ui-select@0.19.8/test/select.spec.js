...
(function(process) {
  ...
  describe('ui-select tests', function() {
    ...
    function createUiSelect(attrs) {
      ...
      return compileTemplate('<ui-select ng-model="selection.selected"' + attrsHtml + '> \
    ...
    function triggerPaste(element, text, isClipboardEvent) {
      ...
      if (isClipboardEvent) {
        e.clipboardData = {getData: function() {
            return text;
          }};
      } else {
        e.originalEvent = {clipboardData: {getData: function() {
              return text;
            }}};
      }
      element.trigger(e);
    ...
    it('should merge both ng-class attributes defined on ui-select and its templates', function() {
      var el = createUiSelect({ngClass: "{class: expression}"});
    ...
    it('should correctly render initial state with track by feature', function() {
      var el = compileTemplate('<ui-select ng-model="selection.selected"> \
        <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
    ...
    it('should clear selection (with object as source)', function() {
      var el = compileTemplate('<ui-select ng-model="selection.selected" theme="select2"> \
    ...
    describe('disabled options', function() {
      function createUiSelect(attrs) {
        ...
        return compileTemplate('<ui-select ng-model="selection.selected"> \
          <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
      ...
      describe('disable on truthy property', function() {
        beforeEach(function() {
          ...
          this.el = createUiSelect({disabled: 'person.inactive'});
        });
      ...
      describe('disable on inverse property check', function() {
        beforeEach(function() {
          ...
          this.el = createUiSelect({disabled: '!person.active'});
        });
        it('should allow the user to define the selected option', function() {
      ...
      describe('disable on expression', function() {
        beforeEach(function() {
          ...
          this.el = createUiSelect({disabled: "person.status == 'inactive'"});
        });
        it('should allow the user to define the selected option', function() {
    ...
    describe('choices group', function() {
      ...
      it('should change activeItem through groups', function() {
        ...
        triggerKeydown(el.find('input'), 40);
        scope.$digest();
    ...
    it('should invoke select callback on select', function() {
      ...
      var el = compileTemplate('<ui-select on-select="onSelectFn($item, $model)" ng-model="selection.selected"> \
        <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
        <ui-select-choices repeat="person.name as person in people | filter: $select.search"> \
          <div ng-bind-html="person.name | highlight: $select.search"></div> \
          <div ng-bind-html="person.email | highlight: $select.search"></div> \
    ...
    it('should invoke remove callback on remove', function() {
      ...
      var el = compileTemplate('<ui-select multiple on-remove="onRemoveFn($item, $model)" ng-model="selection.selected"> \
        <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
        <ui-select-choices repeat="person.name as person in people | filter: $select.search"> \
          <div ng-bind-html="person.name" | highlight: $select.search"></div> \
          <div ng-bind-html="person.email | highlight: $select.search"></div> \
    ...
    it('should call open-close callback with isOpen state as first argument on open and on close', function() {
      var el = compileTemplate('<ui-select uis-open-close="onOpenCloseFn(isOpen)" ng-model="selection.selected"> \
        <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
        <ui-select-choices repeat="person.name as person in people | filter: $select.search"> \
          <div ng-bind-html="person.name | highlight: $select.search"></div> \
          <div ng-bind-html="person.email | highlight: $select.search"></div> \
    ...
    it('should allow creating tag in single select mode with tagging enabled', function() {
      ...
      var el = compileTemplate('<ui-select ng-model="selection.selected" tagging="taggingFunc" tagging-label="false"> \
        <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
        <ui-select-choices repeat="person in people | filter: $select.search"> \
          <div ng-bind-html="person.name" | highlight: $select.search"></div> \
          <div ng-bind-html="person.email | highlight: $select.search"></div> \
    ...
    it('should allow creating tag on ENTER in multiple select mode with tagging enabled, no labels', function() {
      ...
      var el = compileTemplate('<ui-select multiple ng-model="selection.selected" tagging="taggingFunc" tagging-label="false"> \
          <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
          <ui-select-choices repeat="person in people | filter: $select.search"> \
            <div ng-bind-html="person.name" | highlight: $select.search"></div> \
            <div ng-bind-html="person.email | highlight: $select.search"></div> \
    ...
    it('should remove a choice when multiple and remove-selected is not given (default is true)', function() {
      var el = compileTemplate('<ui-select multiple ng-model="selection.selected"> \
        <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
        <ui-select-choices repeat="person in people | filter: $select.search"> \
    ...
    it('should disable a choice instead of removing it when remove-selected is false', function() {
      var el = compileTemplate('<ui-select multiple remove-selected="false" ng-model="selection.selected"> \
        <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
        <ui-select-choices repeat="person in people | filter: $select.search"> \
          <div ng-bind-html="person.name" | highlight: $select.search"></div> \
          <div ng-bind-html="person.email | highlight: $select.search"></div> \
      ...
      var choicesEls = $(el).find('.ui-select-choices-row');
      expect(choicesEls.length).toEqual(8);
      [false, false, false, true, false, true, false, false].forEach(function(bool, index) {
    ...
    it('should retain an invalid view value after refreshing items', function() {
      ...
      var el = compileTemplate('<ui-select ng-model="selection.selected" tagging="taggingFunc" tagging-label="false" test-validator> \
    ...
    describe('search-enabled option', function() {
      ...
      function setupSelectComponent(searchEnabled, theme) {
        el = compileTemplate('<ui-select ng-model="selection.selected" theme="' + theme + '" search-enabled="' + searchEnabled + '"> \
          <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
          <ui-select-choices repeat="person in people | filter: $select.search"> \
            <div ng-bind-html="person.name | highlight: $select.search"></div> \
            <div ng-bind-html="person.email | highlight: $select.search"></div> \
    ...
    describe('multi selection', function() {
      function createUiSelectMultiple(attrs) {
        ...
        return compileTemplate('<ui-select multiple ng-model="selection.selectedMultiple"' + attrsHtml + ' theme="bootstrap" style="width: 800px;"> \
      ...
      it('should render intial state with data-multiple attribute', function() {
        ...
        var el = compileTemplate('<ui-select data-multiple ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
      ...
      it('should render intial state with x-multiple attribute', function() {
        scope.selection.selectedMultiple = [scope.people[0], scope.people[1]];
        var el = compileTemplate('<ui-select x-multiple ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
            <ui-select-match placeholder="Pick one...">{{$item.name}} &lt;{{$item.email}}&gt;</ui-select-match> \
            <ui-select-choices repeat="person in people | filter: $select.search"> \
              <div ng-bind-html="person.name | highlight: $select.search"></div> \
              <div ng-bind-html="person.email | highlight: $select.search"></div> \
      ...
      it('should update size of search input use container width', function() {
        scope.selection.selectedMultiple = [scope.people[4], scope.people[5]];
        var el = createUiSelectMultiple({appendToBody: true});
      ...
      it('should remove highlighted match when pressing BACKSPACE key from search and decrease activeMatchIndex', function() {
        ...
        triggerKeydown(searchInput, Key.Left);
        triggerKeydown(searchInput, Key.Backspace);
        expect(el.scope().$select.selected).toEqual([scope.people[4], scope.people[6]]);
      ...
      it('should NOT remove highlighted match when pressing BACKSPACE key on a locked choice', function() {
        ...
        triggerKeydown(searchInput, Key.Left);
        triggerKeydown(searchInput, Key.Backspace);
        expect(el.scope().$select.selected).toEqual([scope.people[4], scope.people[5], scope.people[6]]);
      ...
      it('should search/open dropdown when writing to search input', function() {
        scope.selection.selectedMultiple = [scope.people[5]];
        var el = createUiSelectMultiple();
        var searchInput = el.find('.ui-select-search');
        el.scope().$select.search = 'r';
        scope.$digest();
      ...
      it('should add selected match to selection array', function() {
        ...
        var searchInput = el.find('.ui-select-search');
        clickItem(el, 'Wladimir');
        expect(scope.selection.selectedMultiple).toEqual([scope.people[5], scope.people[4]]);
      });
      ...
      it('should parse the items correctly using single property binding', function() {
        ...
        var el = compileTemplate('<ui-select multiple ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
              <ui-select-match placeholder="Pick one...">{{$item.name}} &lt;{{$item.email}}&gt;</ui-select-match> \
      ...
      it('should format view value correctly when using single property binding and refresh function', function() {
        ...
        setSearchText(el, 'n');
        clickItem(el, 'Nicole');
        expect(el.find('.ui-select-match-item [uis-transclude-append]:not(.ng-hide)').text()).toBe("Wladimir <wladimir@email.com>Samantha <samantha@email.com>Nicole <nicole@email.com>");
        setSearchText(el, 'o');
        expect(el.find('.ui-select-match-item [uis-transclude-append]:not(.ng-hide)').text()).toBe("Wladimir <wladimir@email.com>Samantha <samantha@email.com>Nicole <nicole@email.com>");
      });
      it('should watch changes for $select.selected and update formatted value correctly', function() {
        ...
        expect(el.find('.ui-select-match-item [uis-transclude-append]:not(.ng-hide)').text()).toBe("Wladimir <wladimir@email.com>Samantha <samantha@email.com>");
        clickItem(el, 'Nicole');
        expect(el.find('.ui-select-match-item [uis-transclude-append]:not(.ng-hide)').text()).toBe("Wladimir <wladimir@email.com>Samantha <samantha@email.com>Nicole <nicole@email.com>");
      ...
      it('should watch changes for $select.selected and refresh choices correctly', function() {
        ...
        var el = compileTemplate('<ui-select multiple ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
              ...
              ');
        scope.selection.selectedMultiple.splice(0, 1);
        var searchInput = el.find('.ui-select-search');
        triggerKeydown(searchInput, Key.Down);
        expect(el.find('.ui-select-choices-content').text()).toContain("wladimir@email.com");
      });
      it('should ensure the multiple selection limit is respected', function() {
        scope.selection.selectedMultiple = ['wladimir@email.com'];
        var el = compileTemplate('<ui-select multiple limit="2" ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
                  <ui-select-match placeholder="Pick one...">{{$item.name}} &lt;{{$item.email}}&gt;</ui-select-match> \
                  <ui-select-choices repeat="person.email as person in people | filter: $select.search"> \
                    <div ng-bind-html="person.name | highlight: $select.search"></div> \
                    <div ng-bind-html="person.email | highlight: $select.search"></div> \
              ...
              ');
        var el2 = compileTemplate('<span class="resultDiv" ng-bind="selection.selectedMultiple"></span>');
        expect(el.find('.ui-select-match-item [uis-transclude-append]:not(.ng-hide)').text()).toBe("Wladimir <wladimir@email.com>");
        clickItem(el, 'Samantha');
        expect(el.find('.ui-select-match-item [uis-transclude-append]:not(.ng-hide)').text()).toBe("Wladimir <wladimir@email.com>Samantha <samantha@email.com>");
        clickItem(el, 'Nicole');
        expect(el.find('.ui-select-match-item [uis-transclude-append]:not(.ng-hide)').text()).toBe("Wladimir <wladimir@email.com>Samantha <samantha@email.com>");
      ...
      it('should change viewvalue only once when updating modelvalue', function() {
        scope.selection.selectedMultiple = ['wladimir@email.com', 'samantha@email.com'];
        var el = compileTemplate('<ui-select ng-change="onlyOnce()" multiple ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
              <ui-select-match placeholder="Pick one...">{{$item.name}} &lt;{{$item.email}}&gt;</ui-select-match> \
              <ui-select-choices repeat="person.email as person in people | filter: $select.search"> \
                <div ng-bind-html="person.name | highlight: $select.search"></div> \
                <div ng-bind-html="person.email | highlight: $select.search"></div> \
      ...
      it('should retain an invalid view value after refreshing items', function() {
        ...
        var el = compileTemplate('<ui-select multiple ng-model="selection.selectedMultiple" tagging="taggingFunc" tagging-label="false" test-validator> \
            <ui-select-match placeholder="Pick one...">{{$select.selected.email}}</ui-select-match> \
            <ui-select-choices repeat="person in people | filter: $select.search"> \
              <div ng-bind-html="person.name" | highlight: $select.search"></div> \
              <div ng-bind-html="person.email | highlight: $select.search"></div> \
      ...
      it('should support multiple="multiple" attribute', function() {
        var el = compileTemplate('<ui-select multiple="multiple" ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
              <ui-select-match placeholder="Pick one...">{{$item.name}} &lt;{{$item.email}}&gt;</ui-select-match> \
              <ui-select-choices repeat="person.email as person in people | filter: $select.search"> \
                <div ng-bind-html="person.name | highlight: $select.search"></div> \
                <div ng-bind-html="person.email | highlight: $select.search"></div> \
      ...
      it('should preserve the model if tagging is enabled on select multiple', function() {
        ...
        var el = compileTemplate('<ui-select multiple="multiple" tagging ng-model="selection.selectedMultiple" theme="bootstrap" style="width: 800px;"> \
              <ui-select-match placeholder="Pick one...">{{$item.name}} &lt;{{$item.email}}&gt;</ui-select-match> \
              <ui-select-choices repeat="person.email as person in people | filter: $select.search"> \
                <div ng-bind-html="person.name | highlight: $select.search"></div> \
                <div ng-bind-html="person.email | highlight: $select.search"></div> \
          ...
          ');
        scope.$digest();
        expect(scope.selection.selectedMultiple).toEqual(["I am not on the list of choices"]);
      });
      ...
      it('should allow decline tags when tagging function returns null in multiple select mode', function() {
        ...
        expect(el.scope().$select.items[0]).toEqual(jasmine.objectContaining({
          name: 'amalie',
          isTag: true
        }));
      ...
      it('should allow creating tag in multi select mode with tagging and group-by enabled', function() {
        ...
        expect(el.scope().$select.items[1]).toEqual(jasmine.objectContaining({
          name: 'Amalie',
          email: 'amalie@email.com'
        }));
      });
      it('should have tolerance for undefined values', function() {
        ...
        var el = compileTemplate('<ui-select multiple ng-model="modelValue" theme="bootstrap" style="width: 800px;"> \
              <ui-select-match placeholder="Pick one...">{{$item.name}} &lt;{{$item.email}}&gt;</ui-select-match> \
              <ui-select-choices repeat="person.email as person in people | filter: $select.search"> \
                <div ng-bind-html="person.name | highlight: $select.search"></div> \
                <div ng-bind-html="person.email | highlight: $select.search"></div> \
      ...
      it('should only have 1 item selected and onSelect function should only be handled once.', function() {
        ...
        var el = createUiSelectMultiple({
          limit: 1,
          onSelect: 'onSelectFn($item, $model)'
        });
        expect(scope.$item).toBeFalsy();
        expect(scope.$model).toBeFalsy();
        clickItem(el, 'Samantha');
    ...
    describe('default configuration via uiSelectConfig', function() {
      describe('searchEnabled option', function() {
        ...
        function setupWithAttr(searchEnabled) {
          return compileTemplate('<ui-select ng-model="selection.selected" search-enabled="' + searchEnabled + '"> \
            <ui-select-match placeholder="Pick one...">{{$select.selected.name}}</ui-select-match> \
            <ui-select-choices repeat="person in people | filter: $select.search"> \
              <div ng-bind-html="person.name | highlight: $select.search"></div> \
              <div ng-bind-html="person.email | highlight: $select.search"></div> \
    ...
    describe('With refresh on active', function() {
      it('should refresh when is activated', function() {
        scope.fetchFromServer = function() {};
        var el = createUiSelect({
          refresh: "fetchFromServer($select.search)",
          refreshDelay: 0
        });
        spyOn(scope, 'fetchFromServer');
